...
export class CreateAppCommand {
    ...
    const { startDocker } = await inquirer.prompt([
      {
        ...
        name: 'startDocker',
        message:
          'No running Aegis instance found. Would you like to start one using Docker?',
        default: true,
      },
    ]);
  ...
  private async promptConnectToLocal(serverUrl: string): Promise<void> {
    const { shouldAuthenticate } = await inquirer.prompt([
      {
        ...
        message: `Would you like to authenticate to the local Aegis instance (${serverUrl})?`,
        default: true,
      },
    ]);
    ...
    if (!shouldAuthenticate) {
      console.log(
        chalk.gray(
          'Authentication skipped. Run `yarn aegis remote add --local` manually.',
        ),
      );
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          chalk.yellow(
            'Authentication failed. Run `yarn aegis remote add --local` manually.',
          ),
        );
      }
    } catch {
      console.log(
        chalk.yellow(
          'Authentication failed. Run `yarn aegis remote add` manually.',
        ),
      );
    }
  }
  ...
  private logSuccess(
    ...
    if (!serverResult) {
      console.log(
        chalk.gray(
          '- yarn aegis remote add          # Authenticate with Aegis',
        ),
      );
    }

    console.log(
      chalk.gray('- yarn aegis dev                  # Start dev mode'),
    );
  }
}
