...
import { resolveRolePermissionConfig } from 'src/engine/aegis-orm/utils/resolve-role-permission-config.util';
...
> {
  ...
  @Inject()
  protected readonly aegisConfigService: AegisConfigService;
  @Inject()
  ...
  private async throttleQueryExecution(authContext: WorkspaceAuthContext) {
    try {
      ...
      const shortConfig = {
        ...
        maxTokens: this.aegisConfigService.get('API_RATE_LIMITING_SHORT_LIMIT'),
        timeWindow: this.aegisConfigService.get(
      ...
      const longConfig = {
        ...
        maxTokens: this.aegisConfigService.get('API_RATE_LIMITING_LONG_LIMIT'),
        timeWindow: this.aegisConfigService.get(
  ...
  private validateQueryComplexity(
    ...
    const maximumComplexity = this.aegisConfigService.get(
