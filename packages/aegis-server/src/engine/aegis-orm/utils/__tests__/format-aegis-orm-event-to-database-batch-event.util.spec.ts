import { type ObjectRecordUpdateEvent } from 'aegis-shared/database-events';
...
describe('formatAegisOrmEventToDatabaseBatchEvent', () => {
  ...
  describe('UPDATED action', () => {
    it('should throw AegisORMException when no matching before entity is found in array of beforeEntities', () => {
      const afterEntities = [
        {
          id: 'record-1',
      ...
      try {
        ...
        expect(error).toBeInstanceOf(AegisORMException);
        expect((error as AegisORMException).code).toBe(
          AegisORMExceptionCode.ORM_EVENT_DATA_CORRUPTED,
        );
        expect((error as AegisORMException).message).toBe(
    ...
    it('should successfully create update events when matching before entities are found and in the right order', () => {
      ...
      const result = formatAegisOrmEventToDatabaseBatchEvent({
        action: DatabaseEventAction.UPDATED,
        objectMetadataItem: flatObjectMetadata,
        flatFieldMetadataMaps,
        workspaceId: mockWorkspaceId,
