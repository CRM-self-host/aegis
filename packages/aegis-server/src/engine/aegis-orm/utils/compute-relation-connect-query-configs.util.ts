...
import { type ConnectObject } from 'src/engine/aegis-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import {
  type RelationConnectQueryConfig,
  type UniqueConstraintCondition,
} from 'src/engine/aegis-orm/entity-manager/types/relation-connect-query-config.type';
import { type RelationConnectQueryFieldsByEntityIndex } from 'src/engine/aegis-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  AegisORMException,
  AegisORMExceptionCode,
} from 'src/engine/aegis-orm/exceptions/aegis-orm.exception';
import { formatCompositeField } from 'src/engine/aegis-orm/utils/format-data.util';
...
const computeRecordToConnectCondition = (
  ...
  if (
    ...
    throw new AegisORMException(
      ...
      AegisORMExceptionCode.CONNECT_NOT_ALLOWED,
      {
...
const checkUniqueConstraintFullyPopulated = (
  ...
  if (!hasUniqueConstraintFieldFullyPopulated) {
    throw new AegisORMException(
      ...
      AegisORMExceptionCode.CONNECT_UNIQUE_CONSTRAINT_ERROR,
      {
