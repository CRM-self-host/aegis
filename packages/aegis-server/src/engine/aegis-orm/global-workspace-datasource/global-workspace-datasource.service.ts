...
import { GlobalWorkspaceDataSource } from 'src/engine/aegis-orm/global-workspace-datasource/global-workspace-datasource';
  ...
  implements OnModuleInit, OnApplicationShutdown
{
  ...
  async onModuleInit(): Promise<void> {
    this.globalWorkspaceDataSource = new GlobalWorkspaceDataSource(
      {
        url: this.aegisConfigService.get('PG_DATABASE_URL'),
        type: 'postgres',
        logging: this.aegisConfigService.getLoggingConfig(),
        entities: [],
        ssl: this.aegisConfigService.get('PG_SSL_ALLOW_SELF_SIGNED')
          ? {
              rejectUnauthorized: false,
            }
          : undefined,
        poolSize: this.aegisConfigService.get('PG_POOL_MAX_CONNECTIONS'),
        extra: {
          query_timeout: this.aegisConfigService.get(
            'PG_DATABASE_PRIMARY_TIMEOUT_MS',
          ),
          idleTimeoutMillis: this.aegisConfigService.get(
            'PG_POOL_IDLE_TIMEOUT_MS',
          ),
          allowExitOnIdle: this.aegisConfigService.get(
    ...
    const shouldInitializeReplicaDataSource = isDefined(
      this.aegisConfigService.get('PG_DATABASE_REPLICA_URL'),
    );

    if (shouldInitializeReplicaDataSource) {
      this.globalWorkspaceDataSourceReplica = new GlobalWorkspaceDataSource(
        {
          url: this.aegisConfigService.get('PG_DATABASE_REPLICA_URL'),
          type: 'postgres',
          logging: this.aegisConfigService.getLoggingConfig(),
          entities: [],
          ssl: this.aegisConfigService.get('PG_SSL_ALLOW_SELF_SIGNED')
