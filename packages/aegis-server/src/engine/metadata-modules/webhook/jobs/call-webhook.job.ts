...
import { ensureAbsoluteUrl } from 'aegis-shared/utils';
...
export class CallWebhookJob {
  ...
  private async callWebhook(data: WebhookJobData): Promise<void> {
    ...
    try {
      ...
      if (secret) {
        headers['X-Aegis-Webhook-Timestamp'] = Date.now().toString();
        headers['X-Aegis-Webhook-Signature'] = this.generateSignature(
          payloadWithoutSecret,
          secret,
          headers['X-Aegis-Webhook-Timestamp'],
        );
        headers['X-Aegis-Webhook-Nonce'] = crypto
          .randomBytes(16)
          .toString('hex');
      }
