...
import {
  AEGIS_ALL_VERSIONS,
  type AegisAllVersion,
} from 'src/engine/core-modules/upgrade/constants/aegis-all-versions.constant';
import { AEGIS_CROSS_UPGRADE_SUPPORTED_VERSIONS } from 'src/engine/core-modules/upgrade/constants/aegis-cross-upgrade-supported-version.constant';
...
export class UpgradeCommandRegistryService implements OnModuleInit {
  ...
  private readonly bundlesByVersion = new Map<AegisAllVersion, VersionBundle>();

  constructor(private readonly discoveryService: DiscoveryService) {}

  onModuleInit(): void {
    for (const version of AEGIS_ALL_VERSIONS) {
    ...
    for (const [version, bundle] of this.bundlesByVersion) {
      ...
      if (totalCount > 0) {
        const crossUpgradeLabel = (
          AEGIS_CROSS_UPGRADE_SUPPORTED_VERSIONS as readonly string[]
        ).includes(version)
  ...
  getBundleForVersion(version: AegisAllVersion): VersionBundle {
  ...
  getCrossUpgradeSupportedFastInstanceCommands(): RegisteredFastInstanceCommand[] {
    return AEGIS_CROSS_UPGRADE_SUPPORTED_VERSIONS.flatMap(
  ...
  private validateAtLeastOneVersionBundleHasWorkspaceCommands(): void {
    ...
    for (const version of AEGIS_CROSS_UPGRADE_SUPPORTED_VERSIONS) {
      const bundle = this.getBundleForVersion(version);
  ...
  private validateNoVersionDuplicatesAcrossConstants(): void {
    ...
    if (uniqueVersions.size !== allVersions.length) {
      ...
      throw new Error(
        `Duplicate version(s) across AEGIS_PREVIOUS_VERSIONS, AEGIS_CURRENT_VERSION, and AEGIS_NEXT_VERSIONS: ${duplicates.join(', ')}`,
      );
    }
  }

  private validatePreviousVersionsNotEmpty(): void {
    if ((AEGIS_PREVIOUS_VERSIONS as readonly string[]).length === 0) {
      throw new Error(
        'AEGIS_PREVIOUS_VERSIONS must contain at least one version before AEGIS_CURRENT_VERSION',
      );
    }
  }
}
