...
export class ApplicationService {
  ...
  async findWorkspaceAegisStandardAndCustomApplicationOrThrow({
    ...
    const aegisStandardApplicationId =
      flatApplicationMaps.idByUniversalIdentifier[
        AEGIS_STANDARD_APPLICATION.universalIdentifier
      ];

    if (!isDefined(aegisStandardApplicationId)) {
      throw new ApplicationException(
    ...
    const aegisStandardFlatApplication =
      flatApplicationMaps.byId[aegisStandardApplicationId];
    const workspaceCustomFlatApplication =
      flatApplicationMaps.byId[workspace.workspaceCustomApplicationId];

    if (
      !isDefined(aegisStandardFlatApplication) ||
  ...
  async findAegisStandardApplicationOrThrow(workspaceId: string): Promise<{
    ...
    const { aegisStandardFlatApplication } =
      await this.findWorkspaceAegisStandardAndCustomApplicationOrThrow({
        workspace,
      });

    const application = await this.applicationRepository.findOne({
      where: {
        id: aegisStandardFlatApplication.id,
  ...
  async createAegisStandardApplication(
    {
      workspaceId,
    ...
    const aegisStandardApplication = await this.create(
      {
        ...AEGIS_STANDARD_APPLICATION,
    ...
    return aegisStandardApplication;
  }
