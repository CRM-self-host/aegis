...
import { AEGIS_MCP_HELPER } from 'src/engine/core-modules/tool/tools/code-interpreter-tool/aegis-mcp-helper.const';
import {
...
export class CodeInterpreterTool implements Tool {
  ...
  async execute(
    ...
    try {
      ...
      const codeWithHelper = AEGIS_MCP_HELPER + '\n\n' + code;
      ...
      const result = await this.codeInterpreterService.execute(
        ...
        {
          env: {
            AEGIS_SERVER_URL: serverUrl,
            AEGIS_API_TOKEN: sessionToken,
          },
        },
        {
          onStdout: (line) => {
