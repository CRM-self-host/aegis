...
export class SearchHelpCenterTool implements Tool {
  description =
    'Search Aegis documentation and help center to find information about features, setup, usage, and troubleshooting.';
  ...
  async execute(
    ...
    try {
      const MINTLIFY_API_KEY = this.aegisConfigService.get('MINTLIFY_API_KEY');
      const MINTLIFY_SUBDOMAIN =
        this.aegisConfigService.get('MINTLIFY_SUBDOMAIN');
      ...
      const endpoint = useDirectApi
        ...
        : 'https://aegis-help-search.com/search/aegis';
