...
describe('DnsManagerService', () => {
  ...
  it('should initialize cloudflareClient when CLOUDFLARE_API_KEY is defined', () => {
    ...
    jest.spyOn(aegisConfigService, 'get').mockReturnValue(mockApiKey);

    const instance = new DnsManagerService(aegisConfigService, {} as any);

    expect(aegisConfigService.get).toHaveBeenCalledWith('CLOUDFLARE_API_KEY');
  ...
  describe('registerHostname', () => {
    ...
    it('should register a custom domain successfully', async () => {
      ...
      jest.spyOn(aegisConfigService, 'get').mockReturnValue('test-zone-id');
