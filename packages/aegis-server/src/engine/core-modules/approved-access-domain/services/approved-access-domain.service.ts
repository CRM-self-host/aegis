...
import { SendApprovedAccessDomainValidation } from 'aegis-emails';
import { FileFolder, SettingsPath } from 'aegis-shared/types';
import { getSettingsPath, isDefined } from 'aegis-shared/utils';
...
export class ApprovedAccessDomainService {
  ...
  async sendApprovedAccessDomainValidationEmail(
    ...
    const emailTemplate = SendApprovedAccessDomainValidation({
      ...
      serverUrl: this.aegisConfigService.get('SERVER_URL'),
    ...
    await this.emailService.send({
      from: `${sender.name.firstName} ${sender.name.lastName} (via Aegis) <${this.aegisConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to,
    ...
    return crypto
      ...
      .update(
        JSON.stringify({
          ...
          domain: approvedAccessDomain.domain,
          key: this.aegisConfigService.get('APP_SECRET'),
        }),
      )
      .digest('hex');
  }
