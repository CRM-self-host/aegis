...
describe('BillingUsageCapService', () => {
  ...
  let aegisConfigService: jest.Mocked<AegisConfigService>;
  ...
  beforeEach(async () => {
    ...
    aegisConfigService = module.get(AegisConfigService);
  });
  ...
  describe('isClickHouseEnabled', () => {
    it('returns true when CLICKHOUSE_URL is configured', () => {
      aegisConfigService.get.mockReturnValue('http://clickhouse:8123');

      expect(service.isClickHouseEnabled()).toBe(true);
    });

    it('returns false when CLICKHOUSE_URL is empty', () => {
      aegisConfigService.get.mockReturnValue('');
