...
import { type ConfigVariables } from 'src/engine/core-modules/aegis-config/config-variables';
...
jest.mock(
  'src/engine/core-modules/aegis-config/constants/config-variables-masking-config',
  () => ({
...
jest.mock(
  'src/engine/core-modules/aegis-config/utils/is-env-only-config-var.util',
  () => ({
    isEnvOnlyConfigVar: jest.fn(),
  }),
);

type AegisConfigServicePrivateProps = {
...
const setupTestModule = async (isDatabaseConfigEnabled = true) => {
  ...
  return {
    service: module.get<AegisConfigService>(AegisConfigService),
...
const setPrivateProps = (
  service: AegisConfigService,
  props: Partial<AegisConfigServicePrivateProps>,
) => {
...
describe('AegisConfigService', () => {
  let service: AegisConfigService;
