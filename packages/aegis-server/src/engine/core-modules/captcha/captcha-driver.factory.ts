...
export class CaptchaDriverFactory extends DriverFactoryBase<CaptchaDriver | null> {
  ...
  protected buildConfigKey(): string {
    const driver = this.aegisConfigService.get('CAPTCHA_DRIVER');
  ...
  protected createDriver(): CaptchaDriver | null {
    const driver = this.aegisConfigService.get('CAPTCHA_DRIVER');
    const siteKey = this.aegisConfigService.get('CAPTCHA_SITE_KEY');
    const secretKey = this.aegisConfigService.get('CAPTCHA_SECRET_KEY');
