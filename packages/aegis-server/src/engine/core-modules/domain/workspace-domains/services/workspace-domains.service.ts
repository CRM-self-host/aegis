...
export class WorkspaceDomainsService {
  ...
  private async getDefaultWorkspace() {
    if (this.aegisConfigService.get('IS_MULTIWORKSPACE_ENABLED')) {
      throw new Error(
  ...
  private getAegisWorkspaceUrl(subdomain: string) {
    const url = this.domainServerConfigService.getFrontUrl();

    url.hostname = this.aegisConfigService.get('IS_MULTIWORKSPACE_ENABLED')
  ...
  getSubdomainAndCustomDomainFromWorkspaceFallbackOnDefaultSubdomain(
    ...
    if (!workspace) {
      return {
        subdomain: this.aegisConfigService.get('DEFAULT_SUBDOMAIN'),
        customDomain: null,
    ...
    return {
      customUrl:
        isCustomDomainEnabled && customDomain
          ...
          : undefined,
      subdomainUrl: this.getAegisWorkspaceUrl(subdomain),
    };
  }
