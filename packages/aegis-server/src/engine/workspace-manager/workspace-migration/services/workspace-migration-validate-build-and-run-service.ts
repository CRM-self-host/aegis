...
export class WorkspaceMigrationValidateBuildAndRunService {
  ...
  > & {
    ...
    if (!isDefined(aegisStandardApplicationId) || !isDefined(applicationId)) {
    ...
    const isBuildingAegisStandardApplication =
      applicationUniversalIdentifier ===
      AEGIS_STANDARD_APPLICATION.universalIdentifier;

    if (!isBuildingAegisStandardApplication) {
      applicationIds.add(aegisStandardApplicationId);
    }
