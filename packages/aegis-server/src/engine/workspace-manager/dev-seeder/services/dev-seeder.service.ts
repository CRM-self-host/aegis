...
import { AegisStandardApplicationService } from 'src/engine/workspace-manager/aegis-standard-application/services/aegis-standard-application.service';
...
export class DevSeederService {
  constructor(
    ...
    private readonly aegisStandardApplicationService: AegisStandardApplicationService,
  ...
  public async seedDev(
    ...
    const isBillingEnabled = this.aegisConfigService.get('IS_BILLING_ENABLED');
    const appVersion = this.aegisConfigService.get('APP_VERSION') ?? 'unknown';
    ...
    const { workspaceCustomFlatApplication, aegisStandardFlatApplication } =
    ...
    await this.aegisStandardApplicationService.synchronizeAegisStandardApplicationOrThrow(
      {
        workspaceId,
      },
    );
    ...
    await this.sdkClientGenerationService.generateSdkClientForApplication({
      workspaceId,
      applicationId: aegisStandardFlatApplication.id,
    ...
    await seedPageLayouts({
      workspaceId,
      flatApplication: aegisStandardFlatApplication,
      objectMetadataItems,
  ...
  public async seedEmptyWorkspace(
    ...
    try {
      ...
      await this.applicationService.createAegisStandardApplication(
        {
          workspaceId,
