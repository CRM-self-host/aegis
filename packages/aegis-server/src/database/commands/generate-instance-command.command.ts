...
export class GenerateInstanceCommandCommand extends CommandRunner {
  ...
  parseVersion(value: string): AegisAllVersion {
    if (
      !AEGIS_ALL_VERSIONS.includes(value as (typeof AEGIS_ALL_VERSIONS)[number])
    ) {
      throw new Error(
        `Invalid version "${value}". Must be one of: ${AEGIS_ALL_VERSIONS.join(', ')}`,
      );
    }

    return value as AegisAllVersion;
  }
  ...
  async run(
    ...
    const version = options.version ?? AEGIS_CURRENT_VERSION;
