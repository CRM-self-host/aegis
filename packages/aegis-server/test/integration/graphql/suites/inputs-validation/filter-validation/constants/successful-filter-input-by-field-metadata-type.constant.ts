...
export const successfulFilterInputByFieldMetadataType: {
  ...
  [FieldMetadataType.LINKS]: [
    {
      gqlFilterInput: {
        linksField: { primaryLinkUrl: { eq: 'aegis.com' } },
      },
      restFilterInput: 'linksField.primaryLinkUrl[eq]:"aegis.com"',
      validateFilter: (record: Record<string, any>) => {
        return record.linksField.primaryLinkUrl === 'aegis.com';
      },
    },
    {
      gqlFilterInput: {
        linksField: {
          primaryLinkLabel: { eq: 'aegis - #1 Open source CRM' },
        },
      },
      restFilterInput:
        'linksField.primaryLinkLabel[eq]:"aegis - #1 Open source CRM"',
      validateFilter: (record: Record<string, any>) => {
        return (
          record.linksField.primaryLinkLabel === 'aegis - #1 Open source CRM'
        );
      },
    },
    {
      gqlFilterInput: { linksField: { secondaryLinks: { like: '%aegis%' } } },
      restFilterInput: 'linksField.secondaryLinks[like]:"%aegis%"',
      validateFilter: (record: Record<string, any>) => {
