...
const aegisClientTemplateSource = readFileSync(
  join(__dirname, '..', 'aegis-client-template.ts'),
  'utf-8',
);

type AegisClassType = new (options?: {
  url?: string;
...
describe('Generated client wrapper auth behavior', () => {
  ...
  let AegisClass: AegisClassType;

  beforeAll(async () => {
    temporaryDir = await mkdtemp(join(tmpdir(), 'aegis-generated-client-'));

    const wrapperSource = buildClientWrapperSource(aegisClientTemplateSource, {
      apiClientName: 'MetadataApiClient',
      defaultUrl: '`${process.env.AEGIS_API_URL}/metadata`',
    ...
    AegisClass = generatedModule.MetadataApiClient as AegisClassType;
  });

  beforeEach(() => {
    delete process.env.AEGIS_APP_ACCESS_TOKEN;
  ...
  it('uses AEGIS_APP_ACCESS_TOKEN before AEGIS_API_KEY', async () => {
    process.env.AEGIS_APP_ACCESS_TOKEN = 'application-token';
    process.env.AEGIS_API_KEY = 'api-key-token';
    ...
    const aegisClient = new AegisClass({
      url: 'https://example.com/graphql',
      fetch: fetchMock as unknown as typeof globalThis.fetch,
    });

    await aegisClient.query({ record: { id: true } });

    expect(capturedAuthorizationHeaders).toEqual(['Bearer application-token']);
  });

  it('falls back to AEGIS_API_KEY when AEGIS_APP_ACCESS_TOKEN is absent', async () => {
    process.env.AEGIS_API_KEY = 'legacy-api-key-token';
  ...
  it('refreshes and retries once after auth error when refresh callback is available', async () => {
    process.env.AEGIS_APP_ACCESS_TOKEN = 'stale-token';
    process.env.AEGIS_API_KEY = 'legacy-api-key-token';
    ...
    const queryResult = await aegisClient.query({ record: { id: true } });

    expect(queryResult).toEqual({ data: { record: { id: 'record-id' } } });
    expect(requestAccessTokenRefresh).toHaveBeenCalledTimes(1);
    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(process.env.AEGIS_APP_ACCESS_TOKEN).toBe('fresh-token');
    expect(process.env.AEGIS_API_KEY).toBe('legacy-api-key-token');
  });
  ...
  it('deduplicates concurrent token refresh requests', async () => {
    ...
    await Promise.all([
      aegisClient.query({ record: { id: true } }),
      aegisClient.query({ record: { id: true } }),
    ]);
  ...
  it('retries uploadFile once after 401 when refresh callback is available', async () => {
    ...
    const uploadResult = await aegisClient.uploadFile(
      Buffer.from('content'),
      'test.txt',
      'text/plain',
      'field-uuid',
  ...
  it('bubbles auth error when refresh callback throws', async () => {
    ...
    await expect(aegisClient.query({ record: { id: true } })).rejects.toThrow(
      'Unauthorized',
    );
    expect(requestAccessTokenRefresh).toHaveBeenCalledTimes(1);
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'Aegis client: token refresh failed',
      expect.any(Error),
    );
