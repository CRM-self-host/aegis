...
export const buildClientWrapperSource = (
  ...
  source = source.replace("'__AEGIS_DEFAULT_URL__'", options.defaultUrl);

  source = source.replace(/AegisGeneratedClient/g, options.apiClientName);
  ...
  return `\n// ${options.apiClientName} (auto-injected by aegis-client-sdk)\n${source}`;
};
