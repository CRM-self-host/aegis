...
describe('Successful user and workspace creation', () => {
  ...
  it('should sign up a new user and create a new workspace successfully', async () => {
    ...
    const aegisStandardApp = findManyApplicationsData.find(
      (application) =>
        application.universalIdentifier ===
        AEGIS_STANDARD_APPLICATION.universalIdentifier,
    );

    jestExpectToBeDefined(aegisStandardApp);
    const {
      sourcePath: _sourcePath,
      sourceType: _sourceType,
      ...expectedStandardAegisApplication
    } = AEGIS_STANDARD_APPLICATION;

    expect(aegisStandardApp).toMatchObject(expectedStandardAegisApplication);
