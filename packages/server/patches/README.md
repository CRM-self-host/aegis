...
yarn patch-commit <temp-folder> > packages/aegis-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```

Reference the patch in `packages/aegis-server/package.json`:
...
yarn patch-commit <CLEAN> > packages/aegis-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```
