...
const bootstrap = async () => {
  ...
  const aegisConfigService = app.get(AegisConfigService);

  const trustProxyRaw = aegisConfigService.get('TRUST_PROXY');
  ...
  app.use(session(getSessionStorageOptions(aegisConfigService)));
  ...
  await app.listen(aegisConfigService.get('NODE_PORT'));
};
