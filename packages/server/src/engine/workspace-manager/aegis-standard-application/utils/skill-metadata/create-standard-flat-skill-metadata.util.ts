...
import { type AllStandardSkillName } from 'src/engine/workspace-manager/aegis-standard-application/types/all-standard-skill-name.type';
import {
  type CreateStandardSkillArgs,
  createStandardSkillFlatMetadata,
} from 'src/engine/workspace-manager/aegis-standard-application/utils/skill-metadata/create-standard-skill-flat-metadata.util';
...
## Calling Aegis Tools from Python (MCP Bridge)

A \`aegis\` helper is automatically available in your code. Use it to call any Aegis tool directly from Python:
...
# Create a record
result = aegis.call_tool('create_company_record', {
    'data': {'name': 'Acme Corp', 'domainName': {'primaryLinkUrl': 'acme.com'}}
})
print(f"Created company: {result['id']}")

# Update a record
aegis.call_tool('update_person_record', {
    'id': 'person-uuid',
    'data': {'jobTitle': 'CEO'}
})

# List available tools
tools = aegis.list_tools()
for tool in tools:
