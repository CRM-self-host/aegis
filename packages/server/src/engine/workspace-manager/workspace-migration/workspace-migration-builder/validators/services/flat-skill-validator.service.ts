...
import { belongsToAegisStandardApp } from 'src/engine/metadata-modules/utils/belongs-to-aegis-standard-app.util';
...
export class FlatSkillValidatorService {
  ...
  >): FailedFlatEntityValidation<'skill', 'delete'> {
    ...
    if (
      !isCallerAegisStandardApp(buildOptions) &&
      belongsToAegisStandardApp({
  ...
  >): FailedFlatEntityValidation<'skill', 'update'> {
    ...
    const isAegisStandardSkill = belongsToAegisStandardApp({
