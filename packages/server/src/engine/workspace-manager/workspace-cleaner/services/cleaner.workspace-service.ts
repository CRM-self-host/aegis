...
export class CleanerWorkspaceService {
  ...
  constructor(
    ...
    this.inactiveDaysBeforeSoftDelete = this.aegisConfigService.get(
      'WORKSPACE_INACTIVE_DAYS_BEFORE_SOFT_DELETION',
    );
    this.inactiveDaysBeforeDelete = this.aegisConfigService.get(
      'WORKSPACE_INACTIVE_DAYS_BEFORE_DELETION',
    );
    this.inactiveDaysBeforeWarn = this.aegisConfigService.get(
      'WORKSPACE_INACTIVE_DAYS_BEFORE_NOTIFICATION',
    );
    this.maxNumberOfWorkspacesDeletedPerExecution = this.aegisConfigService.get(
  ...
  async sendWarningEmail(
    ...
    this.emailService.send({
      ...
      from: `${this.aegisConfigService.get(
        'EMAIL_FROM_NAME',
      )} <${this.aegisConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      subject,
      html,
      text,
    });
