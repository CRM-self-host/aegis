...
import { type RelationConnectQueryConfig } from 'src/engine/aegis-orm/entity-manager/types/relation-connect-query-config.type';
import {
  type RelationConnectQueryFieldsByEntityIndex,
  type RelationDisconnectQueryFieldsByEntityIndex,
} from 'src/engine/aegis-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  AegisORMException,
  AegisORMExceptionCode,
} from 'src/engine/aegis-orm/exceptions/aegis-orm.exception';
import { formatConnectRecordNotFoundErrorMessage } from 'src/engine/aegis-orm/field-operations/relation-nested-queries/utils/formatConnectRecordNotFoundErrorMessage.util';
import { type WorkspaceSelectQueryBuilder } from 'src/engine/aegis-orm/repository/workspace-select-query-builder';
import { computeRelationConnectQueryConfigs } from 'src/engine/aegis-orm/utils/compute-relation-connect-query-configs.util';
import { createSqlWhereTupleInClause } from 'src/engine/aegis-orm/utils/create-sql-where-tuple-in-clause.utils';
import { extractNestedRelationFieldsByEntityIndex } from 'src/engine/aegis-orm/utils/extract-nested-relation-fields-by-entity-index.util';
import { getAssociatedRelationFieldName } from 'src/engine/aegis-orm/utils/get-associated-relation-field-name.util';
import { getObjectMetadataFromEntityTarget } from 'src/engine/aegis-orm/utils/get-object-metadata-from-entity-target.util';
import { getRecordToConnectFields } from 'src/engine/aegis-orm/utils/get-record-to-connect-fields.util';
        ...
        if (
          ...
          if (recordToConnect.length !== 1) {
            ...
            throw new AegisORMException(
              errorMessage,
              AegisORMExceptionCode.CONNECT_RECORD_NOT_FOUND,
              {
                userFriendlyMessage,
              },
            );
