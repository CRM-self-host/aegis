...
import { CONFIG_VARIABLES_GROUP_METADATA } from 'src/engine/core-modules/aegis-config/constants/config-variables-group-metadata';
...
export class AdminPanelConfigService {
  ...
  getConfigVariablesGrouped(): ConfigVariablesDTO {
    const rawEnvVars = this.aegisConfigService.getAll();
  ...
  getConfigVariable(key: string): ConfigVariableDTO {
    const variableWithMetadata =
      this.aegisConfigService.getVariableWithMetadata(
