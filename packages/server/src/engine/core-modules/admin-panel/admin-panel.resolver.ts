...
import { ConfigVariableGraphqlApiExceptionFilter } from 'src/engine/core-modules/aegis-config/filters/config-variable-graphql-api-exception.filter';
...
export class AdminPanelResolver {
  ...
  async getAdminAiModels(): Promise<AdminAiModelsDTO> {
    ...
    const prefs = this.aegisConfigService.get('AI_MODEL_PREFERENCES');
  ...
  async getDatabaseConfigVariable(
    ...
    this.aegisConfigService.validateConfigVariableExists(key as string);
  ...
  async createDatabaseConfigVariable(
    ...
    await this.aegisConfigService.set(key, value);
  ...
  async updateDatabaseConfigVariable(
    ...
    await this.aegisConfigService.update(key, value);
  ...
  async deleteDatabaseConfigVariable(
    ...
    await this.aegisConfigService.delete(key);
  ...
  async addAiProvider(
    ...
    const customProviders = {
      ...this.aegisConfigService.get('AI_PROVIDERS'),
    };

    customProviders[providerName] = providerConfig;
    await this.aegisConfigService.set('AI_PROVIDERS', customProviders);
  ...
  async getAdminAiUsageByWorkspace(
    ...
    const useDollarMode = !this.aegisConfigService.get('IS_BILLING_ENABLED');
