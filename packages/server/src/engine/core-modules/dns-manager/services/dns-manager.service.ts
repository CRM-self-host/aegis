...
export class DnsManagerService {
  ...
  constructor(
    ...
    if (this.aegisConfigService.get('CLOUDFLARE_API_KEY')) {
      this.cloudflareClient = new Cloudflare({
        apiToken: this.aegisConfigService.get('CLOUDFLARE_API_KEY'),
      });
    }
  }
      ...
      records: [
        ...
        },
        {
          ...
          value:
            ...
            `${hostname}.${this.aegisConfigService.get('CLOUDFLARE_DCV_DELEGATION_ID')}.dcv.cloudflare.com`,
        },
      ],
    };
  }
  ...
  private getZoneId(options?: DnsManagerOptions): string {
    return options?.isPublicDomain
      ? this.aegisConfigService.get('CLOUDFLARE_PUBLIC_DOMAIN_ZONE_ID')
