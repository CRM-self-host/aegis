...
export class EnterprisePlanService implements OnModuleInit {
  ...
  private refreshKeyPayload(): void {
    const enterpriseKey = this.aegisConfigService.get('ENTERPRISE_KEY');
  ...
  private async loadValidityToken(): Promise<void> {
    try {
      ...
      const tokenValue =
        dbToken?.value ??
        this.aegisConfigService.get('ENTERPRISE_VALIDITY_TOKEN');
  ...
  private checkLegacyKey(): boolean {
    // temporary
    return isDefined(this.aegisConfigService.get('ENTERPRISE_KEY'));
  }
  ...
  async setEnterpriseKey(enterpriseKey: string): Promise<void> {
    try {
      await this.aegisConfigService.set('ENTERPRISE_KEY', enterpriseKey);
    } catch (error) {
      if (
  ...
  async refreshValidityToken(): Promise<boolean> {
    ...
    const apiUrl = this.aegisConfigService.get('ENTERPRISE_API_URL');
