...
import { AEGIS_ICONS_BASE_URL } from 'shared/branding-constants/AegisIconsBaseUrl';
...
@Injectable()
// oxlint-disable-next-line aegis/inject-workspace-repository
export class SignInUpService {
      ...
      [
        {
          ...
          serverId: this.aegisConfigService.get('SERVER_ID'),
        },
      ],
      undefined,
    );
  ...
  async signUpOnNewWorkspace(
    ...
    try {
      ...
      if (isWorkEmailFound) {
        const logoUrl = `${AEGIS_ICONS_BASE_URL}/${getDomainNameByEmail(email)}`;
        const logoFile =
