  ...
  protected createDriver(): EmailingDomainDriverInterface {
    ...
    switch (driver) {
      case EmailingDomainDriver.AWS_SES: {
        const region = this.aegisConfigService.get('AWS_SES_REGION');
        const accountId = this.aegisConfigService.get('AWS_SES_ACCOUNT_ID');
        ...
        const sessionToken = this.aegisConfigService.get(
