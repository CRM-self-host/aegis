  ...
  implements GqlOptionsFactory<YogaDriverConfig<'express'>>
{
  ...
  createGqlOptions(): YogaDriverConfig {
    const isDebugMode =
      this.aegisConfigService.get('NODE_ENV') === NodeEnvironment.DEVELOPMENT;
    const plugins = [
      useDirectExecution({
      ...
      useGraphQLErrorHandlerHook({
        ...
        aegisConfigService: this.aegisConfigService,
      }),
      useDisableIntrospectionAndSuggestionsForUnauthenticatedUsers(
        this.aegisConfigService.get('NODE_ENV') === NodeEnvironment.PRODUCTION,
      ),
      useValidateGraphqlQueryComplexity({
        maximumAllowedFields: this.aegisConfigService.get('GRAPHQL_MAX_FIELDS'),
        maximumAllowedRootResolvers: this.aegisConfigService.get(
