...
export class MigrateMessagingInfrastructureToMetadataCommand extends ActiveOrSuspendedWorkspaceCommandRunner {
  constructor(
    private readonly aegisORMGlobalManager: GlobalWorkspaceOrmManager,
  ...
  override async runOnWorkspace({
    ...
    const connectedAccountWorkspaceRepository =
      await this.aegisORMGlobalManager.getRepository<ConnectedAccountWorkspaceEntity>(
        workspaceId,
        'connectedAccount',
      );

    const messageChannelWorkspaceRepository =
      await this.aegisORMGlobalManager.getRepository<MessageChannelEntity>(
        workspaceId,
        'messageChannel',
      );

    const calendarChannelWorkspaceRepository =
      await this.aegisORMGlobalManager.getRepository<CalendarChannelEntity>(
        workspaceId,
        'calendarChannel',
      );

    const messageFolderWorkspaceRepository =
      await this.aegisORMGlobalManager.getRepository<MessageFolderWorkspaceEntity>(
        workspaceId,
        'messageFolder',
      );
  ...
  private async buildWorkspaceMemberIdToUserWorkspaceIdMap(
    ...
    const workspaceMemberRepository =
      await this.aegisORMGlobalManager.getRepository<WorkspaceMemberWorkspaceEntity>(
        workspaceId,
        'workspaceMember',
