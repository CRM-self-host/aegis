import { getLatestReleasePreview } from '@/lib/releases/get-latest-release-preview';
import type {
  MenuDataType,
  MenuNavChildPreview,
  MenuNavItemType,
  MenuSocialLinkType,
} from '@/sections/Menu/types';

const FALLBACK_RELEASES_PREVIEW: MenuNavChildPreview = {
  image: '/images/releases/1.23/1.23.0-easier-layouts.webp',
  imageAlt: 'Aegis latest release',
  imageScale: 1.04,
  title: 'See the latest release',
  description:
    'Track every release with changelogs, highlights and demos of the newest features.',
};

function buildNavItems(): MenuNavItemType[] {
  const releasesPreview =
    getLatestReleasePreview() ?? FALLBACK_RELEASES_PREVIEW;

  return [
    { label: 'Why', href: '/why-aegis' },
    {
      label: 'Resources',
      children: [
        {
          label: 'User Guide',
          description: 'Learn how to use Aegis',
          href: 'https://docs.aegis.com/user-guide/introduction',
          external: true,
          icon: 'book',
          preview: {
            image: '/images/product/feature/contacts.webp',
            imageAlt: 'Aegis companies list',
            title: 'Master every corner of Aegis',
            description:
              'Step-by-step guides and playbooks to help your team get the most out of their workspace.',
          },
        },
        {
          label: 'Developers',
          description: 'Create apps on Aegis',
          href: 'https://docs.aegis.com/developers/introduction',
          external: true,
          icon: 'code',
          preview: {
            image: '/images/shared/menu/developers-preview.png',
            imageAlt: 'Blue developer illustration with branching arrows',
            imagePosition: 'center',
            imageScale: 1.6,
            title: 'Build on an open platform',
            description:
              'APIs, SDKs and webhooks to extend Aegis and ship apps on top of your CRM data.',
          },
        },
        {
          label: 'Partners',
          description: 'Find a Aegis partner',
          href: '/partners',
          icon: 'users',
          preview: {
            image: '/images/partner/hero/hero.webp',
            imageAlt: 'Aegis partner ecosystem',
            imagePosition: 'center',
            title: 'Team up with a Aegis expert',
            description:
              'Meet the certified agencies and consultants implementing Aegis for teams worldwide.',
          },
        },
        {
          label: 'Releases',
          description: "Discover what's new",
          href: '/releases',
          icon: 'tag',
          preview: releasesPreview,
        },
      ],
    },
    { label: 'Customers', href: '/customers' },
    { label: 'Pricing', href: '/pricing' },
  ];
}

const SOCIAL_LINKS: MenuSocialLinkType[] = [
  {
    ariaLabel: 'GitHub (opens in new tab)',
    href: 'https://github.com/u84u/aegis',
    icon: 'github',
    showInDesktop: true,
    showInDrawer: true,
  },
  {
    ariaLabel: 'Discord (opens in new tab)',
    className: 'discord-link',
    href: 'https://discord.gg/cx5n4Jzs57',
    icon: 'discord',
    showInDesktop: true,
    showInDrawer: true,
  },
  {
    ariaLabel: 'LinkedIn (opens in new tab)',
    href: 'https://www.linkedin.com/company/aegis',
    icon: 'linkedin',
    showInDesktop: false,
    showInDrawer: true,
  },
  {
    ariaLabel: 'X (opens in new tab)',
    href: 'https://x.com/u84u',
    icon: 'x',
    showInDesktop: false,
    showInDrawer: true,
  },
];

export const MENU_DATA: MenuDataType = {
  get navItems() {
    return buildNavItems();
  },
  socialLinks: SOCIAL_LINKS,
};
