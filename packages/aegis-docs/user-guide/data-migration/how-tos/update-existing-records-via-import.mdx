...
When you import a file containing a **unique identifier** that matches an existing record, Aegis updates that record instead of creating a duplicate.

| If unique identifier... | Aegis will... |
|-------------------------|----------------|
...
4. **Don't change unique identifier values** — or Aegis will create new records
...
If you change `john@acme.com` to `john.smith@acme.com`, Aegis will create a new record instead of updating the existing one.
</Warning>
...
Aegis matches records by the unique identifier and updates them with new values.
...
3. Aegis will match on that field
...
| **Case mismatch** | File has `John@acme.com` but Aegis has `john@acme.com` | Creates new record | Export from Aegis to get exact values |
...
<AccordionGroup>
  ...
  <Accordion title="How do I know which records will be updated vs. created?">
    During the import review step, Aegis shows you how many records will be updated vs. created based on unique identifier matches.
  </Accordion>
