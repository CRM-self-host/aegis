...
description: Crie componentes React que renderizam dentro da UI do Aegis com isolamento em sandbox.
icon: window-maximize
---

Componentes de front-end são componentes React que renderizam diretamente dentro da UI do Aegis. Eles são executados em um Web Worker isolado usando Remote DOM — seu código é sandboxed, mas renderiza nativamente na página, não em um iframe.

## Onde os componentes de front-end podem ser usados

Os componentes de front-end podem ser renderizados em dois locais dentro do Aegis:
...
Após sincronizar com `yarn aegis dev` (ou executando uma única vez o `yarn aegis dev --once`), a ação rápida aparece no canto superior direito da página:
...
Como o componente retorna `null`, o Aegis ignora renderizar um contêiner para ele — nenhum espaço vazio aparece no layout. O componente ainda tem acesso a todos os hooks e à API de comunicação do host.

## Componentes Command do SDK

O pacote `aegis-sdk` fornece quatro componentes auxiliares Command projetados para componentes de front-end headless. Cada componente executa uma ação ao montar, trata erros exibindo uma notificação de snackbar e desmonta automaticamente o componente de front-end ao concluir.

Importe-os de `aegis-sdk/command`:
...
Componentes de front-end podem acionar navegação, modais e notificações usando funções de `aegis-sdk`:
...
O campo `conditionalAvailabilityExpression` permite controlar quando um comando é visível com base no contexto da página atual. Importe variáveis tipadas e operadores de `aegis-sdk` para construir expressões:
...
* **Componentes de UI do Aegis** — importe de `aegis-sdk/ui` (Button, Tag, Status, Chip, Avatar e mais)
