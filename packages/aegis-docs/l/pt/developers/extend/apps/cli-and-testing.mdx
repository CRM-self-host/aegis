...
* **Incluídos nas compilações** — `yarn aegis build` agrupa todos os recursos públicos na saída de distribuição.

### Acessando recursos públicos com `getPublicAssetUrl`

Use o helper `getPublicAssetUrl` de `aegis-sdk` para obter a URL completa de um arquivo no seu diretório `public/`. Funciona tanto em **funções lógicas** quanto em **componentes de front-end**.
...
Ambos os ambientes têm `aegis-client-sdk/core` e `aegis-client-sdk/metadata` disponíveis como módulos pré-fornecidos — eles não são empacotados, mas resolvidos em tempo de execução pelo servidor.

## Testando seu aplicativo

O SDK fornece APIs programáticas que permitem compilar, implantar, instalar e desinstalar seu aplicativo a partir de código de teste. Em conjunto com [Vitest](https://vitest.dev/) e os clientes de API tipados, você pode escrever testes de integração que verificam que seu aplicativo funciona de ponta a ponta em um servidor Aegis real.
...
O subcaminho `aegis-sdk/cli` exporta funções que você pode chamar diretamente a partir do código de teste:
...
Certifique-se de que seu servidor Aegis local esteja em execução e, em seguida:
...
### Executando funções (`yarn aegis exec`)
...
### Visualizando logs de funções (`yarn aegis logs`)
...
<Note>
Isso é diferente de `yarn aegis server logs`, que mostra os logs do contêiner Docker. `yarn aegis logs` mostra os logs de execução de funções do seu aplicativo a partir do servidor Aegis.
</Note>

### Desinstalando um aplicativo (`yarn aegis uninstall`)
...
Um **remoto** é um servidor Aegis ao qual seu aplicativo se conecta. Durante a configuração, o gerador de scaffold cria um para você automaticamente. Você pode adicionar mais remotos ou alternar entre eles a qualquer momento.
...
Suas credenciais são armazenadas em `~/.aegis/config.json`.
...
2. Inicializa um servidor Aegis temporário usando a ação `u84u/aegis/.github/actions/spawn-aegis-docker-image`
3. Instala as dependências com `yarn install --immutable`
4. Executa `yarn test` com `AEGIS_API_URL` e `AEGIS_API_KEY` injetados a partir das saídas da ação
...
Você não precisa configurar nenhum segredo — a ação `spawn-aegis-docker-image` inicia um servidor Aegis efêmero diretamente no runner e fornece os detalhes de conexão. O segredo `GITHUB_TOKEN` é fornecido automaticamente pelo GitHub.

Para fixar uma versão específica do Aegis em vez de `latest`, altere a variável de ambiente `AEGIS_VERSION` no topo do workflow.
