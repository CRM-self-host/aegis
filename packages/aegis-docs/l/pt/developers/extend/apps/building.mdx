---
title: Arquitetura
description: Como as aplicações Aegis funcionam — sandboxing, ciclo de vida e os blocos de construção.
icon: sitemap
---

As aplicações Aegis são pacotes TypeScript que estendem seu espaço de trabalho com objetos personalizados, lógica, componentes de UI e recursos de IA. Elas são executadas na plataforma Aegis com sandboxing completo e controles de permissão.

## Como as aplicações funcionam

Uma aplicação é uma coleção de **entidades** declaradas usando funções `defineEntity()` do pacote `aegis-sdk`. O SDK detecta essas declarações via análise de AST no momento da compilação e produz um **manifesto** — uma descrição completa do que seu aplicativo adiciona a um espaço de trabalho.
...
| **Componente de front-end**   | UI React em sandbox na página do Aegis               | [Componentes de front-end](/l/pt/developers/extend/apps/front-components) |
...
* **Componentes de front-end** executam em Web Workers usando Remote DOM — isolados da página principal, mas renderizando elementos DOM nativos (não iframes). Eles se comunicam com o Aegis por meio de uma API de host com passagem de mensagens.
* **Permissões** são aplicadas no nível da API. O token de tempo de execução (`AEGIS_APP_ACCESS_TOKEN`) é derivado do papel definido em `defineApplication()`.
...
* **`yarn aegis dev`** — observa seus arquivos-fonte e sincroniza ao vivo as alterações com um servidor Aegis conectado. O cliente de API tipado é regenerado automaticamente quando o esquema muda.
* **`yarn aegis build`** — compila TypeScript, empacota funções de lógica e componentes de front-end com o esbuild e produz um manifesto.
...
<CardGroup cols={2}>
  ...
  <Card title="Componentes de front-end" icon="window-maximize" href="/l/pt/developers/extend/apps/front-components">
    Componentes React em sandbox dentro da UI do Aegis.
  </Card>
