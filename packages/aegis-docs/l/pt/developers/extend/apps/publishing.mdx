...
title: Publicação
icon: carregar
description: Distribua seu aplicativo Aegis no Marketplace ou implante-o internamente.
---
...
* **Implantar um tarball** — envie seu aplicativo diretamente para um servidor Aegis específico para uso interno ou privado.
* **Publicar no npm** — liste seu aplicativo no Marketplace da Aegis para que qualquer espaço de trabalho possa descobrir e instalar.
...
Isso compila seu código-fonte em TypeScript, transpila funções de lógica e componentes de front-end e grava tudo em `.aegis/output/`. Adicione `--tarball` para também gerar um pacote `.tgz` para distribuição manual ou para o comando de deploy.

## Implantando em um servidor (tarball)

Para aplicativos que você não quer disponibilizar publicamente — ferramentas proprietárias, integrações apenas para empresas ou builds experimentais — você pode implantar um tarball diretamente em um servidor Aegis.

### Pré-requisitos

Antes de implantar, você precisa de um remote configurado apontando para o servidor de destino. Os remotes armazenam a URL do servidor e as credenciais de autenticação localmente em `~/.aegis/config.json`.
...
2. Execute `yarn aegis deploy` (ou `yarn aegis deploy --remote production`)
...
Os apps gerados com `create-aegis-app` já vêm com dois fluxos de trabalho do GitHub Actions prontos, em `.github/workflows/`. Eles estão prontos para executar assim que você fizer push do repositório para o GitHub — nenhuma configuração extra é necessária para CI, e CD requer apenas um único segredo.
...
2. Inicia uma instância de teste do Aegis isolada usando a ação composta `u84u/aegis/.github/actions/spawn-aegis-app-dev-test@main` (o equivalente em CI de `yarn aegis server start --test`).
3. Habilita o Corepack, configura o Node.js a partir do seu `.nvmrc` e instala as dependências com `yarn install --immutable`.
4. Executa `yarn test`, passando `AEGIS_API_URL` e `AEGIS_API_KEY` da instância iniciada para que seus testes possam se comunicar com um servidor real.

**Opções de configuração:**

* `AEGIS_VERSION` (env, padrão `latest`) — fixe a versão do servidor Aegis usada no CI editando isto em `ci.yml`.
...
Faz o deploy do seu app para um servidor Aegis configurado a cada push para `main` e, opcionalmente, a partir de um pull request quando o rótulo `deploy` é aplicado.

**O que faz:**

1. Faz checkout do head do PR (para PRs rotulados) ou do commit enviado.
2. Executa `u84u/aegis/.github/actions/deploy-aegis-app@main` — o equivalente em CI de `yarn aegis deploy`.
3. Executa `u84u/aegis/.github/actions/install-aegis-app@main` para que a versão recém-implantada seja instalada no workspace de destino.

**Configuração obrigatória:**

| Configuração            | Onde                                                                 | Finalidade                                                                                                       |
| ----------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `AEGIS_DEPLOY_URL`     | `env` em `cd.yml` (padrão `http://localhost:3000`)                   | O servidor Aegis para o qual fazer o deploy. Altere isto para a URL real do seu servidor antes do primeiro uso. |
| `AEGIS_DEPLOY_API_KEY` | Repositório do GitHub **Settings → Secrets and variables → Actions** | Chave de API com permissão de deploy no servidor de destino.                                                     |

<Note>
O `AEGIS_DEPLOY_URL` padrão de `http://localhost:3000` é um placeholder — ele não alcançará nada a partir de um runner hospedado pelo GitHub. Atualize-o para a URL pública do seu servidor (ou use um runner self-hosted com acesso à rede) antes de habilitar o CD.
</Note>
...
Ambos os fluxos de trabalho fazem referência a ações reutilizáveis em `@main`, portanto as atualizações de ações no repositório `u84u/aegis` são aplicadas automaticamente. Se você quiser builds determinísticos, substitua `@main` por um SHA de commit ou uma tag de release em cada linha `uses:`.

## Publicação no npm

Publicar no npm torna seu aplicativo descobrível no Marketplace da Aegis. Qualquer espaço de trabalho da Aegis pode navegar, instalar e atualizar aplicativos do Marketplace diretamente pela UI.

### Requisitos

* Uma conta no [npm](https://www.npmjs.com)
* A palavra-chave `aegis-app` no array `keywords` do seu `package.json` (adicione-a manualmente — não é incluída por padrão no template `create-aegis-app`)
...
O servidor Aegis sincroniza seu catálogo do marketplace a partir do registro do npm **a cada hora**.
...
Para outros sistemas de CI (GitLab CI, CircleCI etc.), aplicam-se os mesmos três comandos: `yarn install`, `yarn aegis build` e, em seguida, `npm publish` a partir de `.aegis/output`.
...
Vá para a página **Configurações > Aplicações** no Aegis, onde é possível navegar e instalar tanto apps do marketplace quanto apps implantados por tarball.
...
Para instalar uma versão mais recente, implante ou publique-a primeiro e, em seguida, execute novamente `yarn aegis install`.
</Note>
