...
A Aegis implementa OAuth 2.0 com código de autorização + PKCE para aplicações voltadas ao utilizador e credenciais de cliente para acesso servidor a servidor. Os clientes são registados dinamicamente via [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591) — sem configuração manual num painel.
...
| Aplicação Aegis com extensões de UI               | [Aplicações](/l/pt/developers/extend/apps/getting-started) (OAuth é tratado automaticamente) |

## Registar um cliente

A Aegis suporta **registo dinâmico de clientes** conforme [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591). Sem necessidade de configuração manual — registe programaticamente:
...
Use este fluxo quando a sua aplicação atuar em nome de um utilizador da Aegis.
...
Após a autorização, a Aegis redireciona de volta para o seu `redirect_uri`:
...
A Aegis publica a sua configuração OAuth num endpoint padrão de descoberta:
