---
title: Aplicativos Aegis
description: Crie e gerencie personalizações do Aegis como código.
---
...
Os aplicativos permitem que você estenda o Aegis com objetos, campos, funções de lógica, componentes de interface, habilidades de IA e mais — tudo gerenciado como código. Em vez de configurar tudo pela UI, você define seu modelo de dados e a lógica em TypeScript e implanta em um ou mais workspaces.
...
* **Componentes de interface** — componentes React que são exibidos na UI do Aegis (páginas de registro, menu de comandos, painéis laterais)
* **Habilidades e agentes de IA** — estenda a IA do Aegis com recursos personalizados
...
Isso cria a estrutura de um novo aplicativo, inicia opcionalmente um servidor Aegis local e começa a monitorar seus arquivos por alterações. Veja o guia [Primeiros passos](/l/pt/developers/extend/apps/getting-started) para o passo a passo completo.
...
| `defineFrontComponent()`           | Componentes React na UI do Aegis                        |
...
1. **`yarn aegis dev`** — observa os arquivos de origem, recompila quando há alterações, sincroniza com o servidor e gera clientes de API tipados
2. **`yarn aegis build`** — produz um build distribuível
3. **`yarn aegis deploy`** — implanta em um servidor Aegis remoto
4. **`yarn aegis add`** — cria a estrutura de uma nova entidade de forma interativa

### Referência da CLI

```bash filename="Terminal"
yarn aegis help          # List all commands
yarn aegis server start  # Start local dev server
yarn aegis remote add    # Connect to a Aegis server
yarn aegis exec -n fn    # Execute a logic function
yarn aegis logs -n fn    # Stream function logs
```
