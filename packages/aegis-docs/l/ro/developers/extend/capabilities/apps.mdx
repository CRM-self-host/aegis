---
title: Aplicații Aegis
description: Construiți și gestionați personalizările Aegis sub formă de cod.
---
...
* **Componente front-end** — componente React care se afișează în interfața Aegis (pagini de înregistrări, meniul de comenzi, panouri laterale)
* **Abilități și agenți IA** — extindeți IA din Aegis cu capabilități personalizate
...
Aceasta creează scheletul unei aplicații noi, pornește opțional un server Aegis local și începe să monitorizeze fișierele pentru modificări. Consultați ghidul [Începeți](/l/ro/developers/extend/apps/getting-started) pentru prezentarea completă.
...
| `defineFrontComponent()`           | Componente React în interfața Aegis                     |
...
1. **`yarn aegis dev`** — monitorizează fișierele sursă, reconstruiește la modificări, sincronizează cu serverul, generează clienți API tipizați
2. **`yarn aegis build`** — produce o versiune distribuibilă
3. **`yarn aegis deploy`** — implementează pe un server Aegis la distanță
4. **`yarn aegis add`** — generează interactiv o entitate nouă

### Referință CLI

```bash filename="Terminal"
yarn aegis help          # Listează toate comenzile
yarn aegis server start  # Pornește serverul local de dezvoltare
yarn aegis remote add    # Conectează-te la un server Aegis
yarn aegis exec -n fn    # Execută o funcție logică
yarn aegis logs -n fn    # Transmite în flux jurnalele funcției
```
