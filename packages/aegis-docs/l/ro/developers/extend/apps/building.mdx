...
description: Definiți obiecte, funcții logice, componente front-end și multe altele cu Aegis SDK.
---
...
Pachetul `aegis-sdk` oferă blocuri de construcție tipizate pentru a crea aplicația. Această pagină acoperă toate tipurile de entități și clienții API disponibili în SDK.
...
import { defineApplication } from 'aegis-sdk';
...
* Tokenul de runtime injectat ca `AEGIS_APP_ACCESS_TOKEN` este derivat din acest rol.
...
import { defineRole, PermissionFlag } from 'aegis-sdk';
...
* Vedeți un exemplu funcțional: [`hello-world/src/roles/function-role.ts`](https://github.com/u84u/aegis/blob/main/packages/aegis-apps/hello-world/src/roles/function-role.ts).
...
import { defineObject, FieldType } from 'aegis-sdk';
...
* Puteți genera obiecte noi folosind `yarn aegis add`, care vă ghidează prin denumire, câmpuri și relații.

<Note>
**Câmpurile de bază sunt create automat.** Când definiți un obiect personalizat, Aegis adaugă automat câmpuri standard
...
Utilizați `defineField()` pentru a adăuga câmpuri la obiecte pe care nu le dețineți — cum ar fi obiectele standard Aegis (Person, Company etc.). sau obiecte din alte aplicații. Spre deosebire de câmpurile inline din `defineObject()`, câmpurile independente necesită un `objectUniversalIdentifier` pentru a specifica obiectul pe care îl extind:

```ts src/fields/company-loyalty-tier.field.ts
import { defineField, FieldType } from 'aegis-sdk';
...
Puncte cheie:
* `objectUniversalIdentifier` identifică obiectul țintă. Pentru obiectele standard, utilizați `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` exportați din `aegis-sdk`.
...
Relațiile conectează obiectele între ele. În Aegis, relațiile sunt întotdeauna bidirecționale — definiți ambele părți, iar fiecare parte o referențiază pe cealaltă.
...
import { defineField, FieldType, RelationType } from 'aegis-sdk';
...
import { defineField, FieldType, RelationType, OnDeleteAction } from 'aegis-sdk';
import { POST_CARD_UNIVERSAL_IDENTIFIER } from '../objects/post-card.object';
import { POST_CARD_RECIPIENT_UNIVERSAL_IDENTIFIER } from '../objects/post-card-recipient.object';
...
Pentru a crea o relație cu un obiect Aegis încorporat (Person, Company etc.), utilizați `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
import { defineLogicFunction } from 'aegis-sdk';
import type { DatabaseEventPayload, ObjectRecordCreateEvent, CronPayload, RoutePayload } from 'aegis-sdk';
import { CoreApiClient, type Person } from 'aegis-client-sdk/core';
...
> de ex. `path: '/post-card/create'` este apelabil la `https://your-aegis-server.com/s/post-card/create`
...
yarn aegis exec -n create-new-post-card -p '{"key": "value"}'
```

```bash filename="Terminal"
yarn aegis exec -y e56d363b-0bdc-4d8a-a393-6f0d1c75bdcf
```
...
Importați tipul `RoutePayload` din `aegis-sdk`:

```ts
import { defineLogicFunction, type RoutePayload } from 'aegis-sdk';
...
Funcțiile logice pot fi expuse ca **instrumente** pentru agenți de IA și fluxuri de lucru. Când este marcată ca instrument, o funcție poate fi descoperită de funcționalitățile de IA ale Aegis și poate fi utilizată în automatizări ale fluxurilor de lucru.
...
import { definePreInstallLogicFunction, type InstallLogicFunctionPayload } from 'aegis-sdk';
...
import { definePostInstallLogicFunction, type InstallLogicFunctionPayload } from 'aegis-sdk';
...
Componentele front-end sunt componente React care se afișează direct în interfața Aegis. Rulează într-un **Web Worker** izolat folosind Remote DOM — codul este izolat (sandboxed), dar se redă nativ în pagină, nu într-un iframe.

#### Unde pot fi utilizate componentele frontale

Componentele frontale pot fi afișate în două locații în cadrul Aegis:
...
import { defineFrontComponent } from 'aegis-sdk';

const HelloWorld = () => {
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      <h1>Hello from my app!</h1>
      <p>This component renders inside Aegis.</p>
    </div>
  );
};
...
După sincronizarea cu `yarn aegis dev` (sau prin rularea o singură dată a comenzii `yarn aegis dev --once`), acțiunea rapidă apare în colțul din dreapta sus al paginii:
...
import { defineFrontComponent, useRecordId, enqueueSnackbar } from 'aegis-sdk';
...
Deoarece componenta returnează `null`, Aegis omite redarea unui container pentru ea — nu apare spațiu gol în layout. Componenta are în continuare acces la toate hook-urile și la API-ul de comunicare cu gazda.

#### Componentele Command din SDK

Pachetul `aegis-sdk` oferă patru componente ajutătoare Command, concepute pentru componente front-end headless. Fiecare componentă execută o acțiune la montare, gestionează erorile afișând o notificare snackbar și demontează automat componenta de interfață la final.

Importă-le din `aegis-sdk/command`:
...
import { defineFrontComponent } from 'aegis-sdk';
import { Command } from 'aegis-sdk/command';
...
import { defineFrontComponent } from 'aegis-sdk';
import { CommandModal } from 'aegis-sdk/command';
...
Componentele front-end pot declanșa navigare, ferestre modale și notificări folosind funcții din `aegis-sdk`:
...
import { defineFrontComponent, useRecordId } from 'aegis-sdk';
import { enqueueSnackbar, closeSidePanel } from 'aegis-sdk';
import { CoreApiClient } from 'aegis-sdk/clients';
...
Câmpul `conditionalAvailabilityExpression` vă permite să controlați când este vizibilă o comandă în funcție de contextul paginii curente. Importați variabile tipizate și operatori din `aegis-sdk` pentru a construi expresii:
...
```tsx
import { defineFrontComponent, getPublicAssetUrl } from 'aegis-sdk';
...
* **Componente Aegis UI** — import din `aegis-sdk/ui` (Button, Tag, Status, Chip, Avatar și altele)
...
```tsx
import { defineFrontComponent } from 'aegis-sdk';
import { Button, Tag, Status } from 'aegis-sdk/ui';
...
import { defineSkill } from 'aegis-sdk';
...
import { defineAgent } from 'aegis-sdk';
...
import { defineView, ViewKey } from 'aegis-sdk';
...
import { defineNavigationMenuItem, NavigationMenuItemType } from 'aegis-sdk';
...
import { definePageLayout, PageLayoutTabLayoutMode } from 'aegis-sdk';
import { EXAMPLE_OBJECT_UNIVERSAL_IDENTIFIER } from '../objects/example-object';
...
* **Incluse în build-uri** — `yarn aegis build` împachetează toate resursele publice în outputul de distribuție.

### Accesarea resurselor publice cu `getPublicAssetUrl`

Utilizați helperul `getPublicAssetUrl` din `aegis-sdk` pentru a obține URL-ul complet al unui fișier din directorul `public/`. Funcționează atât în funcții logice, cât și în componente frontend.

**Într-o funcție logică:**

```ts src/logic-functions/send-invoice.ts
import { defineLogicFunction, getPublicAssetUrl } from 'aegis-sdk';
...
Ambele medii au `aegis-client-sdk/core` și `aegis-client-sdk/metadata` disponibile ca module pre-furnizate — acestea nu sunt incluse în bundle, ci sunt rezolvate la rulare de către server.

## Generarea scheletului entităților cu `yarn aegis add`
...
```bash filename="Terminal"
yarn aegis add object
yarn aegis add logicFunction
yarn aegis add frontComponent
```
...
| ---------------------------- | ------------------------------------ | ------------------------------------- |
| Obiect                       | `yarn aegis add object`             | `src/objects/<name>.ts`               |
| Câmp                         | `yarn aegis add field`              | `src/fields/<name>.ts`                |
| Funcție logică               | `yarn aegis add logicFunction`      | `src/logic-functions/<name>.ts`       |
| Componentă frontend          | `yarn aegis add frontComponent`     | `src/front-components/<name>.tsx`     |
| Rol                          | `yarn aegis add role`               | `src/roles/<name>.ts`                 |
| Abilitate                    | `yarn aegis add skill`              | `src/skills/<name>.ts`                |
| Agent                        | `yarn aegis add agent`              | `src/agents/<name>.ts`                |
| Vizualizare                  | `yarn aegis add view`               | `src/views/<name>.ts`                 |
| Element de meniu de navigare | `yarn aegis add navigationMenuItem` | `src/navigation-menu-items/<name>.ts` |
| Machetă de pagină            | `yarn aegis add pageLayout`         | `src/page-layouts/<name>.ts`          |

### Ce generează scaffolder-ul

Fiecare tip de entitate are propriul său șablon. De exemplu, `yarn aegis add object` solicită:
...
```bash filename="Terminal"
yarn aegis add logicFunction --path src/custom-folder
```

## Clienți API tipizați (aegis-client-sdk)

Pachetul `aegis-client-sdk` oferă doi clienți GraphQL tipați pentru a interacționa cu API-ul Aegis din funcțiile de logică și componentele Front.

| Client              | Importați                    | Endpoint                                                            | Generat?                     |
| ------------------- | ---------------------------- | ------------------------------------------------------------------- | ---------------------------- |
| `CoreApiClient`     | `aegis-client-sdk/core`     | `/graphql` — date ale spațiului de lucru (înregistrări, obiecte)    | Da, în timpul dev/build      |
| `MetadataApiClient` | `aegis-client-sdk/metadata` | `/metadata` — configurarea spațiului de lucru, încărcări de fișiere | Nu, este livrat preconstruit |

<AccordionGroup>
<Accordion title="CoreApiClient" description="Interogați și modificați datele spațiului de lucru (înregistrări, obiecte)">

`CoreApiClient` este clientul principal pentru interogarea și modificarea datelor din spațiul de lucru. Este generat din schema spațiului de lucru în timpul `yarn aegis dev` sau `yarn aegis build`, astfel încât este complet tipizat pentru a corespunde obiectelor și câmpurilor dvs.
...
<Note>
**CoreApiClient este generat în timpul dev/build.** Dacă îl utilizați fără a rula mai întâi `yarn aegis dev` sau `yarn aegis build`, va arunca o eroare. Generarea are loc automat — CLI inspectează schema GraphQL a spațiului dvs. de lucru și generează un client tipizat folosind `@genql/cli`.
</Note>
...
<Note>
  Când codul dvs. rulează pe Aegis (funcții de logică sau componente Front), platforma injectează acreditările ca variabile de mediu:

  * `AEGIS_API_URL` — URL-ul de bază al API-ului Aegis
  * `AEGIS_APP_ACCESS_TOKEN` — Cheie cu durată scurtă, limitată la rolul implicit de funcție al aplicației
...
SDK-ul oferă API-uri programatice care vă permit să construiți, să distribuiți, să instalați și să dezinstalați aplicația din codul de test. Combinat cu [Vitest](https://vitest.dev/) și clienții API tipizați, puteți scrie teste de integrare care verifică faptul că aplicația funcționează cap-coadă împotriva unui server Aegis real.
...
Subruta `aegis-sdk/cli` exportă funcții pe care le puteți apela direct din codul de test:
...
Asigurați-vă că serverul Aegis local rulează, apoi:
...
### Executarea funcțiilor (`yarn aegis exec`)
...
### Vizualizarea jurnalelor funcțiilor (`yarn aegis logs`)
...
<Note>
Acest lucru este diferit de `yarn aegis server logs`, care afișează jurnalele containerului Docker. `yarn aegis logs` afișează jurnalele de execuție ale funcțiilor aplicației de pe serverul Aegis.
</Note>

### Dezinstalarea unei aplicații (`yarn aegis uninstall`)
...
Un „remote” este un server Aegis la care se conectează aplicația. În timpul configurării, generatorul de schelet creează automat unul pentru dvs. Puteți adăuga mai multe remote-uri sau comuta între ele oricând.
...
Acreditările dvs. sunt stocate în `~/.aegis/config.json`.
...
1. Preia codul
2. Pornește un server Aegis temporar folosind acțiunea `u84u/aegis/.github/actions/spawn-aegis-docker-image`
3. Instalează dependențele cu `yarn install --immutable`
4. Rulează `yarn test` cu `AEGIS_API_URL` și `AEGIS_API_KEY` injectate din rezultatele acțiunii
...
Nu trebuie să configurați niciun secret — acțiunea `spawn-aegis-docker-image` pornește un server Aegis efemer direct în runner și oferă detaliile de conectare. Secretul `GITHUB_TOKEN` este furnizat automat de GitHub.

Pentru a fixa o versiune Aegis specifică în loc de `latest`, modificați variabila de mediu `AEGIS_VERSION` din partea de sus a workflow-ului.
