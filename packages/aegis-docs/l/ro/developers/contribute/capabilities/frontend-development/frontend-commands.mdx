...
```bash
npx nx run aegis-front:lint # utilizați --fix pentru a corecta erorile de lint
```
...
```bash
npx nx run aegis-front:test # rulați teste jest
npx nx run aegis-front:storybook:serve:dev # rulați storybook
npx nx run aegis-front:storybook:test # rulați testele # (are nevoie de yarn storybook:serve:dev pentru a rula)
npx nx run aegis-front:storybook:coverage # (are nevoie de yarn storybook:serve:dev pentru a rula)
```
