...
```bash
npx nx run aegis-front:lint # используйте --fix для исправления ошибок линтинга
```
...
```bash
npx nx run aegis-front:test # запуск тестов jest
npx nx run aegis-front:storybook:serve:dev # запуск storybook
npx nx run aegis-front:storybook:test # запуск тестов # (требуется yarn storybook:serve:dev для работы)
npx nx run aegis-front:storybook:coverage # (требуется yarn storybook:serve:dev для работы)
```
