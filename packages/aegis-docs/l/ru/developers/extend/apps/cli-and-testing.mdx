...
* **Включены в сборки** — `yarn aegis build` упаковывает все публичные ресурсы в выходной дистрибутив.

### Доступ к публичным ресурсам с помощью `getPublicAssetUrl`

Используйте хелпер `getPublicAssetUrl` из `aegis-sdk`, чтобы получить полный URL файла в каталоге `public/` вашего приложения. Он работает как в **логических функциях**, так и в **компонентах фронтенда**.
...
В обеих средах доступны как предварительно предоставленные модули `aegis-client-sdk/core` и `aegis-client-sdk/metadata` — они не включаются в бандл, а подставляются сервером во время выполнения.

## Тестирование вашего приложения

SDK предоставляет программные API, которые позволяют собирать, разворачивать, устанавливать и удалять ваше приложение из тестового кода. В сочетании с [Vitest](https://vitest.dev/) и типизированными клиентами API вы можете писать интеграционные тесты, которые проверяют, что ваше приложение работает сквозным образом на реальном сервере Aegis.
...
Подпуть `aegis-sdk/cli` экспортирует функции, которые можно вызывать напрямую из тестового кода:
...
Убедитесь, что ваш локальный сервер Aegis запущен, затем:
...
### Выполнение функций (`yarn aegis exec`)
...
### Просмотр логов функций (`yarn aegis logs`)
...
<Note>
Это отличается от `yarn aegis server logs`, который показывает логи контейнера Docker. `yarn aegis logs` показывает журналы выполнения функций вашего приложения с сервера Aegis.
</Note>

### Удаление приложения (`yarn aegis uninstall`)
...
**Remote** — это сервер Aegis, к которому подключается ваше приложение. Во время настройки скэффолдер автоматически создаст его для вас. Вы можете в любой момент добавлять новые удалённые серверы или переключаться между ними.
...
Ваши учётные данные хранятся в `~/.aegis/config.json`.
...
1. Извлекает ваш код
2. Поднимает временный сервер Aegis с помощью экшена `u84u/aegis/.github/actions/spawn-aegis-docker-image`
3. Устанавливает зависимости с помощью `yarn install --immutable`
4. Запускает `yarn test` с `AEGIS_API_URL` и `AEGIS_API_KEY`, переданными из выходных данных экшена
...
Вам не нужно настраивать секреты — экшен `spawn-aegis-docker-image` запускает эфемерный сервер Aegis прямо в раннере и выводит данные для подключения. Секрет `GITHUB_TOKEN` предоставляется GitHub автоматически.

Чтобы закрепить конкретную версию Aegis вместо `latest`, измените переменную окружения `AEGIS_VERSION` в начале рабочего процесса.
