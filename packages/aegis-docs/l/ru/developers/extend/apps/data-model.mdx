---
title: Модель данных
description: Определяйте объекты, поля, роли и метаданные приложения с помощью Aegis SDK.
icon: database
---

Пакет `aegis-sdk` предоставляет функции `defineEntity` для определения модели данных вашего приложения. Вы должны использовать `export default defineEntity({...})`, чтобы SDK обнаруживал ваши сущности. Эти функции проверяют вашу конфигурацию на этапе сборки и обеспечивают автодополнение в IDE и безопасность типов.
...
* Токен времени выполнения, подставляемый как `AEGIS_APP_ACCESS_TOKEN`, формируется из этой роли.
...
import { defineRole, PermissionFlag } from 'aegis-sdk/define';
...
* См. рабочий пример: [`hello-world/src/roles/function-role.ts`](https://github.com/u84u/aegis/blob/main/packages/aegis-apps/hello-world/src/roles/function-role.ts).
...
* Вы можете сгенерировать новые объекты с помощью `yarn aegis add`, который проведёт вас через выбор именования, полей и связей.

<Note>
**Базовые поля создаются автоматически.** Когда вы определяете пользовательский объект, Aegis автоматически добавляет стандартные поля,
...
Используйте `defineField()` для добавления полей к объектам, которые вам не принадлежат — например, к стандартным объектам Aegis (Person, Company и т. д.). или к объектам из других приложений. В отличие от встроенных полей в `defineObject()`, отдельные поля требуют `objectUniversalIdentifier`, чтобы указать, какой объект они расширяют:
...
Основные моменты:
* `objectUniversalIdentifier` определяет целевой объект. Для стандартных объектов используйте `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`, экспортируемые из `aegis-sdk`.
...
Отношения связывают объекты между собой. В Aegis отношения всегда двунаправленные — вы определяете обе стороны, и каждая сторона ссылается на другую.
...
Чтобы создать отношение со встроенным объектом Aegis (Person, Company и т. д.), используйте `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Создание заготовок сущностей с помощью `yarn aegis add`
...
| -------------------- | ------------------------------------ | ------------------------------------------------------- |
| Объект               | `yarn aegis add object`             | `src/objects/\<name>.ts`               |
| Поле                 | `yarn aegis add field`              | `src/fields/\<name>.ts`                |
| Логическая функция   | `yarn aegis add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Компонент фронтенда  | `yarn aegis add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Роль                 | `yarn aegis add role`               | `src/roles/\<name>.ts`                 |
| Навык                | `yarn aegis add skill`              | `src/skills/\<name>.ts`                |
| Агент                | `yarn aegis add agent`              | `src/agents/\<name>.ts`                |
| Представление        | `yarn aegis add view`               | `src/views/\<name>.ts`                 |
| Пункт меню навигации | `yarn aegis add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Макет страницы       | `yarn aegis add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Что генерирует скэффолдер

У каждого типа сущности есть свой шаблон. Например, `yarn aegis add object` запрашивает:
