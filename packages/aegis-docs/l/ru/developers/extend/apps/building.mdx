---
title: Архитектура
description: Как работают приложения Aegis — изоляция в песочнице, жизненный цикл и базовые элементы.
icon: sitemap
---

Приложения Aegis — это пакеты TypeScript, которые расширяют ваше рабочее пространство пользовательскими объектами, логикой, компонентами интерфейса и возможностями ИИ. Они работают на платформе Aegis с полной изоляцией в песочнице и контролем прав доступа.

## Как работают приложения

Приложение — это набор **сущностей**, объявленных с помощью функций `defineEntity()` из пакета `aegis-sdk`. SDK обнаруживает эти объявления посредством анализа AST на этапе сборки и формирует **манифест** — полное описание того, что ваше приложение добавляет в рабочее пространство.
...
| **Компонент фронтенда**  | Изолированный в песочнице интерфейс React на странице Aegis | [Компоненты фронтенда](/l/ru/developers/extend/apps/front-components) |
...
* **Компоненты фронтенда** запускаются в Web Workers с использованием Remote DOM — изолированы от основной страницы, но при этом рендерят нативные элементы DOM (не iframes). Они взаимодействуют с Aegis через хостовый API обмена сообщениями.
* **Права доступа** применяются на уровне API. Токен времени выполнения (`AEGIS_APP_ACCESS_TOKEN`) выводится из роли, определённой в `defineApplication()`.
...
* **`yarn aegis dev`** — следит за исходными файлами и синхронизирует изменения в реальном времени с подключённым сервером Aegis. Типизированный клиент API автоматически пересоздаётся при изменении схемы.
* **`yarn aegis build`** — компилирует TypeScript, упаковывает логические функции и фронтенд-компоненты с помощью esbuild и формирует манифест.
...
<CardGroup cols={2}>
  ...
  <Card title="Компоненты фронтенда" icon="window-maximize" href="/l/ru/developers/extend/apps/front-components">
    Изолированные в песочнице компоненты React внутри интерфейса Aegis.
  </Card>
