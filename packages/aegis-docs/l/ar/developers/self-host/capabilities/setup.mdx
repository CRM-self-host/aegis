...
<Warning>
**هل هي المرة الأولى التي تقوم فيها بالتثبيت؟** اتبع [دليل تثبيت Docker Compose](/l/ar/developers/self-host/capabilities/docker-compose) لتشغيل Aegis، ثم عد هنا للإعداد.
</Warning>

يوفر Aegis **وضعين للإعداد** ليلائم احتياجات النشر المختلفة:
...
1. الوصول إلى نسخة Aegis الخاصة بك (عادة `http://localhost:3000`)
...
[مرجع تقني كامل →](https://github.com/u84u/aegis/blob/main/packages/aegis-server/src/engine/core-modules/aegis-config/config-variables.ts)
</Warning>
...
افتراضيًا، تعمل Aegis في **وضع مساحة عمل واحدة** — وهو مثالي لمعظم عمليات النشر المستضافة ذاتيًا حيث تحتاج إلى مثيل واحد من نظام إدارة علاقات العملاء لمؤسستك.
...
* مساحة عمل واحدة لكل مثيل من Aegis
...
قم بتمكين وضع تعدد مساحات العمل لعمليات النشر المشابهة لـ SaaS حيث تحتاج عدة فرق مستقلة إلى مساحات عمل خاصة بها على نفس مثيل Aegis.
...
[انظر الشيفرة المصدرية ذات الصلة](https://github.com/u84u/aegis/blob/main/packages/aegis-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
يجب على المستخدمين الحصول على [ترخيص Microsoft 365](https://admin.microsoft.com/Adminportal/Home) ليتمكنوا من استخدام تقويم API ورسائل. لن يتمكنوا من مزامنة حسابهم في Aegis دون واحد منها.
</Warning>
...
[انظر الشيفرة المصدرية ذات الصلة](https://github.com/u84u/aegis/blob/main/packages/aegis-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
<Warning>
افتراضياً، تقوم Aegis بتخزين الملفات المرفوعة على نظام الملفات المحلي. بالنسبة لعمليات النشر في بيئة الإنتاج، استخدم S3 أو خدمة متوافقة مع S3 (MinIO، DigitalOcean Spaces، إلخ) لضمان بقاء الملفات عبر إعادة تشغيل الحاويات وإتاحة التوسّع عبر مثيلات خوادم متعددة.
</Warning>

عيّن `STORAGE_TYPE=S_3` وقم بتهيئة متغيرات `STORAGE_S3_*` من خلال لوحة الإدارة أو `.env`. راجع [مرجع config-variables.ts](https://github.com/u84u/aegis/blob/main/packages/aegis-server/src/engine/core-modules/aegis-config/config-variables.ts) للاطلاع على القائمة الكاملة لمتغيرات S3.

عند استخدام S3 مع الميزات المعتمدة على CORS (مثل تنزيل الملفات داخل المتصفح)، تأكد من أن حاويتك تسمح بأصل واجهة Aegis الأمامية ضمن تهيئة CORS الخاصة بها.

## الوظائف المنطقية ومفسر الشيفرة

تدعم Aegis الوظائف المنطقية لعمليات سير العمل ومفسر الشيفرة لتحليل بيانات الذكاء الاصطناعي. كلاهما يقوم بتشغيل الشيفرة المقدمة من المستخدم ويتطلب تهيئة صريحة لأغراض الأمان.
...
<Note>
عند استخدام `LOGIC_FUNCTION_TYPE=DISABLED` أو `CODE_INTERPRETER_TYPE=DISABLED`، سترجع أي محاولة للتنفيذ خطأً. يكون هذا مفيدًا إذا كنت ترغب في تشغيل Aegis من دون هذه الإمكانات.
</Note>
