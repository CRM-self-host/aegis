...
description: عرّف الكائنات والحقول والأدوار وبيانات تعريف التطبيق باستخدام SDK الخاصة بـ Aegis.
icon: database
---

توفر حزمة `aegis-sdk` دوالّ `defineEntity` لتعريف نموذج بيانات تطبيقك. يجب عليك استخدام `export default defineEntity({...})` لكي يكتشف SDK الكيانات الخاصة بك. تتحقق هذه الدوال من تكوينك وقت البناء وتوفّر إكمالًا تلقائيًا في بيئة التطوير وأمان الأنواع.
...
* رمز وقت التشغيل المحقون باسم `AEGIS_APP_ACCESS_TOKEN` مستمد من هذا الدور.
...
* اطّلع على مثال عملي: [`hello-world/src/roles/function-role.ts`](https://github.com/u84u/aegis/blob/main/packages/aegis-apps/hello-world/src/roles/function-role.ts).
...
* يمكنك إنشاء كائنات جديدة باستخدام `yarn aegis add`، والذي يرشدك خلال التسمية والحقول والعلاقات.

<Note>
**يتم إنشاء الحقول الأساسية تلقائيًا.** عند تعريف كائن مخصص، يضيف Aegis تلقائيًا حقولًا قياسية
...
استخدم `defineField()` لإضافة حقول إلى كائنات لا تملكها — مثل كائنات Aegis القياسية (Person, Company, etc.) أو كائنات من تطبيقات أخرى. على خلاف الحقول المضمّنة في `defineObject()`، تتطلّب الحقول المستقلة `objectUniversalIdentifier` لتحديد الكائن الذي تقوم بتوسيعه:
...
النقاط الرئيسية:
* `objectUniversalIdentifier` يحدّد الكائن الهدف. بالنسبة للكائنات القياسية، استخدم `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` المُصدَّر من `aegis-sdk`.
...
تربط العلاقات الكائنات معًا. في Aegis، تكون العلاقات دائمًا **ثنائية الاتجاه** — حيث تعرّف الجانبين، ويشير كل جانب إلى الآخر.
...
لإنشاء علاقة مع كائن Aegis مضمّن (Person, Company, etc.)، استخدم `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## توليد قوالب الكيانات باستخدام `yarn aegis add`
...
| ------------------ | ------------------------------------ | ------------------------------------------------------- |
| كائن               | `yarn aegis add object`             | `src/objects/\<name>.ts`               |
| الحقل              | `yarn aegis add field`              | `src/fields/\<name>.ts`                |
| دالة منطقية        | `yarn aegis add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| مكوّن أمامي        | `yarn aegis add frontComponent`     | `src/front-components/\<name>.tsx`     |
| دور                | `yarn aegis add role`               | `src/roles/\<name>.ts`                 |
| مهارة              | `yarn aegis add skill`              | `src/skills/\<name>.ts`                |
| وكيل               | `yarn aegis add agent`              | `src/agents/\<name>.ts`                |
| عرض                | `yarn aegis add view`               | `src/views/\<name>.ts`                 |
| عنصر قائمة التنقّل | `yarn aegis add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| تخطيط الصفحة       | `yarn aegis add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### ما الذي تُنشئه أداة القوالب

لكل نوع كيان قالب خاص به. على سبيل المثال، يسأل `yarn aegis add object` عن:
