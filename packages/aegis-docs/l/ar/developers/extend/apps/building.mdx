---
title: الهيكلية
description: كيف تعمل تطبيقات Aegis — العزل، دورة الحياة، واللبنات الأساسية.
icon: sitemap
---

تطبيقات Aegis هي حزم TypeScript توسّع مساحة عملك بكائنات مخصّصة، ومنطق، ومكوّنات واجهة مستخدم (UI)، وقدرات ذكاء اصطناعي. تعمل على منصة Aegis مع عزل كامل وضوابط الأذونات.

## كيف تعمل التطبيقات

التطبيق عبارة عن مجموعة من **الكيانات** يتم إعلانها باستخدام دوال `defineEntity()` من حزمة `aegis-sdk`. يكتشف SDK هذه التصريحات عبر تحليل AST وقت البناء وينتج **ملف بيان** — وصفًا كاملًا لما يضيفه تطبيقك إلى مساحة العمل.
...
| **مكوّن أمامي**        | واجهة مستخدم React معزولة داخل صفحة Aegis.            | [المكوّنات الأمامية](/l/ar/developers/extend/apps/front-components) |
...
* **المكوّنات الأمامية** تعمل ضمن Web Workers باستخدام Remote DOM — معزولة عن الصفحة الرئيسية لكنها تعرض عناصر DOM الأصلية (وليس iframes). تتواصل مع Aegis عبر واجهة API للمضيف تعتمد تمرير الرسائل.
* **الأذونات** تُطبَّق على مستوى واجهة API. يُشتق رمز وقت التشغيل (`AEGIS_APP_ACCESS_TOKEN`) من الدور المعرَّف في `defineApplication()`.
...
* **`yarn aegis dev`** — يراقب ملفات المصدر لديك ويزامن التغييرات مباشرةً إلى خادم Aegis متصل. يُعاد توليد عميل API مضبوط الأنواع تلقائيًا عند تغيّر المخطط.
* **`yarn aegis build`** — يجمّع TypeScript، ويضمّن الدوال المنطقية والمكوّنات الأمامية باستخدام esbuild، وينتج ملف بيان.
...
<CardGroup cols={٢}>
  ...
  <Card title="المكوّنات الأمامية" icon="window-maximize" href="/l/ar/developers/extend/apps/front-components">
    مكوّنات React معزولة داخل واجهة مستخدم Aegis.
  </Card>
