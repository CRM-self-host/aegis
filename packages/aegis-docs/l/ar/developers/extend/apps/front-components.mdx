...
description: أنشئ مكونات React تُعرَض داخل واجهة مستخدم Aegis ضمن بيئة معزولة (sandbox).
icon: window-maximize
---

المكوّنات الأمامية هي مكوّنات React تُعرَض مباشرة داخل واجهة مستخدم Aegis. تعمل ضمن **Web Worker** معزول باستخدام Remote DOM — تكون شيفرتك في صندوق عزل لكنها تُعرَض أصيلًا داخل الصفحة، وليس ضمن iframe.

## أين يمكن استخدام مكوّنات الواجهة الأمامية

يمكن عرض مكوّنات الواجهة الأمامية في موقعين داخل Aegis:
...
بعد المزامنة باستخدام `yarn aegis dev` (أو تشغيل الأمر لمرة واحدة `yarn aegis dev --once`)، يظهر الإجراء السريع في الزاوية العلوية اليمنى من الصفحة:
...
نظرًا لأن المكوّن يُرجع `null`، فإن Aegis يتخطّى عرض حاوية له — ولن تظهر مساحة فارغة في التخطيط. لا يزال لدى المكوّن إمكانية الوصول إلى جميع الخطافات وواجهة برمجة الاتصال مع المضيف.

## مكوّنات Command في SDK

توفر حزمة `aegis-sdk` أربعة مكوّنات مساعدة من نوع Command مصممة للمكوّنات عديمة الرأس في الواجهة الأمامية. كل مكوّن ينفّذ إجراءً عند التركيب، ويتعامل مع الأخطاء بعرض إشعار Snackbar، ويزيل تركيب مكوّن الواجهة الأمامية تلقائيًا عند الانتهاء.

استوردها من `aegis-sdk/command`:
...
يمكن للمكوّنات الأمامية تشغيل التنقّل والنوافذ المنبثقة والإشعارات باستخدام دوال من `aegis-sdk`:
...
يتيح لك الحقل `conditionalAvailabilityExpression` التحكّم في وقت ظهور الأمر بناءً على سياق الصفحة الحالي. استورد متغيّرات ومشغّلات مضبوطة الأنواع من `aegis-sdk` لبناء التعابير:
...
* **مكوّنات Aegis لواجهة المستخدم** — استورد من `aegis-sdk/ui` (Button وTag وStatus وChip وAvatar وغيرها)
