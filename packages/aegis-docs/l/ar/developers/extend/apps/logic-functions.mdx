...
دوال المنطق هي دوال TypeScript على جانب الخادم تعمل على منصة Aegis. يمكن تشغيلها بواسطة طلبات HTTP أو جداول cron أو أحداث قاعدة البيانات — كما يمكن إتاحتها كأدوات لوكلاء الذكاء الاصطناعي.
...
> مثال: `path: '/post-card/create'` يمكن استدعاؤه عبر `https://your-aegis-server.com/s/post-card/create`
...
استورد نوع `RoutePayload` من `aegis-sdk`:
...
يمكن إتاحة الدوال المنطقية بوصفها **أدوات** لوكلاء الذكاء الاصطناعي وسير العمل. عند تمييز دالة كأداة، تصبح قابلة للاكتشاف بواسطة ميزات الذكاء الاصطناعي في Aegis ويمكن استخدامها في أتمتة سير العمل.
...
* متغيرات البيئة `APPLICATION_ID` و`APP_ACCESS_TOKEN` و`API_URL` متاحة داخل المعالج (كما في أي دالة منطق أخرى)، لذا يمكنك استدعاء واجهة Aegis API باستخدام رمز وصول للتطبيق مقيّد بنطاق تطبيقك.
...
* **لا يُنفَّذ في وضع التطوير**: عند تسجيل تطبيق محليًا (عبر `yarn aegis dev`)، يتجاوز الخادم تدفّق التثبيت بالكامل ويُزامن الملفات مباشرةً عبر مراقِب CLI — لذا لن يعمل ما بعد التثبيت في وضع التطوير مطلقًا، بغضّ النظر عن `shouldRunSynchronously`. استخدم `yarn aegis exec --postInstall` لتشغيله يدويًا على مساحة عمل قيد التشغيل.
...
* **لا يُنفَّذ في وضع التطوير**: كما في ما بعد التثبيت — يتم تجاوز تدفّق التثبيت بالكامل للتطبيقات المسجّلة محليًا، لذا لن يعمل ما قبل التثبيت مطلقًا عند `yarn aegis dev`. استخدم `yarn aegis exec --preInstall` لتشغيله يدويًا.
...
## عملاء واجهة برمجة تطبيقات مضبوطة الأنواع (`aegis-client-sdk`)

توفر حزمة `aegis-client-sdk` عميلين لـ GraphQL ذوي أنواع ثابتة للتفاعل مع واجهة Aegis البرمجية من وظائفك المنطقية ومكوّنات الواجهة الأمامية.

| العميل              | استيراد                      | نقطة النهاية                                        | مُولَّد؟                   |
| ------------------- | ---------------------------- | --------------------------------------------------- | -------------------------- |
| `CoreApiClient`     | `aegis-client-sdk/core`     | `/graphql` — بيانات مساحة العمل (السجلات، الكائنات) | نعم، في وقت التطوير/البناء |
| `MetadataApiClient` | `aegis-client-sdk/metadata` | `/metadata` — تكوين مساحة العمل، رفع الملفات        | لا، يأتي مُجهزًا مسبقًا    |

<AccordionGroup>
<Accordion title="CoreApiClient" description="استعلام وتعديل بيانات مساحة العمل (السجلات، الكائنات)">

`CoreApiClient` هو العميل الرئيسي للاستعلام وتعديل بيانات مساحة العمل. يُولَّد من مخطط مساحة العمل لديك أثناء `yarn aegis dev` أو `yarn aegis build`، لذا فهو مضبوط الأنواع بالكامل ليتوافق مع كائناتك وحقولك.
...
<Note>
**يتم توليد CoreApiClient في وقت التطوير/البناء.** إذا استخدمته دون تشغيل `yarn aegis dev` أو `yarn aegis build` أولًا، فسيؤدي ذلك إلى خطأ. تحدث عملية التوليد تلقائيًا — إذ يستطلع CLI مخطط GraphQL لمساحة عملك وينشئ عميلًا مضبوط الأنواع باستخدام `@genql/cli`.
</Note>
...
<Note>
  عند تشغيل كودك على Aegis (وظائف منطقية أو مكوّنات أمامية)، يقوم النظام الأساسي بحقن بيانات الاعتماد كمتغيرات بيئية:

  * `AEGIS_API_URL` — عنوان URL الأساسي لواجهة Aegis البرمجية
  * `AEGIS_APP_ACCESS_TOKEN` — مفتاح قصير العمر ذو نطاق يقتصر على الدور الافتراضي لوظيفة تطبيقك
