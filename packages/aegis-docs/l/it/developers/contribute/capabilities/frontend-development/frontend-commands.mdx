...
```bash
npx nx run aegis-front:lint # aggiungere --fix per correggere gli errori di lint
```

## Traduzioni

```bash
npx nx run aegis-front:lingui:extract
npx nx run aegis-front:lingui:compile
```

### Test

```bash
npx nx run aegis-front:test # eseguire i test jest
npx nx run aegis-front:storybook:serve:dev # eseguire storybook
npx nx run aegis-front:storybook:test # eseguire i test # (è necessario che yarn storybook:serve:dev sia in esecuzione)
npx nx run aegis-front:storybook:coverage # (è necessario che yarn storybook:serve:dev sia in esecuzione)
```
