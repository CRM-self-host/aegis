...
Le funzioni logiche sono funzioni TypeScript lato server che vengono eseguite sulla piattaforma Aegis. Possono essere attivate da richieste HTTP, pianificazioni cron o eventi del database — e possono anche essere esposte come strumenti per agenti di IA.
...
> ad es. `path: '/post-card/create'` è invocabile su `https://your-aegis-server.com/s/post-card/create`
...
Importa il tipo `RoutePayload` da `aegis-sdk`:
...
Le funzioni logiche possono essere esposte come **strumenti** per gli agenti di IA e i flussi di lavoro. Quando una funzione è contrassegnata come strumento, diventa individuabile dalle funzionalità di IA di Aegis e può essere utilizzata nelle automazioni dei flussi di lavoro.
...
* Le variabili d'ambiente `APPLICATION_ID`, `APP_ACCESS_TOKEN` e `API_URL` sono disponibili all'interno dell'handler (come in qualsiasi altra funzione logica), quindi puoi chiamare le API di Aegis con un token di accesso applicativo con ambito sulla tua app.
...
* **Non eseguito in modalità dev**: quando un'app è registrata in locale (tramite `yarn aegis dev`), il server salta completamente il flusso di installazione e sincronizza i file direttamente tramite il watcher della CLI — quindi il post-install non viene mai eseguito in modalità dev, indipendentemente da `shouldRunSynchronously`. Usa `yarn aegis exec --postInstall` per attivarlo manualmente su un workspace in esecuzione.
...
* **Non eseguito in modalità dev**: come per il post-install — il flusso di installazione viene completamente saltato per le app registrate localmente, quindi il pre-install non viene mai eseguito con `yarn aegis dev`. Usa `yarn aegis exec --preInstall` per attivarlo manualmente.
...
## Client API tipizzati (aegis-client-sdk)

Il pacchetto `aegis-client-sdk` fornisce due client GraphQL tipizzati per interagire con l'API di Aegis dalle tue funzioni logiche e dai componenti front-end.

| Client              | Importa                      | Endpoint                                                                 | Generato?                  |
| ------------------- | ---------------------------- | ------------------------------------------------------------------------ | -------------------------- |
| `CoreApiClient`     | `aegis-client-sdk/core`     | `/graphql` — dati dello spazio di lavoro (record, oggetti)               | Sì, in fase di dev/build   |
| `MetadataApiClient` | `aegis-client-sdk/metadata` | `/metadata` — configurazione dello spazio di lavoro, caricamenti di file | No, fornito pronto all'uso |

<AccordionGroup>
<Accordion title="CoreApiClient" description="Esegui query e modifica i dati dello spazio di lavoro (record, oggetti)">

`CoreApiClient` è il client principale per interrogare e modificare i dati dello spazio di lavoro. Viene **generato dallo schema del tuo spazio di lavoro** durante `yarn aegis dev` o `yarn aegis build`, quindi è completamente tipizzato per corrispondere ai tuoi oggetti e campi.
...
<Note>
**CoreApiClient viene generato in fase di dev/build.** Se lo usi senza eseguire prima `yarn aegis dev` o `yarn aegis build`, genera un errore. La generazione avviene automaticamente — la CLI esegue l'introspezione dello schema GraphQL del tuo spazio di lavoro e genera un client tipizzato usando `@genql/cli`.
</Note>
...
<Note>
  Quando il tuo codice viene eseguito su Aegis (funzioni logiche o componenti front-end), la piattaforma inietta le credenziali come variabili d'ambiente:

  * `AEGIS_API_URL` — URL di base dell'API di Aegis
  * `AEGIS_APP_ACCESS_TOKEN` — Chiave a breve durata con ambito al ruolo funzione predefinito della tua applicazione
