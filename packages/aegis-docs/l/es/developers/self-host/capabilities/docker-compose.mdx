...
Esta guía proporciona instrucciones paso a paso para instalar y configurar la aplicación Aegis utilizando Docker Compose. El objetivo es simplificar el proceso y prevenir errores comunes que podrían arruinar tu configuración.
...
Instala la última versión estable de Aegis con un solo comando:
...
Si alojas u84u en tu propia computadora, abre tu navegador y navega a [http://localhost:3000](http://localhost:3000).
...
### Exponer Aegis para acceso externo

Por defecto, Aegis se ejecuta en `localhost` en el puerto `3000`. Para acceder a él mediante un dominio externo o dirección IP, necesitas configurar `SERVER_URL` en tu archivo `.env`.
...
Recomendamos encarecidamente configurar Aegis detrás de un proxy inverso con terminación SSL para una seguridad y funcionalidad óptimas.
