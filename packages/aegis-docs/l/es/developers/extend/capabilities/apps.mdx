...
Consulta también: las páginas de referencia de la CLI para [create-aegis-app](https://www.npmjs.com/package/create-aegis-app) y [aegis-sdk CLI](https://www.npmjs.com/package/aegis-sdk).
...
* Añade una dependencia local de `aegis-sdk` y la configuración de Yarn 4
...
* **package.json**: Declara el nombre de la aplicación, la versión, los entornos (Node 24+, Yarn 4) y agrega `aegis-sdk` además de scripts como `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall` y `auth:login` que delegan en la CLI local `aegis`.
...
* `yarn app:dev` genera automáticamente el cliente Aegis tipado en `node_modules/aegis-sdk/generated`.
...
El aegis-sdk proporciona bloques de construcción tipados y funciones auxiliares que utilizas dentro de tu aplicación. A continuación, las partes clave que usarás con más frecuencia.
...
import { defineFunction } from 'aegis-sdk';
...
Cuando un disparador de ruta invoca tu función de lógica, esta recibe un objeto `RoutePayload` que sigue el formato de AWS HTTP API v2. Importa el tipo desde `aegis-sdk`:

```typescript
import { defineFunction, type RoutePayload } from 'aegis-sdk';
...
`yarn app:dev` genera automáticamente el cliente Aegis tipado en `node_modules/aegis-sdk/generated`. Úsalo en tus funciones:
...
Aunque recomendamos usar `create-aegis-app` para la mejor experiencia de inicio, también puedes configurar un proyecto manualmente. No instales la CLI globalmente. En su lugar, agrega `aegis-sdk` como dependencia local y conecta scripts en tu package.json:

```bash filename="Terminal"
yarn add -D aegis-sdk
```
