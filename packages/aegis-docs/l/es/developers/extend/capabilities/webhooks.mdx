...
Los webhooks envían datos a tus sistemas en tiempo real cuando ocurren eventos en Aegis — no se requiere sondeo. Úsalos para mantener sincronizados los sistemas externos, activar automatizaciones o enviar alertas.
...
Aegis envía webhooks para estos tipos de eventos:
...
Aegis firma cada solicitud de webhook por seguridad. Valida las firmas para garantizar que las solicitudes sean auténticas.

### Encabezados

| Encabezado                   | Descripción                     |
| ---------------------------- | ------------------------------- |
| `X-Aegis-Webhook-Signature` | Firma HMAC SHA256               |
| `X-Aegis-Webhook-Timestamp` | Marca de tiempo de la solicitud |

### Pasos de validación

1. Obtén la marca de tiempo de `X-Aegis-Webhook-Timestamp`
2. Crea la cadena: `{timestamp}:{JSON payload}`
3. Calcula HMAC SHA256 usando tu secreto de webhook
4. Compara con `X-Aegis-Webhook-Signature`
...
| **Disparador de webhook de flujo de trabajo** | ENTRADA   | Recibir datos en Aegis desde sistemas externos                                   |
