...
```bash
npx nx run aegis-front:lint # pasar --fix para corregir errores de lint
```
...
```bash
npx nx run aegis-front:test # ejecutar pruebas con Jest
npx nx run aegis-front:storybook:serve:dev # ejecutar Storybook
npx nx run aegis-front:storybook:test # ejecutar pruebas # (requiere que yarn storybook:serve:dev esté en ejecución)
npx nx run aegis-front:storybook:coverage # (requiere que yarn storybook:serve:dev esté en ejecución)
```
