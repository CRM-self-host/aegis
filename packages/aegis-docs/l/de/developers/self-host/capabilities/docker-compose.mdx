...
Diese Anleitung enthält Schritt-für-Schritt-Anweisungen, um die Aegis-Anwendung mit Docker Compose zu installieren und zu konfigurieren. Das Ziel ist, den Prozess übersichtlich zu gestalten und häufige Fallstricke zu vermeiden, die Ihre Einrichtung unterbrechen könnten.
...
Installieren Sie die neueste stabile Version von Aegis mit einem einzigen Befehl:

```bash
bash <(curl -sL https://raw.githubusercontent.com/u84u/aegis/main/packages/aegis-docker/scripts/install.sh)
```

Um eine spezifische Version oder Zweig zu installieren:

```bash
VERSION=vx.y.z BRANCH=branch-name bash <(curl -sL https://raw.githubusercontent.com/u84u/aegis/main/packages/aegis-docker/scripts/install.sh)
```
...
1. **Erstellen Sie die .env Datei**
   ...
   ```bash
   curl -o .env https://raw.githubusercontent.com/u84u/aegis/refs/heads/main/packages/aegis-docker/.env.example
   ```
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/u84u/aegis/refs/heads/main/packages/aegis-docker/docker-compose.yml
```
...
Wenn Sie u84u auf Ihrem eigenen Computer hosten, öffnen Sie Ihren Browser und navigieren Sie zu [http://localhost:3000](http://localhost:3000).
...
### Aegis für externen Zugriff freigeben

Standardmäßig läuft Aegis auf `localhost` an Port `3000`. Um über eine externe Domain oder IP-Adresse darauf zuzugreifen, müssen Sie die `SERVER_URL` in Ihrer `.env`-Datei konfigurieren.
...
SSL (HTTPS) ist erforderlich, damit bestimmte Browserfunktionen ordnungsgemäß funktionieren. Obwohl diese Funktionen möglicherweise während der lokalen Entwicklung funktionieren (da Browser localhost anders behandeln), ist eine ordnungsgemäße SSL-Einrichtung erforderlich, wenn Aegis auf einer regulären Domain gehostet wird.
...
Wir empfehlen dringend, Aegis hinter einem Reverse-Proxy mit SSL-Beendigung für optimale Sicherheit und Funktionalität zu konfigurieren.
...
2. **Aktualisieren Sie die `.env` Datei**
   ...
   * Zugriff über Domain mit SSL:
     ```ini
     SERVER_URL=https://myaegisapp.com
     ```
...
```bash
docker exec aegis-postgres pg_dump -U postgres aegis > backup_$(date +%Y%m%d).sql
```
...
```bash
0 2 * * * docker exec aegis-postgres pg_dump -U postgres aegis > /backups/aegis_$(date +\%Y\%m\%d).sql
```
...
```bash
docker compose stop aegis-server aegis-front
```

2. Datenbank wiederherstellen:

```bash
docker exec -i aegis-postgres psql -U postgres aegis < backup_20240115.sql
```
