...
Wenn Sie bereits eine bestehende Aegis-Instanz mit Produktionsdaten haben, **NICHT** diesen Schritten folgen, da diese permanent Ihre Datenbank löschen!

Bei der ersten Installation von Aegis möchten Sie möglicherweise das Standard-Datenbankpasswort ändern.
...
Während der Aegis-Installation müssen Sie Ihre Postgres-Datenbank mit den richtigen Schemata, Erweiterungen und Benutzern bereitstellen.
...
#### Modul 'aegis-emails' oder seine entsprechenden Typdeklarationen nicht gefunden.

Sie müssen das Paket `aegis-emails` erstellen, bevor Sie die Initialisierung der Datenbank mit `npx nx run aegis-emails:build` ausführen.

#### Fehlendes Aegis-x Paket

Stellen Sie sicher, dass Sie `yarn` im Root-Verzeichnis ausführen und dann `npx nx server:dev aegis-server` ausführen. Wenn das immer noch nicht funktioniert, versuchen Sie, das fehlende Paket manuell zu erstellen.
...
#### Beim Ausführen von `npx nx start` oder `npx nx start aegis-front` wird ein Speicherfehler angezeigt

Führen Sie nur die Services aus, die Sie benötigen, anstatt `npx nx start`. Wenn Sie zum Beispiel am Server arbeiten, führen Sie nur `npx nx worker aegis-server` aus.

**Wenn es nicht funktioniert:**
Wenn Sie versucht haben, nur `npx nx run aegis-server:start` auf WSL zu starten und es mit dem untenstehenden Speicherfehler fehlschlägt:
...
**Wenn es nicht funktioniert:**
Untersuchen Sie, welche Prozesse den größten Teil Ihres RAMs beanspruchen. Bei Aegis haben wir festgestellt, dass einige vscode-Erweiterungen viel RAM verwenden, daher deaktivieren wir sie vorübergehend.
...
* Öffnen Sie Ihre Aegis-App, navigieren Sie zu `/settings`, und aktivieren Sie den **Erweitert**-Schalter unten links auf dem Bildschirm.
* Erstellen Sie einen neuen Webhook.
* Fügen Sie **Ihre einzigartige Webhook-URL** in das Feld **Endpoint-Url** in Aegis ein. Stellen Sie die **Filter** auf `Companies` und `Created` ein.
...
Kommentieren Sie das Checker-Plugin in `packages/aegis-ui/vite-config.ts` wie im folgenden Beispiel aus
...
Wenn Sie Aegis hinter einem Reverse-Proxy ausführen und Verbindungsprobleme haben:
...
2. **Reverse-Proxy-Einstellungen prüfen:**
   ...
   * Stellen Sie sicher, dass Ihr Reverse-Proxy Anfragen korrekt an den Aegis-Server weiterleitet.
   * Stellen Sie sicher, dass Header wie `X-Forwarded-For` und `X-Forwarded-Proto` korrekt gesetzt sind.

3. **Dienste neu starten:**

   Starten Sie nach Änderungen sowohl den Reverse-Proxy als auch die Aegis-Container neu.
...
* Unterstützung aus der Community:
  ...
  Wenden Sie sich an die [Aegis Community](https://github.com/u84u/aegis/issues) oder [Support-Kanäle](https://discord.gg/cx5n4Jzs57) für Unterstützung.
