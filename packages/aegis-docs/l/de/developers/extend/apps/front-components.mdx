...
description: Erstellen Sie React-Komponenten, die innerhalb der Aegis-UI gerendert werden und durch eine Sandbox isoliert sind.
icon: window-maximize
---

Front-Komponenten sind React-Komponenten, die direkt innerhalb der Aegis-UI gerendert werden. Sie laufen in einem **isolierten Web Worker** unter Verwendung von Remote DOM — Ihr Code wird in einer Sandbox ausgeführt, rendert jedoch nativ auf der Seite, nicht in einem iframe.

## Wo Front-Komponenten verwendet werden können

Front-Komponenten können an zwei Stellen innerhalb von Aegis gerendert werden:
...
Nach dem Synchronisieren mit `yarn aegis dev` (oder durch einmaliges Ausführen von `yarn aegis dev --once`) erscheint die Schnellaktion oben rechts auf der Seite:
...
import { defineFrontComponent } from 'aegis-sdk/define';
import { useRecordId, enqueueSnackbar } from 'aegis-sdk/front-component';
...
Da die Komponente `null` zurückgibt, überspringt Aegis das Rendern eines Containers dafür — im Layout entsteht kein Leerraum. Die Komponente hat dennoch Zugriff auf alle Hooks und die Host-Kommunikations-API.

## SDK-Command-Komponenten

Das Paket `aegis-sdk` stellt vier Command-Hilfskomponenten bereit, die für Headless-Front-Komponenten ausgelegt sind. Jede Komponente führt beim Mounten eine Aktion aus, behandelt Fehler durch Anzeige einer Snackbar-Benachrichtigung und unmountet die Front-Komponente nach Abschluss automatisch.

Importieren Sie sie aus `aegis-sdk/command`:
...
Front-Komponenten können Navigation, Modals und Benachrichtigungen mittels Funktionen aus `aegis-sdk` auslösen:
...
import { enqueueSnackbar, closeSidePanel } from 'aegis-sdk/front-component';
import { CoreApiClient } from 'aegis-sdk/clients';
...
Mit dem Feld `conditionalAvailabilityExpression` können Sie basierend auf dem aktuellen Seitenkontext steuern, wann ein Befehl sichtbar ist. Importieren Sie typisierte Variablen und Operatoren aus `aegis-sdk`, um Ausdrücke zu erstellen:
...
* **Aegis-UI-Komponenten** — Import aus `aegis-sdk/ui` (Button, Tag, Status, Chip, Avatar und mehr)
