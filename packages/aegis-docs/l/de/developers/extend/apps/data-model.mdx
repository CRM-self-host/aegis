...
* Das zur Laufzeit als `AEGIS_APP_ACCESS_TOKEN` injizierte Token wird aus dieser Rolle abgeleitet.
...
* Ein funktionierendes Beispiel finden Sie unter: [`hello-world/src/roles/function-role.ts`](https://github.com/u84u/aegis/blob/main/packages/aegis-apps/hello-world/src/roles/function-role.ts).
...
* Sie können mit `yarn aegis add` neue Objekte erzeugen; der Assistent führt Sie durch Benennung, Felder und Beziehungen.

<Note>
**Basisfelder werden automatisch erstellt.** Wenn Sie ein benutzerdefiniertes Objekt definieren, fügt Aegis automatisch Standardfelder hinzu
...
Verwenden Sie `defineField()`, um Objekten, die Ihnen nicht gehören — etwa Standardobjekten von Aegis (Person, Company usw.) — Felder hinzuzufügen oder Objekten aus anderen Apps. Im Gegensatz zu Inline-Feldern in `defineObject()` benötigen eigenständige Felder einen `objectUniversalIdentifier`, um anzugeben, welches Objekt sie erweitern:
...
Hauptpunkte:
* Der `objectUniversalIdentifier` identifiziert das Zielobjekt. Für Standardobjekte verwenden Sie `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`, die aus `aegis-sdk` exportiert werden.
...
Relationen verbinden Objekte miteinander. In Aegis sind Relationen stets **bidirektional** — Sie definieren beide Seiten, und jede Seite referenziert die andere.
...
Um eine Relation mit einem integrierten Aegis-Objekt (Person, Company usw.) zu erstellen, verwenden Sie `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Entitäten mit `yarn aegis add` erstellen
...
| Feld                   | `yarn aegis add field`              | `src/fields/\<name>.ts`                |
| Logikfunktion          | `yarn aegis add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Frontend-Komponente    | `yarn aegis add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rolle                  | `yarn aegis add role`               | `src/roles/\<name>.ts`                 |
| Skill                  | `yarn aegis add skill`              | `src/skills/\<name>.ts`                |
| Agent                  | `yarn aegis add agent`              | `src/agents/\<name>.ts`                |
| Ansicht                | `yarn aegis add view`               | `src/views/\<name>.ts`                 |
| Navigationsmenüeintrag | `yarn aegis add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Seitenlayout           | `yarn aegis add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### Was der Scaffolder generiert

Jeder Entitätstyp hat seine eigene Vorlage. Zum Beispiel fragt `yarn aegis add object` nach:
