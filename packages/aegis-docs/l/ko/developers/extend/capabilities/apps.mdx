...
참고: [create-aegis-app](https://www.npmjs.com/package/create-aegis-app) 및 [twenty-sdk CLI](https://www.npmjs.com/package/twenty-sdk)의 CLI 참고 페이지도 확인하세요.
...
* **package.json**: 앱 이름, 버전, 엔진(Node 24+, Yarn 4)을 선언하고, `twenty-sdk`와 함께 `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall`, `auth:login` 같은 스크립트를 추가합니다. 이 스크립트들은 로컬 `aegis` CLI에 위임됩니다.
...
* `yarn app:dev`는 `node_modules/twenty-sdk/generated`에 타입드 Aegis 클라이언트를 자동으로 생성합니다.
...
`yarn app:dev`는 `node_modules/twenty-sdk/generated`에 타입드 Aegis 클라이언트를 자동으로 생성합니다. 함수에서 사용하세요:
...
최적의 시작 경험을 위해 `create-aegis-app` 사용을 권장하지만, 프로젝트를 수동으로 설정할 수도 있습니다. CLI를 전역으로 설치하지 마세요. 대신 `twenty-sdk`를 로컬 종속성으로 추가하고 package.json에 스크립트를 연결하세요:
