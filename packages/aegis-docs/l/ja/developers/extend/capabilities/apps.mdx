...
参考: [create-aegis-app](https://www.npmjs.com/package/create-aegis-app) および [twenty-sdk CLI](https://www.npmjs.com/package/twenty-sdk) の CLI リファレンスページをご覧ください。
...
* **package.json**: Declares the app name, version, engines (Node 24+, Yarn 4), and adds `twenty-sdk` plus scripts like `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall`, and `auth:login` that delegate to the local `aegis` CLI.
...
* `yarn app:dev` は `node_modules/twenty-sdk/generated` に型付き Aegis クライアントを自動生成します。
...
`yarn app:dev` は `node_modules/twenty-sdk/generated` に型付き Aegis クライアントを自動生成します。 関数内で使用します：
...
最適な導入体験のために `create-aegis-app` の使用を推奨しますが、手動でプロジェクトをセットアップすることもできます。 CLI をグローバルにインストールしないでください。 代わりに、`twenty-sdk` をローカル依存関係として追加し、package.json にスクリプトを設定します：
