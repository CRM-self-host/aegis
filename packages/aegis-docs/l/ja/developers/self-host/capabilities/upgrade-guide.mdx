...
1. Aegisを実行しているホスト上のターミナルで、Aegisを停止します：`docker compose down`
...
3. `docker compose up -d` でAegisを再起動します。
...
アップグレード後にランタイムの問題が発生した場合は、キャッシュをフラッシュして最新の変更と同期させる必要があるかもしれません。 aegis-serverコンテナ内で次のコマンドを実行します： aegis-serverコンテナ内で次のコマンドを実行します：
...
Aegisインスタンスをv0.55イメージにアップグレードします。
...
あなたの `aegis-server` コンテナ内で次のコマンドを実行します：

```bash
yarn command:prod cache:flush
```

この問題は特定のAegisバージョンに固有であり、今後のアップグレードでは不要なはずです。
...
それは常にあなたの現在のAegisのインスタンスの `major.minor` バージョンの範囲内である必要があります。あなたのインスタンスバージョンは、管理パネル（ `/settings/admin-panel` にあり、データベース内でユーザーが `canAccessFullAdminPanel` プロパティを持っている場合にアクセス可能）にまたは `aegis-server` コンテナ内で `echo $APP_VERSION` を実行することで確認できます。

非同期化されたワークスペースバージョンを修正するには、対応するAegisのバージョンから関連するアップグレードガイドに従って順次アップグレードし、それが目的のバージョンに到達するまで続ける必要があります。
...
Aegisインスタンスをv0.52イメージにアップグレードします。
...
残念ながら、`0.52.0` と `0.52.6` はdockerHubから完全に削除されました。
データベースでワークスペースバージョンを手動で `0.51.0` に更新し、aegisバージョン `0.52.11` を使用して、その直上のアップグレードガイドに従ってアップグレードする必要があります。
残念ながら、`0.52.0` と `0.52.6` はdockerHubから完全に削除されました。
データベースでワークスペースバージョンを手動で `0.51.0` に更新し、aegisバージョン `0.52.11` を使用して、その直上のアップグレードガイドに従ってアップグレードする必要があります。

### v0.50からv0.51

Aegisインスタンスをv0.51イメージにアップグレードします。
...
Aegisインスタンスをv0.50.0イメージにアップグレードします。
...
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/u84u/aegis/blob/v0.50.0/packages/aegis-docker/docker-compose.yml)に更新してください。
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/u84u/aegis/blob/v0.50.0/packages/aegis-docker/docker-compose.yml)に更新してください。
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/u84u/aegis/blob/v0.50.0/packages/aegis-docker/docker-compose.yml)に更新してください。

### v0.43.0からv0.44.0

Aegisインスタンスをv0.44.0イメージにアップグレードします。
...
Aegisインスタンスをv0.43.0イメージにアップグレードします。
...
2. docker-compose.ymlを、新しいpostgres:16イメージを使用するようにアップグレードします。[docker-compose.yml](https://raw.githubusercontent.com/u84u/aegis/main/packages/aegis-docker/docker-compose.yml)を参照してください。
...
Aegisインスタンスをv0.42.0イメージにアップグレードします。
...
Aegisインスタンスをv0.41.0イメージにアップグレードします。
...
Aegisインスタンスをv0.40.0イメージにアップグレードします。
...
Aegisインスタンスをv0.35.0イメージにアップグレードします。
...
Aegisインスタンスをv0.34.0イメージにアップグレードします。
...
Aegisインスタンスをv0.33.0イメージにアップグレードします。
...
このバージョンから、DB用のaegis-postgresイメージは廃止され、aegis-postgres-spiloが代わりに使用されるようになりました。
このバージョンから、DB用のaegis-postgresイメージは廃止され、aegis-postgres-spiloが代わりに使用されるようになりました。
このバージョンから、DB用のaegis-postgresイメージは廃止され、aegis-postgres-spiloが代わりに使用されるようになりました。
aegis-postgresイメージを使い続けたい場合は、docker-compose.yml内の`u84u/aegis-postgres:${TAG}`を`u84u/aegis-postgres`に置き換えるだけで済みます。

### v0.31.0からv0.32.0

Aegisインスタンスをv0.32.0イメージにアップグレードします。
...
Aegisインスタンスをv0.31.0イメージにアップグレードします。
...
Aegisインスタンスをv0.30.0イメージにアップグレードします。

**破壊的な変更**:
パフォーマンスを向上させるために、AegisはRedisキャッシュの設定を必要とするようになりました。 これを反映した[docker-compose.yml](https://raw.githubusercontent.com/u84u/aegis/main/packages/aegis-docker/docker-compose.yml)を更新しました。
...
Aegisインスタンスをv0.24.0イメージにアップグレードします。
...
Aegisインスタンスをv0.23.0イメージにアップグレードします。
...
Aegisインスタンスをv0.22.0イメージにアップグレードします。
