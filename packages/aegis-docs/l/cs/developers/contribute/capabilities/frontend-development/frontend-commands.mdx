...
```bash
npx nx run aegis-front:lint # přidejte --fix pro opravu chyb ve formátování
```
...
```bash
npx nx run aegis-front:test # spusťte jest testy
npx nx run aegis-front:storybook:serve:dev # spusťte storybook
npx nx run aegis-front:storybook:test # spusťte testy # (vyžaduje, aby byl spuštěn yarn storybook:serve:dev)
npx nx run aegis-front:storybook:coverage # (vyžaduje, aby byl spuštěn yarn storybook:serve:dev)
```
