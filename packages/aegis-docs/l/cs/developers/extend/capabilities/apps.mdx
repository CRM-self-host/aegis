---
title: Aplikace Aegis
description: Vytvářejte a spravujte přizpůsobení Aegis jako kód.
---
...
Aplikace vám umožňují rozšířit Aegis o vlastní objekty, pole, logické funkce, front-endové komponenty, AI schopnosti a další — vše je spravováno jako kód. Místo konfigurace všeho přes uživatelské rozhraní definujete v TypeScriptu svůj datový model a logiku a nasadíte je do jednoho nebo více pracovních prostorů.
...
* **Front-endové komponenty** — komponenty Reactu, které se vykreslují v uživatelském rozhraní Aegis (stránky záznamů, příkazová nabídka, postranní panely)
* **Dovednosti AI a agenti** — rozšiřte AI v Aegis o vlastní možnosti
...
Tímto se vytvoří kostra nové aplikace, volitelně se spustí lokální server Aegis a začne sledovat změny ve vašich souborech. Podrobný postup najdete v průvodci [Začínáme](/l/cs/developers/extend/apps/getting-started).
...
| `defineFrontComponent()`           | Komponenty Reactu v uživatelském rozhraní Aegis |
...
1. **`yarn aegis dev`** — sleduje zdrojové soubory, při změně znovu sestaví, synchronizuje se serverem a generuje typované klienty API
2. **`yarn aegis build`** — vytvoří distribuovatelný build
3. **`yarn aegis deploy`** — nasadí na vzdálený server Aegis
4. **`yarn aegis add`** — interaktivně vytvoří kostru nové entity
