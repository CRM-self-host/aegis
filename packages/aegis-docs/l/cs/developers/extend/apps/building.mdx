---
title: Architektura
description: Jak fungují aplikace Aegis — sandboxing, životní cyklus a stavební bloky.
icon: sitemap
---

Aplikace Aegis jsou balíčky TypeScriptu, které rozšiřují váš pracovní prostor o vlastní objekty, logiku, komponenty UI a funkce AI. Běží na platformě Aegis s plnou izolací (sandboxingem) a řízením oprávnění.

## Jak aplikace fungují

Aplikace je kolekce **entit** deklarovaných pomocí funkcí `defineEntity()` z balíčku `aegis-sdk`. SDK tyto deklarace detekuje pomocí analýzy AST při sestavení a vytváří **manifest** — úplný popis toho, co vaše aplikace přidává do pracovního prostoru.
...
| **Frontendová komponenta**    | Izolované React UI na stránce Aegis            | [Frontendové komponenty](/l/cs/developers/extend/apps/front-components) |
...
* **Frontendové komponenty** běží ve Web Workerech s využitím Remote DOM — jsou oddělené od hlavní stránky, ale vykreslují nativní prvky DOM (nikoli iframy). Komunikují s Aegis prostřednictvím hostitelského API pro předávání zpráv.
* **Oprávnění** jsou vynucována na úrovni API. Běhový token (`AEGIS_APP_ACCESS_TOKEN`) je odvozen z role definované v `defineApplication()`.
...
* **`yarn aegis dev`** — sleduje vaše zdrojové soubory a průběžně synchronizuje změny s připojeným serverem Aegis. Typovaný klient API se při změně schématu automaticky znovu vygeneruje.
* **`yarn aegis build`** — zkompiluje TypeScript, zabalí logické funkce a frontendové komponenty pomocí esbuild a vytvoří manifest.
...
<CardGroup cols={2}>
  ...
  <Card title="Frontendové komponenty" icon="window-maximize" href="/l/cs/developers/extend/apps/front-components">
    Izolované komponenty Reactu v uživatelském rozhraní Aegis.
  </Card>
