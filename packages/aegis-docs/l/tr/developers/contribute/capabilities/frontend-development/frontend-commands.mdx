...
```bash
npx nx run aegis-front:lint # lint hatalarını düzeltmek için --fix'i geç
```
...
```bash
npx nx run aegis-front:test # jest testlerini çalıştır
npx nx run aegis-front:storybook:serve:dev # storybook'u çalıştır
npx nx run aegis-front:storybook:test # testleri çalıştır # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
npx nx run aegis-front:storybook:coverage # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
```
