...
* **Derlemelere dahil edilir** — `yarn aegis build`, tüm genel varlıkları dağıtım çıktısına paketler.

### `getPublicAssetUrl` ile genel varlıklara erişme

`aegis-sdk` içindeki `getPublicAssetUrl` yardımcı işlevini kullanarak `public/` dizininizdeki bir dosyanın tam URL'sini alın. Hem **mantık işlevlerinde** hem de **ön uç bileşenlerinde** çalışır.
...
Her iki ortamda da `aegis-client-sdk/core` ve `aegis-client-sdk/metadata` önceden sağlanmış modüller olarak mevcuttur — bunlar paketlenmez, ancak çalışma zamanında sunucu tarafından çözülür.

## Uygulamanızı test etme

SDK, test kodundan uygulamanızı derlemenize, dağıtmanıza, yüklemenize ve kaldırmanıza olanak tanıyan programatik API'ler sağlar. Tiplenmiş API istemcileriyle birlikte [Vitest](https://vitest.dev/) kullanarak, uygulamanızın gerçek bir Aegis sunucusunda uçtan uca çalıştığını doğrulayan entegrasyon testleri yazabilirsiniz.
...
`aegis-sdk/cli` alt yolu, test kodundan doğrudan çağırabileceğiniz fonksiyonları dışa aktarır:
...
Yerel Aegis sunucunuzun çalıştığından emin olun, ardından:
...
### Fonksiyonları çalıştırma (`yarn aegis exec`)
...
### Fonksiyon günlüklerini görüntüleme (`yarn aegis logs`)
...
<Note>
Bu, Docker konteyner günlüklerini gösteren `yarn aegis server logs` komutundan farklıdır. `yarn aegis logs`, uygulamanızın fonksiyon yürütme günlüklerini Aegis sunucusundan gösterir.
</Note>

### Bir uygulamayı kaldırma (`yarn aegis uninstall`)
...
Bir **uzak**, uygulamanızın bağlandığı Aegis sunucusudur. Kurulum sırasında iskelet oluşturucu sizin için otomatik olarak bir tane oluşturur. Dilediğiniz zaman daha fazla uzak ekleyebilir veya aralarında geçiş yapabilirsiniz.
...
Kimlik bilgileriniz `~/.aegis/config.json` içinde saklanır.
...
2. `u84u/aegis/.github/actions/spawn-aegis-docker-image` eylemini kullanarak geçici bir Aegis sunucusu başlatır
3. `yarn install --immutable` ile bağımlılıkları kurar
4. Eylem çıktılarından enjekte edilen `AEGIS_API_URL` ve `AEGIS_API_KEY` ile `yarn test` çalıştırır
...
Herhangi bir gizli değişken yapılandırmanız gerekmez — `spawn-aegis-docker-image` eylemi, koşucu içinde doğrudan geçici bir Aegis sunucusu başlatır ve bağlantı ayrıntılarını çıktı olarak verir. `GITHUB_TOKEN` gizli değişkeni GitHub tarafından otomatik olarak sağlanır.

`latest` yerine belirli bir Aegis sürümünü sabitlemek için iş akışının başındaki `AEGIS_VERSION` ortam değişkenini değiştirin.
