...
description: Aegis'nin UI'si içinde, korumalı alan (sandbox) izolasyonu ile görüntülenen React bileşenleri oluşturun.
icon: window-maximize
---

Ön uç bileşenler, Aegis'nin UI'si içinde doğrudan görüntülenen React bileşenleridir. Remote DOM kullanan izole bir Web Worker içinde çalışırlar — kodunuz izole bir ortamda (sandbox) çalışır ancak bir iframe içinde değil, sayfada yerel olarak işlenir.

## Ön uç bileşenlerinin kullanılabileceği yerler

Ön uç bileşenler, Aegis içinde iki konumda işlenebilir:
...
`yarn aegis dev` ile senkronize ettikten sonra (veya tek seferlik bir `yarn aegis dev --once` çalıştırdıktan sonra), hızlı işlem sayfanın sağ üst köşesinde görünür:
...
Bileşen `null` döndürdüğü için, Aegis bunun için bir kapsayıcı oluşturmayı atlar — düzende boş alan görünmez. Bileşen yine de tüm hook'lara ve host iletişim API'sine erişime sahiptir.

## SDK Command bileşenleri

`aegis-sdk` paketi, headless ön bileşenler için tasarlanmış dört Command yardımcı bileşeni sağlar. Her bileşen bağlandığında bir eylem yürütür, hataları bir snackbar bildirimi göstererek ele alır ve tamamlandığında ön bileşeni otomatik olarak kaldırır.

Bunları `aegis-sdk/command` içinden içe aktarın:
...
Ön uç bileşenleri, `aegis-sdk`'deki işlevleri kullanarak gezinmeyi, modalları ve bildirimleri tetikleyebilir:
...
`conditionalAvailabilityExpression` alanı, geçerli sayfa bağlamına göre bir komutun ne zaman görünür olacağını kontrol etmenizi sağlar. İfadeler oluşturmak için `aegis-sdk`'den türlendirilmiş değişkenleri ve operatörleri içe aktarın:
...
* **Aegis UI bileşenleri** — `aegis-sdk/ui` içinden içe aktarın (Button, Tag, Status, Chip, Avatar ve daha fazlası)
