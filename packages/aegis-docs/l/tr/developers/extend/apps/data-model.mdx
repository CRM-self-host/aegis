---
title: Veri modeli
description: Aegis SDK ile nesneleri, alanları, rolleri ve uygulama meta verilerini tanımlayın.
icon: database
---

`aegis-sdk` paketi, uygulamanızın veri modelini tanımlamak için `defineEntity` işlevlerini sağlar. SDK'nin varlıklarınızı algılayabilmesi için `export default defineEntity({...})` kullanmanız gerekir. Bu fonksiyonlar, derleme zamanında yapılandırmanızı doğrular ve IDE otomatik tamamlama ile tür güvenliği sağlar.
...
* `AEGIS_APP_ACCESS_TOKEN` olarak enjekte edilen çalışma zamanı belirteci bu rolden türetilir.
...
* Çalışan bir örnek için bkz.: [`hello-world/src/roles/function-role.ts`](https://github.com/u84u/aegis/blob/main/packages/aegis-apps/hello-world/src/roles/function-role.ts).
...
* `yarn aegis add` kullanarak, adlandırma, alanlar ve ilişkiler konusunda sizi yönlendirerek yeni nesneler oluşturabilirsiniz.

<Note>
**Temel alanlar otomatik olarak oluşturulur.** Özel bir nesne tanımladığınızda Aegis, standart alanları otomatik olarak ekler
...
Önemli noktalar:
* `objectUniversalIdentifier` hedef nesneyi tanımlar. Standart nesneler için, `aegis-sdk`'den dışa aktarılan `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`'ı kullanın.
...
İlişkiler nesneleri birbirine bağlar. Aegis'de ilişkiler her zaman **çift yönlüdür** — her iki tarafı da tanımlarsınız ve her taraf diğerine başvurur.
...
Yerleşik bir Aegis nesnesiyle (Person, Company, vb.) ilişki oluşturmak için `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` kullanın:
...
## `yarn aegis add` ile varlıklar için iskelet oluşturma
...
| -------------------- | ------------------------------------ | ------------------------------------------------------- |
| Nesne                | `yarn aegis add object`             | `src/objects/\<name>.ts`               |
| Alan                 | `yarn aegis add field`              | `src/fields/\<name>.ts`                |
| Mantık işlevi        | `yarn aegis add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Ön uç bileşeni       | `yarn aegis add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Rol                  | `yarn aegis add role`               | `src/roles/\<name>.ts`                 |
| Beceri               | `yarn aegis add skill`              | `src/skills/\<name>.ts`                |
| Temsilci             | `yarn aegis add agent`              | `src/agents/\<name>.ts`                |
| Görünüm              | `yarn aegis add view`               | `src/views/\<name>.ts`                 |
| Gezinme menüsü öğesi | `yarn aegis add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Sayfa düzeni         | `yarn aegis add pageLayout`         | `src/page-layouts/\<name>.ts`          |

### İskelet oluşturucunun ürettikleri

Her varlık türünün kendi şablonu vardır. Örneğin, `yarn aegis add object` şunları sorar:
