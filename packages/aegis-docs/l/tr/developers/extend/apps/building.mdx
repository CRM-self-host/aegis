---
title: Mimari
description: Aegis uygulamaları nasıl çalışır — korumalı alan, yaşam döngüsü ve yapı taşları.
icon: sitemap
---

Aegis uygulamaları, çalışma alanınızı özel nesneler, mantık, UI bileşenleri ve yapay zekâ yetenekleriyle genişleten TypeScript paketleridir. Tam korumalı alan ve izin kontrolleriyle Aegis platformunda çalışırlar.

## Uygulamalar nasıl çalışır

Bir uygulama, `aegis-sdk` paketindeki `defineEntity()` işlevleri kullanılarak bildirilen **varlıklar** koleksiyonudur. SDK, bu bildirimleri derleme sırasında AST analiziyle algılar ve bir **manifest** üretir — uygulamanızın bir çalışma alanına neler eklediğinin eksiksiz bir açıklaması.
...
| **Ön Uç Bileşeni**       | Aegis'nin sayfasında korumalı alanda React kullanıcı arayüzü | [Ön Uç Bileşenleri](/l/tr/developers/extend/apps/front-components)         |
...
* **Ön uç bileşenleri**, Remote DOM kullanan Web Worker'larda çalışır — ana sayfadan yalıtılmıştır ancak yerel DOM öğelerini (iframe'ler değil) oluşturur. Aegis ile mesaj iletimi yapan bir ana makine API'si aracılığıyla iletişim kurarlar.
* **İzinler**, API düzeyinde uygulanır. Çalışma zamanı belirteci (`AEGIS_APP_ACCESS_TOKEN`), `defineApplication()` içinde tanımlanan rolden türetilir.
...
* **`yarn aegis dev`** — kaynak dosyalarınızı izler ve bağlı bir Aegis sunucusuna değişiklikleri canlı olarak senkronize eder. Şema değiştiğinde tipli API istemcisi otomatik olarak yeniden oluşturulur.
* **`yarn aegis build`** — TypeScript'i derler, mantık işlevlerini ve ön uç bileşenlerini esbuild ile paketler ve bir manifest üretir.
...
<CardGroup cols={2}>
  ...
  <Card title="Ön uç bileşenleri" icon="window-maximize" href="/l/tr/developers/extend/apps/front-components">
    Aegis'nin kullanıcı arayüzünde korumalı alanda React bileşenleri.
  </Card>
