...
逻辑函数是在 Aegis 平台上运行的服务端 TypeScript 函数。 它们可以由 HTTP 请求、cron 调度或数据库事件触发——也可以作为工具暴露给 AI 代理。
...
> 例如 `path: '/post-card/create'` 可在 `https://your-aegis-server.com/s/post-card/create` 调用
...
从 `aegis-sdk` 导入 `RoutePayload` 类型：
...
逻辑函数可以作为供 AI 智能体和工作流使用的**工具**对外提供。 当函数被标记为工具时，Aegis 的 AI 功能即可发现它，并可在工作流自动化中使用。
...
* 你可以将 `isTool` 与触发器结合使用——一个函数既可以作为工具（由 AI 智能体调用），也可以同时由事件触发。
...
* 在处理程序内可使用环境变量 `APPLICATION_ID`、`APP_ACCESS_TOKEN` 和 `API_URL`（与其他逻辑函数相同），因此你可以使用作用域限定到你应用的应用访问令牌调用 Aegis API。
...
* **开发模式下不执行**：当应用在本地注册（通过 `yarn aegis dev`）时，服务器会完全跳过安装流程，并通过 CLI 监视器直接同步文件——因此无论 `shouldRunSynchronously` 如何，安装后在开发模式下都不会运行。 使用 `yarn aegis exec --postInstall` 在运行中的工作区上手动触发它。
...
* **开发模式下不执行**：与安装后相同——对于本地注册的应用将完全跳过安装流程，因此在 `yarn aegis dev` 下不会运行安装前。 使用 `yarn aegis exec --preInstall` 手动触发它。
...
## 类型化 API 客户端（`aegis-client-sdk`）

`aegis-client-sdk` 包提供了两个类型化的 GraphQL 客户端，供你的逻辑函数和前端组件与 Aegis API 交互。

| 客户端                 | 导入                           | 端点                       | 是否生成？     |
| ------------------- | ---------------------------- | ------------------------ | --------- |
| `CoreApiClient`     | `aegis-client-sdk/core`     | `/graphql`——工作区数据（记录、对象） | 是，在开发/构建时 |
| `MetadataApiClient` | `aegis-client-sdk/metadata` | `/metadata`——工作区配置、文件上传  | 否，已预构建提供  |

<AccordionGroup>
<Accordion title="CoreApiClient" description="查询和变更工作区数据（记录、对象）">

`CoreApiClient` 是用于查询和变更工作区数据的主要客户端。 它会在执行 `yarn aegis dev` 或 `yarn aegis build` 时**根据你的工作区架构生成**，因此完全类型化以匹配你的对象和字段。
...
<Note>
**CoreApiClient 在开发/构建时生成。** 如果在未先运行 `yarn aegis dev` 或 `yarn aegis build` 的情况下尝试使用它，将会抛出错误。 该生成过程是自动完成的——CLI 会自省你的工作区 GraphQL 架构，并使用 `@genql/cli` 生成类型化客户端。
</Note>
...
<Note>
  当你的代码在 Aegis 上运行（逻辑函数或前端组件）时，平台会以环境变量的形式注入凭据：

  * `AEGIS_API_URL`——Aegis API 的基础 URL
  * `AEGIS_APP_ACCESS_TOKEN`——作用域限定为你的应用默认函数角色的短期密钥
