...
Logic functions are server-side TypeScript functions that run on the Aegis platform. They can be triggered by HTTP requests, cron schedules, or database events — and can also be exposed as tools for AI agents.
...
> e.g. `path: '/post-card/create'` is callable at `https://your-aegis-server.com/s/post-card/create`
...
Import the `RoutePayload` type from `aegis-sdk`:
...
Logic functions can be exposed as **tools** for AI agents and workflows. When marked as a tool, a function becomes discoverable by Aegis's AI features and can be used in workflow automations.
...
- The environment variables `APPLICATION_ID`, `APP_ACCESS_TOKEN`, and `API_URL` are available inside the handler (same as any other logic function), so you can call the Aegis API with an application access token scoped to your app.
...
- **Not executed in dev mode**: when an app is registered locally (via `yarn aegis dev`), the server skips the install flow entirely and syncs files directly through the CLI watcher — so post-install never runs in dev mode, regardless of `shouldRunSynchronously`. Use `yarn aegis exec --postInstall` to trigger it manually against a running workspace.
...
- **Not executed in dev mode**: same as post-install — the install flow is skipped entirely for locally-registered apps, so pre-install never runs under `yarn aegis dev`. Use `yarn aegis exec --preInstall` to trigger it manually.
...
## Typed API clients (aegis-client-sdk)

The `aegis-client-sdk` package provides two typed GraphQL clients for interacting with the Aegis API from your logic functions and front components.

| Client | Import | Endpoint | Generated? |
|--------|--------|----------|------------|
| `CoreApiClient` | `aegis-client-sdk/core` | `/graphql` — workspace data (records, objects) | Yes, at dev/build time |
| `MetadataApiClient` | `aegis-client-sdk/metadata` | `/metadata` — workspace config, file uploads | No, ships pre-built |

<AccordionGroup>
<Accordion title="CoreApiClient" description="Query and mutate workspace data (records, objects)">

`CoreApiClient` is the main client for querying and mutating workspace data. It is **generated from your workspace schema** during `yarn aegis dev` or `yarn aegis build`, so it is fully typed to match your objects and fields.
...
<Note>
**CoreApiClient is generated at dev/build time.** If you use it without running `yarn aegis dev` or `yarn aegis build` first, it throws an error. The generation happens automatically — the CLI introspects your workspace's GraphQL schema and generates a typed client using `@genql/cli`.
</Note>
...
<Note>
  When your code runs on Aegis (logic functions or front components), the platform injects credentials as environment variables:

  - `AEGIS_API_URL` — Base URL of the Aegis API
  - `AEGIS_APP_ACCESS_TOKEN` — Short-lived key scoped to your application's default function role
