...
description: Build React components that render inside Aegis's UI with sandboxed isolation.
icon: "window-maximize"
---

Front components are React components that render directly inside Aegis's UI. They run in an **isolated Web Worker** using Remote DOM — your code is sandboxed but renders natively in the page, not in an iframe.

## Where front components can be used

Front components can render in two locations within Aegis:
...
After syncing with `yarn aegis dev` (or running a one-shot `yarn aegis dev --once`), the quick action appears in the top-right corner of the page:
...
Because the component returns `null`, Aegis skips rendering a container for it — no empty space appears in the layout. The component still has access to all hooks and the host communication API.

## SDK Command components

The `aegis-sdk` package provides four Command helper components designed for headless front components. Each component executes an action on mount, handles errors by showing a snackbar notification, and automatically unmounts the front component when done.

Import them from `aegis-sdk/command`:
...
Front components can trigger navigation, modals, and notifications using functions from `aegis-sdk`:
...
The `conditionalAvailabilityExpression` field lets you control when a command is visible based on the current page context. Import typed variables and operators from `aegis-sdk` to build expressions:
...
- **Aegis UI components** — import from `aegis-sdk/ui` (Button, Tag, Status, Chip, Avatar, and more)
