---
title: Architecture
description: How Aegis apps work — sandboxing, lifecycle, and the building blocks.
icon: "sitemap"
---

Aegis apps are TypeScript packages that extend your workspace with custom objects, logic, UI components, and AI capabilities. They run on the Aegis platform with full sandboxing and permission controls.

## How apps work

An app is a collection of **entities** declared using `defineEntity()` functions from the `aegis-sdk` package. The SDK detects these declarations via AST analysis at build time and produces a **manifest** — a complete description of what your app adds to a workspace.
...
| **Front Component** | Sandboxed React UI in Aegis's page | [Front Components](/developers/extend/apps/front-components) |
...
- **Front components** run in Web Workers using Remote DOM — sandboxed from the main page but rendering native DOM elements (not iframes). They communicate with Aegis via a message-passing host API.
- **Permissions** are enforced at the API level. The runtime token (`AEGIS_APP_ACCESS_TOKEN`) is derived from the role defined in `defineApplication()`.
...
- **`yarn aegis dev`** — watches your source files and live-syncs changes to a connected Aegis server. The typed API client is regenerated automatically when the schema changes.
- **`yarn aegis build`** — compiles TypeScript, bundles logic functions and front components with esbuild, and produces a manifest.
...
<CardGroup cols={2}>
  ...
  <Card title="Front Components" icon="window-maximize" href="/developers/extend/apps/front-components">
    Sandboxed React components inside Aegis's UI.
  </Card>
