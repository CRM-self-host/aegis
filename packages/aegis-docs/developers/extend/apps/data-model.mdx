---
title: Data Model
description: Define objects, fields, roles, and application metadata with the Aegis SDK.
icon: "database"
---

The `aegis-sdk` package provides `defineEntity` functions to declare your app's data model. You must use `export default defineEntity({...})` for the SDK to detect your entities. These functions validate your configuration at build time and provide IDE autocompletion and type safety.
...
- The runtime token injected as `AEGIS_APP_ACCESS_TOKEN` is derived from this role.
...
- See a working example: [`hello-world/src/roles/function-role.ts`](https://github.com/u84u/aegis/blob/main/packages/aegis-apps/hello-world/src/roles/function-role.ts).
...
- You can scaffold new objects using `yarn aegis add`, which guides you through naming, fields, and relationships.

<Note>
**Base fields are created automatically.** When you define a custom object, Aegis automatically adds standard fields
...
Use `defineField()` to add fields to objects you don't own — such as standard Aegis objects (Person, Company, etc.) or objects from other apps. Unlike inline fields in `defineObject()`, standalone fields require an `objectUniversalIdentifier` to specify which object they extend:
...
Key points:
- `objectUniversalIdentifier` identifies the target object. For standard objects, use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS` exported from `aegis-sdk`.
...
Relations connect objects together. In Aegis, relations are always **bidirectional** — you define both sides, and each side references the other.
...
To create a relation with a built-in Aegis object (Person, Company, etc.), use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
...
## Scaffolding entities with `yarn aegis add`
...
|-------------|---------|----------------|
| Object | `yarn aegis add object` | `src/objects/<name>.ts` |
| Field | `yarn aegis add field` | `src/fields/<name>.ts` |
| Logic function | `yarn aegis add logicFunction` | `src/logic-functions/<name>.ts` |
| Front component | `yarn aegis add frontComponent` | `src/front-components/<name>.tsx` |
| Role | `yarn aegis add role` | `src/roles/<name>.ts` |
| Skill | `yarn aegis add skill` | `src/skills/<name>.ts` |
| Agent | `yarn aegis add agent` | `src/agents/<name>.ts` |
| View | `yarn aegis add view` | `src/views/<name>.ts` |
| Navigation menu item | `yarn aegis add navigationMenuItem` | `src/navigation-menu-items/<name>.ts` |
| Page layout | `yarn aegis add pageLayout` | `src/page-layouts/<name>.ts` |

### What the scaffolder generates

Each entity type has its own template. For example, `yarn aegis add object` asks for:
