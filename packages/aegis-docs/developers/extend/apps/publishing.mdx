...
title: Publishing
icon: "upload"
description: Distribute your Aegis app to the marketplace or deploy it internally.
---
...
- **Deploy a tarball** — upload your app directly to a specific Aegis server for internal or private use.
- **Publish to npm** — list your app in the Aegis marketplace for any workspace to discover and install.
...
This compiles TypeScript sources, transpiles logic functions and front components, and writes everything to `.aegis/output/`. Add `--tarball` to also produce a `.tgz` package for manual distribution or the deploy command.

## Deploying to a server (tarball)

For apps you don't want publicly available — proprietary tools, enterprise-only integrations, or experimental builds — you can deploy a tarball directly to a Aegis server.

### Prerequisites

Before deploying, you need a configured remote pointing to the target server. Remotes store the server URL and authentication credentials locally in `~/.aegis/config.json`.
...
2. Run `yarn aegis deploy` (or `yarn aegis deploy --remote production`)
...
Apps generated with `create-aegis-app` ship with two GitHub Actions workflows out of the box, under `.github/workflows/`. They are ready to run as soon as you push the repo to GitHub — no extra setup is needed for CI, and CD only requires a single secret.
...
2. Spawns an isolated Aegis test instance using the `u84u/aegis/.github/actions/spawn-aegis-app-dev-test@main` composite action (the CI equivalent of `yarn aegis server start --test`).
3. Enables Corepack, sets up Node.js from your `.nvmrc`, and installs dependencies with `yarn install --immutable`.
4. Runs `yarn test`, passing `AEGIS_API_URL` and `AEGIS_API_KEY` from the spawned instance so your tests can talk to a real server.

**Config knobs:**

- `AEGIS_VERSION` (env, defaults to `latest`) — pin the Aegis server version used in CI by editing this in `ci.yml`.
...
Deploys your app to a configured Aegis server on every push to `main`, and optionally from a pull request when the `deploy` label is applied.

**What it does:**

1. Checks out the PR head (for labeled PRs) or the pushed commit.
2. Runs `u84u/aegis/.github/actions/deploy-aegis-app@main` — the CI equivalent of `yarn aegis deploy`.
3. Runs `u84u/aegis/.github/actions/install-aegis-app@main` so the newly deployed version is installed into the target workspace.

**Required configuration:**

| Setting | Where | Purpose |
|---------|-------|---------|
| `AEGIS_DEPLOY_URL` | `env` in `cd.yml` (defaults to `http://localhost:3000`) | The Aegis server to deploy to. Change this to your real server URL before first use. |
| `AEGIS_DEPLOY_API_KEY` | GitHub repo **Settings → Secrets and variables → Actions** | API key with deploy permission on the target server. |

<Note>
The default `AEGIS_DEPLOY_URL` of `http://localhost:3000` is a placeholder — it will not reach anything from a GitHub-hosted runner. Update it to your server's public URL (or use a self-hosted runner with network access) before enabling CD.
</Note>
...
Both workflows reference reusable actions at `@main`, so action updates in the `u84u/aegis` repo are picked up automatically. If you want deterministic builds, replace `@main` with a commit SHA or release tag on each `uses:` line.

## Publishing to npm

Publishing to npm makes your app discoverable in the Aegis marketplace. Any Aegis workspace can browse, install, and upgrade marketplace apps directly from the UI.

### Requirements

- An [npm](https://www.npmjs.com) account
- The `aegis-app` keyword in your `package.json` `keywords` array (add it manually — it is not included by default in the `create-aegis-app` template)
...
The Aegis server syncs its marketplace catalog from the npm registry **every hour**.
...
<Note>
If your app does not define an `aboutDescription` in `defineApplication()`, the marketplace will automatically use your package's `README.md` from npm as the about page content. This means you can maintain a single README for both npm and the Aegis marketplace. If you want a different description in the marketplace, explicitly set `aboutDescription`.
</Note>
...
For other CI systems (GitLab CI, CircleCI, etc.), the same three commands apply: `yarn install`, `yarn aegis build`, then `npm publish` from `.aegis/output`.
...
Go to the **Settings > Applications** page in Aegis, where both marketplace and tarball-deployed apps can be browsed and installed.
...
To install a newer version, deploy or publish it first, then re-run `yarn aegis install`.
</Note>
