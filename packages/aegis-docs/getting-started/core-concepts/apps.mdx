...
title: Apps
icon: "cube"
description: Extend Aegis with code — custom objects, server-side logic, UI components, and AI agents, all as TypeScript packages.
---


Most CRMs give you a config panel. Aegis gives you a platform. Apps are how developers extend Aegis beyond what the UI offers — defining data models, server-side logic, UI components, and AI capabilities as code, then deploying them to one or more workspaces.
...
An app is a TypeScript package that declares **entities** using the `aegis-sdk`:
...
| **Front Components** | Sandboxed React components that render inside Aegis's UI (side panel, widgets, command menu) |
...
`yarn aegis dev` watches your source files, rebuilds on change, and live-syncs to a local Aegis instance. The typed API client regenerates automatically when the schema changes. When you're ready, `yarn aegis deploy` pushes to production. Apps can also be published to npm and listed in the Aegis marketplace.
