...
export const WithSecondaryLinks: Story = {
  args: {
    value: {
      ...
      secondaryLinks: [
        {
          url: 'https://docs.aegis.com',
          label: 'Documentation',
        },
        {
          url: 'https://github.com/u84u/aegis',
          label: 'GitHub',
        },
      ],
    },
...
export const CreatePrimaryLink: Story = {
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://www.aegis.com{enter}');

    const linkDisplay = await canvas.findByText('aegis.com');
...
export const AddSecondaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://docs.aegis.com{enter}');

    const secondaryLink = await canvas.findByText('docs.aegis.com');
...
export const DeletePrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const listItemToDelete = await canvas.findByText('Aegis Website');
...
export const DeletePrimaryLinkAndUseSecondaryLinkAsTheNewPrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const oldPrimaryLink = canvas.queryByText('Aegis Website');
