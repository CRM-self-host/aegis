...
Aegis, and react to GitHub webhook events in real time.

This app showcases how to build a non-trivial third-party connector with the
Aegis SDK: custom objects with rich relationships, navigation menu items,
...
Seven front components surface the connector inside the Aegis UI:
...
### Option A — Live development (`yarn aegis dev`)

Use this when you want every code change to be re-synced into your local
Aegis server automatically.

```bash
cd packages/aegis-apps/community/github-connector
yarn install

# Register your local Aegis server as a remote (interactive prompt).
# When asked for the URL use http://localhost:2021 and paste an API key
# from Settings -> Developers in the Aegis UI.
...
The first `yarn aegis dev` run installs the app on the remote and starts
...
cd packages/aegis-apps/community/github-connector
yarn install
yarn aegis remote add        # same prompts as above
yarn aegis install           # builds and installs once
```

## Configure authentication

Once the app is installed, open the Aegis UI and go to
...
- `owner/number` — e.g. `u84u/24,octo/3`. Owner can be an org or a user.
- Full project URL — e.g. `https://github.com/orgs/u84u/projects/24` or
  `https://github.com/users/octocat/projects/3`.

## Running a sync

In the Aegis UI, open any of the GitHub objects (e.g. **Pull Requests**) and
...
Point a GitHub repo or App webhook at the public URL of your Aegis server,
...
> the original request body bytes. The Aegis SDK currently parses JSON
...
> terminate signature verification at a reverse proxy in front of Aegis.
...
`aegis-app-dev-test` container.

```bash
docker run -d --name aegis-app-dev-test \
  -p 2021:2021 u84u/aegis-app-dev:v2.0.3
