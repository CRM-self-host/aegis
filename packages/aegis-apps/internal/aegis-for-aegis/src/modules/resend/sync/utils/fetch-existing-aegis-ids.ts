...
import { AEGIS_PAGE_SIZE } from '@modules/resend/constants/sync-config';
...
export const fetchExistingAegisIdsByResendIds = async (
  ...
  const result = await client.query({
    [objectNamePlural]: {
      __args: {
        ...
        first: Math.max(resendIds.length, AEGIS_PAGE_SIZE),
      },
      edges: {
        node: {
          id: true,
