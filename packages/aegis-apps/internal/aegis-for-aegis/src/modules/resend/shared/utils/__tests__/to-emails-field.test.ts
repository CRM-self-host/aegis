...
describe('toEmailsField', () => {
  it('returns a plain email unchanged', () => {
    expect(toEmailsField('thomas@mail.aegis.com')).toEqual({
      primaryEmail: 'thomas@mail.aegis.com',
      additionalEmails: null,
    });
  });

  it('extracts the email from "Name <email>" format', () => {
    expect(toEmailsField('Thomas <thomas@mail.aegis.com>')).toEqual({
      primaryEmail: 'thomas@mail.aegis.com',
      additionalEmails: null,
    });
  });
  ...
  it('extracts the email from a quoted display name', () => {
    expect(toEmailsField('"Last, First" <thomas@mail.aegis.com>')).toEqual({
      primaryEmail: 'thomas@mail.aegis.com',
      additionalEmails: null,
    });
  });
