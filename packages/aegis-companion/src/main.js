...
function getAppIcon() {
  try {
    return nativeImage.createFromDataURL(aegisIconDataUrl);
  } catch (error) {
...
// Check Aegis CRM integration status at startup
aegisClient.isConfigured();
...
app.whenReady().then(() => {
  if (process.platform === 'darwin' && app.dock) {
    ...
    if (dockIcon && !dockIcon.isEmpty()) {
      ...
      console.log('Aegis dock icon set successfully');
    } else {
...
// Poll Recall for the audio URL then call the Aegis end-recording logic function.
...
async function endCallRecordingWithRetry(windowId, maxAttempts = 10, delayMs = 5000) {
  try {
    ...
    if (!meeting?.aegisRecordId || !meeting?.recallRecordingId) {
      console.log('[Aegis] No Aegis record or Recall recording ID found, skipping end-recording');
      return;
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (audioUrl) {
          await aegisClient.endCallRecording({
            callRecordingId: meeting.aegisRecordId,
            audioUrl,
            transcriptUrl,
        ...
        console.error(`[Aegis] Attempt ${attempt}/${maxAttempts} — error fetching recording:`, error.message);
      }

      if (attempt < maxAttempts) {
        console.log(`[Aegis] Audio URL not ready, retrying in ${delayMs / 1000}s (${attempt}/${maxAttempts})`);
        await new Promise((resolve) => setTimeout(resolve, delayMs));
      }
    }

    console.error('[Aegis] Audio URL never became available, giving up');
  } catch (error) {
    console.error('Failed to end callRecording in Aegis:', error.message);
  }
}
...
async function createMeetingNoteAndRecord(platformName) {
  ...
  try {
    ...
    // Create a call recording record in Aegis CRM
    if (aegisClient.isConfigured()) {
      try {
        const aegisRecord = await aegisClient.createCallRecording(meetingTitle);

        if (aegisRecord?.id) {
          newMeeting.aegisRecordId = aegisRecord.id;

          if (global.activeMeetingIds && global.activeMeetingIds[detectedMeeting.window.id]) {
            global.activeMeetingIds[detectedMeeting.window.id].aegisRecordId = aegisRecord.id;
          }

          console.log('Created callRecording in Aegis:', aegisRecord.id);
        }
      } catch (error) {
        console.error('Failed to create callRecording in Aegis:', error.message);
      }
    }
