...
const LOG_PREFIX = '[Aegis]';

let configLogged = false;

function getApiUrl() {
  return process.env.AEGIS_API_URL;
}

function getWorkspaceSubdomain() {
  return process.env.AEGIS_WORKSPACE_SUBDOMAIN;
}
...
function isConfigured() {
  ...
  if (!configLogged) {
    ...
    if (!configured) {
      console.log(
        `${LOG_PREFIX} Integration disabled — AEGIS_API_URL or AEGIS_API_KEY not set`,
      );
    } else {
      console.log(
