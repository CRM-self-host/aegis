...
### Aegis CRM Integration (optional)

To automatically create `callRecording` records in Aegis when a meeting starts (and mark them as ended when the meeting closes), configure:
...
The `call-recording` Aegis app must be installed in your workspace first (`packages/aegis-apps/internal/call-recording`). Generate an API key at `<your-aegis-instance>/settings/api-webhooks`.

To launch the Aegis Desktop application, start the server first, then the app:
