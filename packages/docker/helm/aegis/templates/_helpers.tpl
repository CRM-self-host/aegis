...
{{- define "aegis.image.repository" -}}
{{- regexFind "^([^:|]+)" . -}}
{{- end -}}
{{- define "aegis.image.tag" -}}
{{- regexFind ":([^|]+)" . | trimPrefix ":" -}}
{{- end -}}
{{- define "aegis.image.pullPolicy" -}}
{{- regexFind "\\|(.+)$" . | trimPrefix "|" -}}
{{- end -}}

{{/* Check if using external secret for database password */}}
{{- define "aegis.db.useExternalSecret" -}}
...
{{- define "aegis.dbUrl.secretName" -}}
{{- printf "%s-db-url" (include "aegis.fullname" .) -}}
{{- end -}}

{{/* Database password secret name */}}
{{- define "aegis.dbPassword.secretName" -}}
{{- if eq (include "aegis.db.useExternalSecret" .) "true" -}}
{{- .Values.db.external.secretName -}}
{{- else -}}
{{- include "aegis.dbUrl.secretName" . -}}
{{- end -}}
{{- end -}}

{{/* Database password secret key */}}
{{- define "aegis.dbPassword.secretKey" -}}
{{- if eq (include "aegis.db.useExternalSecret" .) "true" -}}
...
{{- define "aegis.redis.useExternalSecret" -}}
...
{{- $host := printf "%s-redis" (include "aegis.fullname" .) -}}
{{- printf "redis://%s.%s.svc.cluster.local:6379" $host (include "aegis.namespace" .) -}}
{{- else -}}
{{- $host := .Values.redis.external.host | default "redis" -}}
{{- $port := .Values.redis.external.port | default 6379 -}}
{{- if or (eq (include "aegis.redis.useExternalSecret" .) "true") (.Values.redis.external.password) -}}
{{- $auth := ":$(REDIS_PASSWORD)@" -}}
...
{{- else -}}
{{- $svc := printf "%s-server" (include "aegis.fullname" .) -}}
{{- $ns := include "aegis.namespace" . -}}
...
{{/* Tokens secret name */}}
{{- define "aegis.secret.tokens.name" -}}
...
{{- define "aegis.secret.tokens.access" -}}
{{- $name := include "aegis.secret.tokens.name" . -}}
{{- $ns := include "aegis.namespace" . -}}
...
{{- define "aegis.server.containerPort" -}}
...
{{- define "aegis.storageEnv" -}}
{{- if eq (include "aegis.storageType" .) "s3" -}}
