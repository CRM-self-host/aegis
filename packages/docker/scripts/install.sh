...
version=${VERSION:-$(curl -s "https://hub.docker.com/v2/repositories/u84u/aegis/tags" | grep -o '"name":"[^"]*"' | grep -v 'latest' | cut -d'"' -f4 | sort -V | tail -n1)}
branch=${BRANCH:-$(curl -s https://api.github.com/repos/u84u/aegis/tags | grep '"name":' | head -n 1 | cut -d '"' -f 4)}
...
# Create a directory named aegis
...
if [ "$answer" = "n" ]; then
  ...
  pid=$!
  while [ ! $(docker inspect --format='{{.State.Health.Status}}' aegis-server-1) = "healthy" ]; do
    sleep 1
  done
  kill $pid
  echo ""
