...
su-exec postgres psql -h localhost -tc \
  "SELECT 1 FROM pg_roles WHERE rolname='aegis'" | grep -q 1 \
  || su-exec postgres psql -h localhost -c "CREATE ROLE aegis WITH LOGIN PASSWORD 'aegis' SUPERUSER"
...
has_workspace=$(PGPASSWORD=aegis psql -h localhost -U aegis -d default -tAc \
