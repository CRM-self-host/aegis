...
Exposes [Exa](https://exa.ai) structured web search to Aegis AI agents
(chat + workflow agents + MCP) as the `app_exa_web_search` tool.

## Installation

1. Register the app on the Aegis server once (admin API / UI):
   `aegis-exa` from npm.
...
The handler calls Aegis's generic app billing endpoint
