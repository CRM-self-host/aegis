...
describe('syncBroadcasts', () => {
  ...
  it('captures html, text, and topicId from the broadcast detail', async () => {
    ...
    mockUpsertRecords.mockResolvedValue({
      ...
      ok: true,
      aegisIdByResendId: new Map([['broadcast-1', 'aegis-broadcast-1']]),
    });

    mockFindAegisIdsByResendId.mockImplementation(
      async (_client: unknown, plural: string) => {
        if (plural === 'resendSegments') {
          return new Map([['seg-1', 'aegis-seg-1']]);
        }

        if (plural === 'resendTopics') {
          return new Map([['topic-1', 'aegis-topic-1']]);
        }
    ...
    expect(createDto).toMatchObject({
      htmlBody: '<p>Hello!</p>',
      textBody: 'Hello!',
      segmentId: 'aegis-seg-1',
      topicId: 'aegis-topic-1',
      subject: 'hello world',
