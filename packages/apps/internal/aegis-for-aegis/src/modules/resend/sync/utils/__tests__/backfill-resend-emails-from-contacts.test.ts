...
describe('backfillResendEmailsFromContacts', () => {
  ...
  it('only fills the FK fields that are currently null and skips records where both are already set', async () => {
    ...
    const result = await backfillResendEmailsFromContacts(
      ...
      new Map([
        [
          'foo@example.com',
          { contactId: 'aegis-contact-foo', personId: 'aegis-person-foo' },
        ],
        ['bar@example.com', { contactId: 'aegis-contact-bar' }],
      ]),
    );
    ...
    expect(callArgs).toEqual([
      {
        id: 'email-1',
        data: {
          contactId: 'aegis-contact-foo',
          personId: 'aegis-person-foo',
        },
      },
      {
        id: 'email-3',
        data: { contactId: 'aegis-contact-bar' },
      },
      {
        id: 'email-4',
        data: { personId: 'aegis-person-foo' },
      },
    ]);
  });
  ...
  it('captures mutation errors per record without aborting the batch', async () => {
    ...
    const result = await backfillResendEmailsFromContacts(
      buildClient(query, mutation),
      new Map([['foo@example.com', { contactId: 'aegis-contact-foo' }]]),
    );
