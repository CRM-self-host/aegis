...
export const handleContactCreatedOrUpdated = async (
  ...
  if (isDefined(firstSegmentResendId)) {
    const segmentIdMap = await findAegisIdsByResendId(
      client,
      'resendSegments',
  ...
  if (isDefined(existingId)) {
    ...
    return { action: 'updated', aegisId: existingId, resendId: data.id, personId };
  }
  ...
  return {
    action: 'created',
    aegisId: createResult.createResendContact?.id,
    resendId: data.id,
    personId,
  };
};
...
const handleContactDeleted = async (
  ...
  if (!isDefined(existingId)) {
    return { skipped: true, reason: 'contact not found in Aegis' };
  }
  ...
  return { action: 'deleted', aegisId: existingId, resendId: data.id };
};
...
const handleEmailEvent = async (
  ...
  if (!isDefined(existingId)) {
    return {
      skipped: true,
      reason: `email ${data.email_id} not found in Aegis`,
    };
  }
