# How to deploy aegis on podman

DISCLAIMER: The k8s and podman deployments are not maintained by the core team.
These files are provided and maintained by the community. Aegis core team
...
2. Start aegis by running `podman-compose up -d`.

If you need to stop aegis, you can do so by running `podman-compose down`.


### Install systemd service (optional)

If you want to install a systemd service to run aegis, you can use the provided systemd service. 

Edit `u84u.service` and change these two variables:


	WorkingDirectory=/opt/apps/aegis
	EnvironmentFile=/opt/apps/aegis/.env
...
Note: this script will enable the service and also start it. So it will assume that aegis is not currently running.
