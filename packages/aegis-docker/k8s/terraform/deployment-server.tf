resource "kubernetes_deployment" "u84u_server" {
  metadata {
  ...
  spec {
    replicas = var.u84u_server_replicas
    selector {
      match_labels = {
        app = "${var.u84u_app_name}-server"
      }
    ...
    template {
      ...
      spec {
        container {
          ...
          volume_mount {
            name       = "server-data"
            mount_path = var.u84u_server_data_mount_path
          }

          volume_mount {
            name       = "docker-data"
            mount_path = var.u84u_docker_data_mount_path
          }
        }
