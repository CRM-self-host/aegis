...
describe('setWorkerEnv', () => {
  ...
  it('should set process.env on globalThis', () => {
    setWorkerEnv({
      AEGIS_APP_ACCESS_TOKEN: 'test-key',
      AEGIS_API_URL: 'https://api.example.com',
    });
    ...
    expect(processEnvironment['AEGIS_APP_ACCESS_TOKEN']).toBe('test-key');
    expect(processEnvironment['AEGIS_API_URL']).toBe('https://api.example.com');
  });
