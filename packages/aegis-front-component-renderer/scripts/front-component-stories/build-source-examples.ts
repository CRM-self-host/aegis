...
const aegisUiIndividualIndex = path.resolve(
  dirname,
  '../../../aegis-ui/dist/individual/individual-entry.js',
);
...
const aegisSharedIndividualDir = path.resolve(
  dirname,
  '../../../aegis-shared/dist/individual',
);

const AEGIS_SHARED_SUBMODULES = [
  'ai',
  'application',
  'constants',
  'database-events',
...
const aegisSharedAliases = Object.fromEntries(
  AEGIS_SHARED_SUBMODULES.map((submodule) => [
    `aegis-shared/${submodule}`,
    path.join(aegisSharedIndividualDir, submodule, 'index.js'),
  ]),
);
...
const STORY_COMPONENTS = [
  ...
  'aegis-ui-example.front-component',
