...
const rewriteSdkImports = (
  ...
  const specifierToBlobUrl: Record<string, string> = {
    'aegis-client-sdk/core': sdkClientUrls.core,
    'aegis-client-sdk/metadata': sdkClientUrls.metadata,
  };
...
const render: WorkerExports['render'] = async (
  ...
  if (isDefined(renderContext.apiUrl)) {
    setWorkerEnv({
      AEGIS_API_URL: renderContext.apiUrl,
    });
  }

  if (isDefined(renderContext.applicationAccessToken)) {
    setWorkerEnv({
      AEGIS_APP_ACCESS_TOKEN: renderContext.applicationAccessToken,
    });
  }
