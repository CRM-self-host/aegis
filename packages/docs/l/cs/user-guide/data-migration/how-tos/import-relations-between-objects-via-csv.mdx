...
Nadřazený („jedna“ strana) musí v Aegis existovat, než na něj budete moci odkazovat.
...
1. Exportujte nadřazené záznamy z Aegis
...
Použijte `https://domain.com` (ne pouze `domain.com`). To odpovídá způsobu, jak Aegis ukládá domény společností, a předchází chybám při párování.
</Warning>
...
Aegis automaticky propojí každý podřízený záznam s odpovídajícím nadřazeným.
...
| **Neexistující záznamy**     | Nadřazený záznam v Aegis neexistuje            | Nejprve importujte nadřazené záznamy nebo zkontrolujte překlepy |
| **Rozdílná velikost písmen** | `Acme.com` v souboru, ale v Aegis `acme.com`   | Zajistěte přesnou shodu velikosti písmen                        |
...
<AccordionGroup>
  <Accordion title="Co když můj nadřazený záznam nemá doménu ani e-mail?">
    Máte dvě možnosti:
    1. Použijte Aegis `id` (exportujte nadřazené záznamy, abyste získali jejich ID)
