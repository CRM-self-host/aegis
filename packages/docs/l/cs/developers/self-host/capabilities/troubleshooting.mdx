...
Pokud máte existující instanci Aegis s produkčními daty, **NEPROVÁDĚJTE** tyto kroky, protože trvale smažou vaši databázi!

Při první instalaci Aegis můžete chtít změnit výchozí heslo pro databázi.
...
Během instalace Aegis musíte nastavit svou postgres databázi s správnými schématy, rozšířeními a uživateli.
...
#### Nelze najít modul 'aegis-emails' ani jeho odpovídající deklarace typů.

Před spuštěním inicializace databáze musíte sestavit balíček `aegis-emails` s `npx nx run aegis-emails:build`

#### Chybí balíček aegis-x

Ujistěte se, že v kořenovém adresáři spouštíte `yarn` a poté spusťte `npx nx server:dev aegis-server`. Pokud stále nefunguje, zkuste chybějící balíček sestavit ručně.
...
#### Při spuštění `npx nx start` nebo `npx nx start aegis-front` došlo k chybě nedostatku paměti

Spusťte pouze služby, které potřebujete, místo `npx nx start`. Například pokud pracujete na serveru, spusťte pouze `npx nx worker aegis-server`

**Pokud to nefunguje:**
Pokud jste se pokusili spustit pouze `npx nx run aegis-server:start` na WSL a selhává to s následující chybou paměti:
...
**Pokud to nefunguje:**
Analyzujte, které procesy vám zabírají většinu paměti na stroji. V Aegis jsme si všimli, že některá rozšíření VScode zabírala hodně paměti RAM, takže jsme je dočasně deaktivovali.
...
* Otevřete svou Aegis aplikaci, přejděte na `/settings` a na levé dolní části obrazovky aktivujte přepínač **Pokročilé**.
* Vytvořte nový webhook.
* Vložte **Svou unikátní webhook URL** do pole **Endpoint Url** ve Aegis. Nastavte **Filtry** na `Companies` a `Created`.
...
Zakomentujte plugin checker v `packages/aegis-ui/vite-config.ts` jako v příkladu níže
...
Pokud provozujete Aegis za reverzní proxy a máte problémy s připojením:
...
2. **Zkontrolujte nastavení reverzní proxy:**
   ...
   * Potvrďte, že vaše reverzní proxy správně směruje požadavky na server Aegis.
   * Ujistěte se, že záhlaví jako `X-Forwarded-For` a `X-Forwarded-Proto` jsou správně nastaveny.

3. **Restartujte služby:**

   Po provedení změn restartujte jak reverzní proxy, tak kontejnery Aegis.
...
* Podpora komunity:
  ...
  Obraťte se na [komunitu Aegis](https://github.com/u84u/aegis/issues) nebo [podpůrné kanály](https://discord.gg/cx5n4Jzs57) pro pomoc.
