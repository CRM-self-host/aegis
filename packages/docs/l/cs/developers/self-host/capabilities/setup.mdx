...
<Warning>
**Instalujete poprvé?** Postupujte podle [průvodce instalací Docker Compose](/l/cs/developers/self-host/capabilities/docker-compose) k rozchození Aegis, a poté se sem vraťte pro konfiguraci.
</Warning>

Aegis nabízí **dvě konfigurační režimy** pro různé potřeby nasazení:
...
1. Přistupte ke své instanci Aegis (obvykle `http://localhost:3000`)
...
[Kompletní technická referenční příručka →](https://github.com/u84u/aegis/blob/main/packages/aegis-server/src/engine/core-modules/aegis-config/config-variables.ts)
</Warning>
...
Ve výchozím nastavení běží Aegis v **režimu jednoho pracovního prostoru** — ideální pro většinu samostatně hostovaných nasazení, kde potřebujete jednu instanci CRM pro svou organizaci.
...
* Jeden pracovní prostor na instanci Aegis
...
Povolte režim více pracovních prostorů pro nasazení typu SaaS, kde více nezávislých týmů potřebuje své vlastní pracovní prostory na téže instanci Aegis.
...
[Zobrazit relevantní zdrojový kód](https://github.com/u84u/aegis/blob/main/packages/aegis-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
Uživatelé musí mít [Licence Microsoft 365](https://admin.microsoft.com/Adminportal/Home), aby mohli používat API Kalendáře a Zpráv. Bez něj nebudou moci synchronizovat svůj účet na Aegis.
</Warning>
...
[Zobrazit relevantní zdrojový kód](https://github.com/u84u/aegis/blob/main/packages/aegis-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
...
<Warning>
Ve výchozím nastavení Aegis ukládá nahrané soubory do místního souborového systému. Pro produkční nasazení použijte S3 nebo službu kompatibilní se S3 (MinIO, DigitalOcean Spaces atd.). aby byly soubory zachovány při restartech kontejnerů a bylo možné škálovat napříč více instancemi serveru.
</Warning>

Nastavte `STORAGE_TYPE=S_3` a nakonfigurujte proměnné `STORAGE_S3_*` prostřednictvím administračního panelu nebo `.env`. Viz [referenci config-variables.ts](https://github.com/u84u/aegis/blob/main/packages/aegis-server/src/engine/core-modules/aegis-config/config-variables.ts) pro úplný seznam proměnných S3.

Při používání S3 s funkcemi závislými na CORS (např. stahování souborů v prohlížeči) se ujistěte, že váš bucket povoluje origin vašeho frontendu Aegis ve své konfiguraci CORS.

## Logické funkce a interpret kódu

Aegis podporuje logické funkce pro pracovní postupy a interpret kódu pro analýzu dat s využitím AI. Obě spouštějí kód poskytnutý uživatelem a z důvodu zabezpečení vyžadují explicitní konfiguraci.
...
<Note>
Při použití `LOGIC_FUNCTION_TYPE=DISABLED` nebo `CODE_INTERPRETER_TYPE=DISABLED` vrátí každý pokus o spuštění chybu. To je užitečné, pokud chcete provozovat Aegis bez těchto schopností.
</Note>
