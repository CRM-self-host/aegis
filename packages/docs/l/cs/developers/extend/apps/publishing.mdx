...
title: Publikování
icon: nahrát
description: Distribuujte svou aplikaci Aegis do Marketplace nebo ji nasaďte interně.
---
...
* **Nasaďte tarball** — nahrajte svou aplikaci přímo na konkrétní server Aegis pro interní nebo soukromé použití.
* **Publish to npm** — uveďte svou aplikaci v Marketplace Aegis, aby ji mohl kterýkoli pracovní prostor objevit a nainstalovat.
...
Tím se zkompilují zdrojové soubory TypeScriptu, transpilují logické funkce a frontendové komponenty a vše se zapíše do `.aegis/output/`. Přidejte `--tarball`, abyste také vytvořili balíček `.tgz` pro ruční distribuci nebo příkaz deploy.

## Nasazení na server (tarball)

U aplikací, které nechcete zpřístupnit veřejně — proprietární nástroje, integrace pouze pro enterprise nebo experimentální buildy — můžete nasadit tarball přímo na server Aegis.

### Předpoklady

Před nasazením potřebujete nakonfigurovaný vzdálený cíl směřující na cílový server. Vzdálené cíle ukládají adresu URL serveru a přihlašovací údaje lokálně v `~/.aegis/config.json`.
...
2. Spusťte `yarn aegis deploy` (nebo `yarn aegis deploy --remote production`)
...
Aplikace vygenerované pomocí `create-aegis-app` jsou hned připravené se dvěma workflowy GitHub Actions ve složce `.github/workflows/`. Jsou připravené ke spuštění hned, jakmile repozitář pushnete na GitHub — pro CI není potřeba žádné další nastavení a CD vyžaduje pouze jeden secret.
...
2. Spustí izolovanou testovací instanci Aegis pomocí složené akce `u84u/aegis/.github/actions/spawn-aegis-app-dev-test@main` (ekvivalent v CI k `yarn aegis server start --test`).
3. Povolí Corepack, nastaví Node.js podle vašeho `.nvmrc` a nainstaluje závislosti pomocí `yarn install --immutable`.
4. Spustí `yarn test` a předá `AEGIS_API_URL` a `AEGIS_API_KEY` ze spuštěné instance, aby vaše testy mohly komunikovat se skutečným serverem.

**Konfigurační volby:**

* `AEGIS_VERSION` (env, výchozí hodnota `latest`) — uzamkněte v CI používanou verzi serveru Aegis úpravou této hodnoty v `ci.yml`.
...
Nasazuje vaši aplikaci na nakonfigurovaný server Aegis při každém pushi do `main` a volitelně také z pull requestu, pokud je přidán štítek `deploy`.

**K čemu slouží:**

1. Provede checkout headu PR (u označených PR) nebo pushnutého commitu.
2. Spustí `u84u/aegis/.github/actions/deploy-aegis-app@main` — ekvivalent v CI k `yarn aegis deploy`.
3. Spustí `u84u/aegis/.github/actions/install-aegis-app@main`, aby se nově nasazená verze nainstalovala do cílového workspace.

**Požadovaná konfigurace:**

| Nastavení               | Kde                                                             | Účel                                                                                                 |
| ----------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `AEGIS_DEPLOY_URL`     | `env` v `cd.yml` (výchozí `http://localhost:3000`)              | Server Aegis, na který se nasazuje. Před prvním použitím to změňte na skutečnou URL vašeho serveru. |
| `AEGIS_DEPLOY_API_KEY` | GitHub repozitář **Settings → Secrets and variables → Actions** | API klíč s oprávněním k nasazení na cílovém serveru.                                                 |

<Note>
Výchozí `AEGIS_DEPLOY_URL` `http://localhost:3000` je pouze zástupná hodnota — z runneru hostovaného GitHubem tato adresa nebude dosažitelná. Před povolením CD ji aktualizujte na veřejnou URL vašeho serveru (nebo použijte self-hosted runner s přístupem do sítě).
</Note>
...
Obě workflowy odkazují na znovupoužitelné akce na `@main`, takže aktualizace akcí v repozitáři `u84u/aegis` se přeberou automaticky. Pokud chcete deterministická sestavení, nahraďte `@main` v každém řádku `uses:` za commit SHA nebo tag vydání.

## Publikování na npm

Publikování na npm zajistí, že bude vaše aplikace dohledatelná v Marketplace Aegis. Jakýkoli pracovní prostor Aegis může procházet, instalovat a aktualizovat aplikace z Marketplace přímo z UI.

### Požadavky

* Účet na [npm](https://www.npmjs.com)
* Klíčové slovo `aegis-app` ve vašem poli `keywords` v souboru `package.json` (přidejte je ručně — ve výchozím nastavení není zahrnuto v šabloně `create-aegis-app`)
...
Server Aegis synchronizuje svůj katalog tržiště z registru npm **každou hodinu**.
...
<Note>
Pokud vaše aplikace nedefinuje `aboutDescription` v `defineApplication()`, tržiště automaticky použije soubor `README.md` vašeho balíčku z npm jako obsah stránky O aplikaci. To znamená, že můžete spravovat jediný soubor README jak pro npm, tak pro tržiště Aegis. Pokud chcete v tržišti jiný popis, explicitně nastavte `aboutDescription`.
</Note>
...
Pro jiné systémy CI (GitLab CI, CircleCI atd.) platí stejné tři příkazy: `yarn install`, `yarn aegis build` a poté `npm publish` z `.aegis/output`.
...
Přejděte na stránku **Nastavení > Aplikace** v Aegis, kde lze procházet a instalovat jak aplikace z tržiště, tak aplikace nasazené jako tarball.
...
K instalaci novější verze ji nejprve nasaďte nebo publikujte, poté znovu spusťte `yarn aegis install`.
</Note>
