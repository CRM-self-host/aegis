...
Webhooky posílají data do vašich systémů v reálném čase, když v Aegis dojde k událostem — bez potřeby průběžného dotazování. Použijte je k udržování externích systémů v synchronizaci, spouštění automatizací nebo zasílání upozornění.
...
Aegis odesílá webhooky pro tyto typy událostí:
...
Aegis kvůli bezpečnosti podepisuje každý požadavek webhooku. Ověřujte podpisy, abyste zajistili, že požadavky jsou autentické.

### Záhlaví

| Hlavička                     | Popis                    |
| ---------------------------- | ------------------------ |
| `X-Aegis-Webhook-Signature` | Podpis HMAC SHA256       |
| `X-Aegis-Webhook-Timestamp` | Časové razítko požadavku |

### Kroky ověření

1. Získejte časové razítko z `X-Aegis-Webhook-Timestamp`
2. Vytvořte řetězec: `{timestamp}:{JSON payload}`
3. Vypočítejte HMAC SHA256 pomocí vašeho tajného klíče webhooku
4. Porovnejte s `X-Aegis-Webhook-Signature`
...
| **Spouštěč webhooku v pracovním postupu** | IN   | Přijímejte data do Aegis z externích systémů                   |
