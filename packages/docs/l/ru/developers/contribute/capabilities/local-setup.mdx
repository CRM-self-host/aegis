...
icon: laptop-code
description: Руководство для участников (или любопытных разработчиков), которые хотят запускать Aegis локально.
---
...
Прежде чем установить и использовать Aegis, убедитесь, что у вас установлено следующее:
...
Aegis требует кэша Redis для обеспечения наилучшей производительности.
...
## Шаг 8: Использовать Aegis

**Frontend**

Фронтенд Aegis будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

* Сервер Aegis будет работать на [http://localhost:3000](http://localhost:3000).
