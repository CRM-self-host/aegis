...
title: 发布
icon: 上传
description: 将你的 Aegis 应用分发到应用市场，或进行内部部署。
---
...
* **部署 tar 包** — 直接将你的应用上传到特定的 Aegis 服务器，以供内部或私有使用。
* **发布到 npm** — 将你的应用在 Aegis 应用市场上架，供任何工作区发现并安装。
...
对于你不希望公开的应用（专有工具、仅供企业使用的集成或实验性构建），你可以将 tar 包直接部署到某台 Aegis 服务器。

### 先决条件

在部署之前，你需要配置一个指向目标服务器的远程。 远程会将服务器 URL 和身份验证凭据本地存储在 `~/.aegis/config.json` 中。
...
使用 `create-aegis-app` 生成的应用开箱即带有两个 GitHub Actions 工作流，位于 `.github/workflows/`。 当你将仓库推送到 GitHub 后即可运行——CI 无需额外设置，CD 只需要一个机密。
...
1. 检出你的应用源代码。
2. 使用 `u84u/aegis/.github/actions/spawn-aegis-app-dev-test@main` 组合 action 启动一个隔离的 Aegis 测试实例（相当于 CI 中的 `yarn aegis server start --test`）。
3. 启用 Corepack，从你的 `.nvmrc` 设置 Node.js，并使用 `yarn install --immutable` 安装依赖。
4. 运行 `yarn test`，并从启动的实例传入 `AEGIS_API_URL` 和 `AEGIS_API_KEY`，以便你的测试可以与真实服务器通信。

**配置选项:**

* `AEGIS_VERSION`（环境变量，默认 `latest`）— 通过在 `ci.yml` 中编辑它来固定 CI 使用的 Aegis 服务器版本。
...
在每次向 `main` 推送时将你的应用部署到已配置的 Aegis 服务器；当为拉取请求添加 `deploy` 标签时，也可从该拉取请求进行部署。

**作用:**

1. 检出 PR 的 head（针对已加标签的 PR），或被推送的提交。
2. 运行 `u84u/aegis/.github/actions/deploy-aegis-app@main`——相当于 CI 中的 `yarn aegis deploy`。
3. 运行 `u84u/aegis/.github/actions/install-aegis-app@main`，将新部署的版本安装到目标工作区。

**必需的配置:**

| 设置                      | 位置                                                       | 目的                                       |
| ----------------------- | -------------------------------------------------------- | ---------------------------------------- |
| `AEGIS_DEPLOY_URL`     | `cd.yml` 中的 `env`（默认值为 `http://localhost:3000`）          | 要部署到的 Aegis 服务器。 首次使用前将其更改为你真实的服务器 URL。 |
| `AEGIS_DEPLOY_API_KEY` | GitHub 仓库 **Settings → Secrets and variables → Actions** | 在目标服务器上具有部署权限的 API 密钥。                   |

<Note>
默认的 `AEGIS_DEPLOY_URL` 值 `http://localhost:3000` 只是占位符——从 GitHub 托管的 runner 无法访问任何资源。 在启用 CD 之前，将其更新为你服务器的公网 URL（或使用具有网络访问权限的自托管 runner）。
</Note>
...
两个工作流都引用了位于 `@main` 的可复用 actions，因此会自动获取 `u84u/aegis` 仓库中的 action 更新。 如果你希望构建具有确定性，请在每个 `uses:` 行中将 `@main` 替换为某个提交的 SHA 或发行标签。

## 发布到 npm

发布到 npm 可让你的应用在 Aegis 应用市场中被发现。 任何 Aegis 工作区都可以直接通过 UI 浏览、安装和升级应用市场中的应用。

### 要求

* 一个 [npm](https://www.npmjs.com) 账户
* 你在 `package.json` 的 `keywords` 数组中的 `aegis-app` 关键字（需要手动添加 — 在 `create-aegis-app` 模板中默认不包含）
...
<Note>
如果您的应用未在 `defineApplication()` 中定义 `aboutDescription`，市场将自动使用 npm 上您的软件包的 `README.md` 作为关于页面内容。 这意味着您可以为 npm 和 Aegis 市场维护同一个 README。 如果您希望在市场中使用不同的描述，请显式设置 `aboutDescription`。
</Note>
...
对于其他 CI 系统（GitLab CI、CircleCI 等），同样适用以下三条命令：`yarn install`、`yarn aegis build`，然后在 `.aegis/output` 目录下执行 `npm publish`。
...
若要安装较新的版本，请先部署或发布它，然后重新运行 `yarn aegis install`。
</Note>
