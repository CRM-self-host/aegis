---
title: Aegis 应用
description: 以代码的形式构建并管理 Aegis 自定义项。
---
...
* **前端组件** — 在 Aegis 的 UI 中渲染的 React 组件（记录页面、命令菜单、侧边面板）
* **AI 技能和智能体** — 通过自定义能力扩展 Aegis 的 AI
...
这将搭建一个新的应用，可选地启动本地 Aegis 服务器，并开始监视你的文件变更。 请参阅[开始使用](/l/zh/developers/extend/apps/getting-started)指南，查看完整演练。
...
| `defineFrontComponent()`           | Aegis 的 UI 中的 React 组件 |
| `defineRole()`                     | 权限角色                    |
| `defineView()`                     | 已保存视图配置                 |
...
1. **`yarn aegis dev`** — 监视源文件、更改时重新构建、同步到服务器、生成类型化的 API 客户端
2. **`yarn aegis build`** — 生成可分发的构建产物
3. **`yarn aegis deploy`** — 部署到远程 Aegis 服务器
4. **`yarn aegis add`** — 交互式生成一个新实体

### CLI 参考

```bash filename="Terminal"
yarn aegis help          # 列出所有命令
yarn aegis server start  # 启动本地开发服务器
yarn aegis remote add    # 连接到 Aegis 服务器
yarn aegis exec -n fn    # 执行逻辑函数
yarn aegis logs -n fn    # 实时查看函数日志
```
