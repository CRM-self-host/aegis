...
Bu kılavuz, Docker Compose kullanarak Aegis uygulamasını kurmak ve yapılandırmak için adım adım yönergeler sunar. Amaç, süreci sadeleştirmek ve kurulumunuzu bozabilecek yaygın sorunları önlemektir.
...
Tek bir komutla en son kararlı Aegis sürümünü yükleyin:
...
```bash
VERSION=vx.y.z BRANCH=dal-adı bash <(curl -sL https://raw.githubusercontent.com/u84u/aegis/main/packages/aegis-docker/scripts/install.sh)
```
...
Eğer u84u'i kendi bilgisayarınızda barındırıyorsanız, tarayıcınızı açın ve [http://localhost:3000](http://localhost:3000) adresine gidin.
...
### Aegis'yi Dış Erişime Açma

Varsayılan olarak, Aegis `localhost` üzerinde `3000` portunda çalışır. Harici bir alan adı veya IP adresi aracılığıyla erişim sağlamak için `.env` dosyanızda `SERVER_URL`'i yapılandırmanız gerekir.
...
Belirli tarayıcı özelliklerinin düzgün çalışabilmesi için SSL (HTTPS) gereklidir. Bu özellikler yerel geliştirme sırasında çalışabilirken (tarayıcılar localhost'u farklı şekilde ele alır), Aegis'yi düzenli bir alanda barındırırken uygun bir SSL kurulumu gereklidir.
...
Optimum güvenlik ve işlevsellik için Aegis'nin SSL sonlandırma ile bir ters proxy arkasında ayarlanmasını şiddetle öneririz.
