---
title: Aegis Uygulamaları
description: Aegis özelleştirmelerini kod olarak oluşturun ve yönetin.
---
...
* **Ön yüz bileşenleri** — Aegis'nin kullanıcı arayüzünde (kayıt sayfaları, komut menüsü, yan paneller) görüntülenen React bileşenleri
* **Yapay zekâ yetenekleri ve ajanları** — Aegis'nin yapay zekâsını özel yeteneklerle genişletin
...
Bu, yeni bir uygulamanın iskeletini oluşturur, isteğe bağlı olarak yerel bir Aegis sunucusunu başlatır ve dosyalarınızdaki değişiklikleri izlemeye başlar. Tam adım adım anlatım için [Başlarken](/l/tr/developers/extend/apps/getting-started) kılavuzuna bakın.
...
| `defineFrontComponent()`           | Aegis'nin kullanıcı arayüzündeki React bileşenleri        |
...
1. **`yarn aegis dev`** — kaynak dosyaları izler, değişiklikte yeniden derler, sunucuyla senkronize eder, tipli API istemcileri üretir
2. **`yarn aegis build`** — dağıtılabilir bir derleme üretir
3. **`yarn aegis deploy`** — uzak bir Aegis sunucusuna dağıtır
4. **`yarn aegis add`** — etkileşimli olarak yeni bir varlık iskeleti oluşturur

### CLI başvurusu

```bash filename="Terminal"
yarn aegis help          # Tüm komutları listele
yarn aegis server start  # Yerel geliştirme sunucusunu başlat
yarn aegis remote add    # Bir Aegis sunucusuna bağlan
yarn aegis exec -n fn    # Bir mantık fonksiyonunu çalıştır
yarn aegis logs -n fn    # Fonksiyon günlüklerini izle
```
