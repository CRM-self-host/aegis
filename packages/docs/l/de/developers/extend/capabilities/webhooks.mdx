...
Webhooks übermitteln Daten in Echtzeit an Ihre Systeme, wenn Ereignisse in Aegis auftreten — kein Polling erforderlich. Verwenden Sie sie, um externe Systeme synchron zu halten, Automatisierungen auszulösen oder Benachrichtigungen zu senden.
...
Aegis sendet Webhooks für diese Ereignistypen:
...
Aegis signiert jede Webhook-Anfrage zu Sicherheitszwecken. Validieren Sie die Signaturen, um sicherzustellen, dass die Anfragen authentisch sind.

### Header

| Kopfzeile                    | Beschreibung            |
| ---------------------------- | ----------------------- |
| `X-Aegis-Webhook-Signature` | HMAC-SHA256-Signatur    |
| `X-Aegis-Webhook-Timestamp` | Zeitstempel der Anfrage |

### Validierungsschritte

1. Den Zeitstempel aus `X-Aegis-Webhook-Timestamp` abrufen
2. Zeichenfolge erstellen: `{timestamp}:{JSON payload}`
3. Den HMAC-SHA256-Hash mit Ihrem Webhook-Geheimnis berechnen
4. Mit `X-Aegis-Webhook-Signature` vergleichen
...
| **Workflow-Webhook-Trigger** | IN       | Daten aus externen Systemen in Aegis empfangen                                  |
