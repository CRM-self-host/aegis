...
### Relations courantes dans Aegis
...
Le parent (côté "un") doit exister dans Aegis avant que vous ne puissiez y faire référence.
...
1. Exportez les enregistrements parents depuis Aegis
...
<Warning>
  **Le format du domaine est important !**
  ...
  Utilisez `https://domain.com` (et non simplement `domain.com`). Cela correspond à la manière dont Aegis stocke les domaines des Entreprises et évite les erreurs d'association.
</Warning>
...
Aegis liera automatiquement chaque enregistrement enfant au parent correspondant.
...
| **Enregistrements inexistants** | L'enregistrement parent n'existe pas dans Aegis            | Importez d'abord les enregistrements parents, ou vérifiez les fautes de frappe |
| **Incohérence de casse**        | `Acme.com` dans le fichier mais `acme.com` dans Aegis      | Assurez une correspondance exacte de la casse                                  |
...
<AccordionGroup>
  <Accordion title="Que faire si mon enregistrement parent n'a pas de domaine ni d'e-mail ?">
    ...
    1. Utilisez l'`id` Aegis (exportez les enregistrements parents pour obtenir leurs identifiants)
