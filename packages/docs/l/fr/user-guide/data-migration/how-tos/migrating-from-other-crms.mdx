...
description: Guide pas à pas pour migrer vos données de n'importe quel CRM vers Aegis.
---

## Vue d'ensemble

Ce guide vous accompagne pour migrer vos données depuis n'importe quel CRM vers Aegis. Le processus consiste à auditer vos données, à préparer votre espace de travail sur Aegis, à exporter depuis votre système actuel et à importer dans Aegis.
...
Créez un document de correspondance entre votre CRM actuel et Aegis :
...
## Étape 3 : Configurez votre espace de travail Aegis

Avant d'importer des données, préparez votre espace de travail Aegis :
...
<Warning>
  **Invitez les utilisateurs AVANT d'importer des données.**
  ...
  Si vos données incluent des références à des utilisateurs (propriétaire du compte, assigné, etc.), ces utilisateurs doivent exister dans Aegis avant l'importation. Sinon, ces relations ne pourront pas être mises en correspondance.
</Warning>
...
Ouvrez chaque CSV exporté dans une application de tableur et préparez-le pour Aegis.
...
3. Vérifiez qu'il n'existe pas déjà de doublons dans Aegis
...
<Warning>
  **Le format du domaine est critique.**
  ...
  Utilisez `https://domain.com` (pas `domain.com` ni `www.domain.com`). Cela correspond au format de Aegis et évite les doublons lorsque vous connectez la synchronisation e-mail/calendrier.
</Warning>
...
2. Utilisez les mêmes adresses e-mail que celles utilisées par les utilisateurs pour rejoindre votre espace de travail Aegis
...
1. Accédez à l'objet dans Aegis
...
* Aegis commencera à synchroniser les e-mails avec les enregistrements de contacts
...
<AccordionGroup>
  <Accordion title="Puis-je migrer les flux de travail automatiquement ?">
    Pas pour le moment. Les flux de travail doivent être recréés manuellement dans Aegis.
  </Accordion>
...
Contactez [contact@aegis.com](mailto:contact@aegis.com) ou découvrez nos [services de mise en œuvre](/l/fr/user-guide/getting-started/capabilities/implementation-services).
