...
Les webhooks envoient des données à vos systèmes en temps réel lorsque des événements se produisent dans Aegis — aucune interrogation n'est nécessaire. Utilisez-les pour maintenir les systèmes externes synchronisés, déclencher des automatisations ou envoyer des alertes.
...
Aegis envoie des webhooks pour ces types d'événements :
...
Aegis signe chaque requête de webhook pour des raisons de sécurité. Validez les signatures pour garantir l'authenticité des requêtes.

### En-têtes

| En-tête                      | Description              |
| ---------------------------- | ------------------------ |
| `X-Aegis-Webhook-Signature` | Signature HMAC SHA256    |
| `X-Aegis-Webhook-Timestamp` | Horodatage de la requête |

### Étapes de validation

1. Récupérez l'horodatage depuis `X-Aegis-Webhook-Timestamp`
2. Créez la chaîne : `{timestamp}:{JSON payload}`
3. Calculez le HMAC SHA256 en utilisant votre secret de webhook
4. Comparez avec `X-Aegis-Webhook-Signature`
...
const timestamp = req.headers["x-aegis-webhook-timestamp"];
...
const isValid = expectedSignature === req.headers["x-aegis-webhook-signature"];
```
...
| **Déclencheur de webhook de workflow** | ENTRÉE    | Recevoir des données dans Aegis depuis des systèmes externes                         |
