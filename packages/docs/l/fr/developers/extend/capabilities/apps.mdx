...
Voir aussi : les pages de référence CLI pour [create-aegis-app](https://www.npmjs.com/package/create-aegis-app) et [twenty-sdk CLI](https://www.npmjs.com/package/twenty-sdk).
...
* **package.json** : Déclare le nom de l’application, la version, les moteurs (Node 24+, Yarn 4), et ajoute `twenty-sdk` ainsi que des scripts comme `app:dev`, `entity:add`, `function:logs`, `function:execute`, `app:uninstall` et `auth:login` qui délèguent à la CLI locale `aegis`.
...
* `yarn app:dev` génère automatiquement le client Aegis typé dans `node_modules/twenty-sdk/generated`.
...
`yarn app:dev` génère automatiquement le client Aegis typé dans `node_modules/twenty-sdk/generated`. Utilisez-le dans vos fonctions :
...
Même si nous recommandons d’utiliser `create-aegis-app` pour une expérience de démarrage optimale, vous pouvez également configurer un projet manuellement. N’installez pas la CLI globalement. Ajoutez plutôt `twenty-sdk` comme dépendance locale et reliez des scripts dans votre package.json :
