...
```bash
"npx nx run aegis-front:lint # مرر --fix لإصلاح أخطاء التدقيق"
```
...
```bash
"npx nx run aegis-front:test # تشغيل اختبارات jest\nnpx nx run aegis-front:storybook:serve:dev # تشغيل storybook\nnpx nx run aegis-front:storybook:test # تشغيل الاختبارات # (يحتاج إلى تشغيل yarn storybook:serve:dev)\nnpx nx run aegis-front:storybook:coverage # (يحتاج إلى تشغيل yarn storybook:serve:dev)"
```
