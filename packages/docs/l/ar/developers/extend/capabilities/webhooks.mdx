...
تدفع خطافات الويب البيانات إلى أنظمتك في الوقت الفعلي عند وقوع أحداث في Aegis — دون الحاجة إلى الاستطلاع الدوري. استخدمها للحفاظ على تزامن الأنظمة الخارجية، وتشغيل الأتمتة، أو إرسال التنبيهات.
...
يرسل Aegis خطافات الويب لأنواع الأحداث التالية:
...
يقوم Aegis بتوقيع كل طلب خطاف ويب لأغراض الأمان. تحقّق من التواقيع للتأكد من أن الطلبات أصيلة.

### الرؤوس

| رأس الصفحة                   | الوصف               |
| ---------------------------- | ------------------- |
| `X-Aegis-Webhook-Signature` | توقيع HMAC SHA256   |
| `X-Aegis-Webhook-Timestamp` | الطابع الزمني للطلب |

### خطوات التحقق

1. احصل على الطابع الزمني من `X-Aegis-Webhook-Timestamp`
2. أنشئ السلسلة: `{timestamp}:{JSON payload}`
3. احسب HMAC SHA256 باستخدام سر خطاف الويب الخاص بك
4. قارِن مع `X-Aegis-Webhook-Signature`
...
| **مشغّل خطاف ويب لسير العمل** | IN      | استقبال البيانات في Aegis من الأنظمة الخارجية              |
