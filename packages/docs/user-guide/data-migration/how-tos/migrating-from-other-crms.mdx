...
description: Step-by-step guide to migrate your data from any CRM to Aegis.
---

## Overview

This guide walks you through migrating your data from any CRM to Aegis. The process involves auditing your data, preparing your Aegis workspace, exporting from your current system, and importing into Aegis.
...
Create a mapping document between your current CRM and Aegis:
...
## Step 3: Set Up Your Aegis Workspace

Before importing data, prepare your Aegis workspace:
...
If your data includes user references (Account Owner, Assignee, etc.), those users must exist in Aegis before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for Aegis.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in Aegis already
...
Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches Aegis's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
1. Navigate to the object in Aegis
...
- Aegis will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in Aegis.
  </Accordion>
...
Contact [contact@aegis.com](mailto:contact@aegis.com) or explore our [Implementation Services](/user-guide/getting-started/capabilities/implementation-services).
