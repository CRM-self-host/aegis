...
The parent ("one" side) must exist in Aegis before you can reference it.
...
If you need the `id`:
1. Export the parent records from Aegis
...
Use `https://domain.com` (not just `domain.com`). This matches how Aegis stores Company domains and prevents matching errors.
</Warning>
...
Aegis will automatically link each child record to the matching parent.
...
| **Non-existent records** | The parent record doesn't exist in Aegis | Import parent records first, or check for typos |
| **Case mismatch** | `Acme.com` in file but `acme.com` in Aegis | Ensure exact case matching |
...
<AccordionGroup>
  <Accordion title="What if my parent record doesn't have a domain or email?">
    ...
    1. Use the Aegis `id` (export parent records to get their IDs)
