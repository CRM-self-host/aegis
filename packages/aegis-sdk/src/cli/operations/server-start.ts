...
const innerServerStart = async (
  ...
  const volumeData = isTest ? 'aegis-app-dev-test-data' : 'aegis-app-dev-data';
  ...
  if (existingUrl) {
    ...
    onProgress?.(`Aegis server detected on ${existingUrl}`);
    ...
    if (!healthy) {
      return {
        ...
        error: {
          ...
          message:
            'Aegis server did not become healthy in time.\n' +
            "Check: 'yarn aegis server logs'",
        },
      };
    }
  ...
  if (containerExists(containerName)) {
    ...
    if (existingPort !== port) {
      onProgress?.(
        `Existing container uses port ${existingPort}. Run 'yarn aegis server reset${isTest ? ' --test' : ''}' first to change ports.`,
      );
    }
    ...
    onProgress?.('Starting Aegis container...');
    ...
    const runResult = spawnSync(
      ...
      [
        ...
        '-v',
        `${volumeStorage}:/app/packages/aegis-server/.local-storage`,
        IMAGE,
      ],
      { stdio: 'inherit' },
    );
    ...
    if (runResult.status !== 0) {
      return {
        ...
        error: {
          ...
          message: 'Failed to start Aegis container.',
        },
      };
    }
  }

  onProgress?.('Waiting for Aegis to be ready...');
