...
import { OUTPUT_DIR, type Manifest } from 'aegis-shared/application';
  ...
  if (!validateAuth.serverUp) {
    return {
      ...
      error: {
        ...
        message:
          'Cannot reach Aegis server.\n\n' +
          '  Start a local server:\n' +
          '    yarn aegis server start\n\n' +
          '  Check server status:\n' +
          '    yarn aegis server status',
      },
    };
  }
  ...
  if (!validateAuth.authValid) {
    return {
      ...
      error: {
        code: APP_ERROR_CODES.SYNC_FAILED,
        message:
          'Authentication failed. Run `yarn aegis remote add --local` to authenticate.',
      },
    };
  }
