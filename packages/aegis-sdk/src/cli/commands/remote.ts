...
export const registerRemoteCommands = (program: Command): void => {
  const remote = program
    .command('remote')
    .description('Manage remote Aegis servers');
  ...
  remote
    ...
    .option('--local', 'Connect to a local Aegis server (auto-detect)')
        ...
        if (!apiUrl) {
          ...
          if (options.local) {
            if (!detectedUrl) {
              console.error(
                chalk.red(
                  'No local Aegis server found.\n' +
                    'Start one with: yarn aegis server start',
                ),
              );
              process.exit(1);
            }
  ...
  remote
    ...
    .action(async () => {
      ...
      if (remotes.length === 0) {
        ...
        console.log("Use 'aegis remote add' to add one.");
      ...
      for (const remoteName of remotes) {
        ...
        const authMethod = config.aegisCLIAccessToken
          ? 'oauth'
          : config.apiKey
            ? 'api-key'
            : 'none';
      ...
      console.log(
        '\n',
        chalk.gray("Use 'aegis remote switch <name>' to change default"),
      );
    });
