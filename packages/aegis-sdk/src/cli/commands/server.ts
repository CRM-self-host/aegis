...
export const registerServerCommands = (program: Command): void => {
  const server = program
    .command('server')
    .description('Manage a local Aegis server instance');

  server
    .command('start')
    .description('Start a local Aegis server')
  ...
  server
    .command('stop')
    .description('Stop the local Aegis server')
    ...
    .action((options: { test?: boolean }) => {
      ...
      if (!containerExists(containerName)) {
        console.log(chalk.yellow('No Aegis server container found.'));

        return;
      }

      execSync(`docker stop ${containerName}`, { stdio: 'ignore' });
      console.log(chalk.green('Aegis server stopped.'));
    });

  server
    .command('logs')
    .description('Stream Aegis server logs')
  ...
  server
    .command('status')
    .description('Show Aegis server status')
    ...
    .action(async (options: { test?: boolean }) => {
      ...
      if (!containerExists(containerName)) {
        ...
        console.log(
          chalk.gray(
            `  Run 'yarn aegis server start${options.test ? ' --test' : ''}' to create one.`,
          ),
        );
  ...
  server
    ...
    .action((options: { test?: boolean }) => {
      ...
      const volumeData = options.test
        ? 'aegis-app-dev-test-data'
        : 'aegis-app-dev-data';
      const volumeStorage = options.test
        ? 'aegis-app-dev-test-storage'
        : 'aegis-app-dev-storage';
      ...
      console.log(chalk.green('Aegis server data reset.'));
      console.log(
        chalk.gray(
          `Run 'yarn aegis server start${options.test ? ' --test' : ''}' to start a fresh instance.`,
        ),
      );
    });
};
