...
const getAppDeclaredSdkVersion = async (
  ...
  try {
    ...
    return (
      pkg.dependencies?.['aegis-sdk'] ??
      pkg.devDependencies?.['aegis-sdk'] ??
      undefined
    );
  } catch {
    return undefined;
...
export const checkSdkVersionCompatibility = async (
  ...
  if (cliMajor !== appMajor) {
    console.warn(
      chalk.yellow(
        `⚠ Version mismatch: your app requires aegis-sdk@${appMajor}.x ` +
          `but the CLI running is v${cliVersion}. ` +
          `Major version mismatches may cause unexpected behavior.\n` +
          `  Update with: npm install -g aegis-sdk@${appMajor}`,
      ),
    );
    console.warn('');
  }
