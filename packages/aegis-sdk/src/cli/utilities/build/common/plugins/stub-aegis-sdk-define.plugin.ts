...
// Everything in aegis-sdk/define is build-time metadata for the manifest
// extractor and is dead code in the user-app bundle. Stubbing it drops zod,
// aegis-shared and ~1MB per logic-function bundle.
//
// Exports are derived from `Object.keys(aegisSdkDefine)` so new ones land
...
export const AEGIS_SDK_DEFINE_STUBBED_EXPORTS = partitionDefineExports(
  aegisSdkDefine as unknown as Record<string, unknown>,
);

const VIRTUAL_NAMESPACE = 'aegis-sdk-define-stub';
const STUB_RESOLVED_PATH = '__aegis-sdk-define-stub__';

const STUB_PRELUDE = `
// Auto-generated stub for aegis-sdk/define injected by the SDK CLI build.
// Real implementations would pull in zod, aegis-shared and ~1MB of code; at
...
const buildStubModuleSource = (): string => {
  ...
  for (const name of AEGIS_SDK_DEFINE_STUBBED_EXPORTS.any) {
...
export const createStubAegisSdkDefinePlugin = (): esbuild.Plugin => ({
  name: 'aegis-sdk-define-stub',
  setup(build) {
    build.onResolve({ filter: /^aegis-sdk\/define$/ }, () => ({
