...
export type RemoteConfig = {
  apiUrl: string;
  apiKey?: string;
  // CLI OAuth app credentials (from `yarn aegis remote add`)
  aegisCLIRegistrationId?: string;
  aegisCLIRegistrationClientId?: string;
  aegisCLIAccessToken?: string;
  aegisCLIRefreshToken?: string;
...
export class ConfigService {
  ...
  // to the current format (remotes, aegisCLIAccessToken/aegisCLIRefreshToken)
  private async migrateConfigIfNeeded(
    ...
    const migrateRemoteFields = (
      ...
      aegisCLIRegistrationClientId:
        str(source.aegisCLIRegistrationClientId) ?? str(source.oauthClientId),
      aegisCLIAccessToken:
        str(source.aegisCLIAccessToken) ??
        str(source.accessToken) ??
        str(source.applicationAccessToken),
      aegisCLIRefreshToken:
        str(source.aegisCLIRefreshToken) ??
  ...
  async getConfigForRemote(remoteName: string): Promise<RemoteConfig> {
    ...
    try {
      ...
      return {
        ...
        aegisCLIRegistrationId: remoteConfig.aegisCLIRegistrationId,
        aegisCLIRegistrationClientId: remoteConfig.aegisCLIRegistrationClientId,
        aegisCLIAccessToken: remoteConfig.aegisCLIAccessToken,
        aegisCLIRefreshToken: remoteConfig.aegisCLIRefreshToken,
