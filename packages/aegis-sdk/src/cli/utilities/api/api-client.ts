...
export class ApiClient {
  ...
  constructor(options?: {
    ...
    this.client.interceptors.request.use(async (config) => {
      const aegisConfig = await this.configService.getConfig();

      config.baseURL = this.serverUrlOverride ?? aegisConfig.apiUrl;
    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        if (error.response?.status === 401) {
          console.error(
            chalk.red(
              'Authentication failed. Run `yarn aegis remote add` to authenticate.',
            ),
          );
          ...
          console.error(
            chalk.red('Cannot connect to Aegis server. Is it running?'),
          );
        }
        throw error;
      },
  ...
  async refreshToken(): Promise<string | null> {
    ...
    if (!config.aegisCLIRefreshToken || !config.aegisCLIRegistrationClientId) {
      return null;
    }
    ...
    try {
      const tokenResponse = await axios.post(`${config.apiUrl}/oauth/token`, {
        ...
        refresh_token: config.aegisCLIRefreshToken,
        client_id: config.aegisCLIRegistrationClientId,
      });
      ...
      await this.configService.setConfig({
        aegisCLIAccessToken: newAccessToken,
        ...(newRefreshToken ? { aegisCLIRefreshToken: newRefreshToken } : {}),
      });
  ...
  async resolveAuthToken(): Promise<string | undefined> {
    ...
    const config = await this.configService.getConfig();
    const cliToken = config.aegisCLIAccessToken;
