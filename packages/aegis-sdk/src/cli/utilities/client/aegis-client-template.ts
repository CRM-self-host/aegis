...
const APP_ACCESS_TOKEN_ENV_KEY = 'AEGIS_APP_ACCESS_TOKEN';
const API_KEY_ENV_KEY = 'AEGIS_API_KEY';

type AegisGeneratedClientOptions = ClientOptions;
...
const defaultOptions: AegisGeneratedClientOptions = {
  url: '__AEGIS_DEFAULT_URL__',
  headers: {
    'Content-Type': 'application/json',
  },
};

export class AegisGeneratedClient {
  ...
  constructor(options?: AegisGeneratedClientOptions) {
    const merged: AegisGeneratedClientOptions = {
      ...defaultOptions,
      ...options,
    };
    ...
    if (!this.fetchImplementation) {
      throw new Error(
        'Global `fetch` function is not available, ' +
          'pass a fetch implementation to the Aegis client',
      );
    }
    ...
    if (!this.refreshAccessTokenPromise) {
      this.refreshAccessTokenPromise = refreshAccessTokenFunction()
        ...
        .catch((refreshError: unknown) => {
          console.error('Aegis client: token refresh failed', refreshError);
